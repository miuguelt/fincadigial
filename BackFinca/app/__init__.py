...
def create_app(config_name='development'):
    ...
    if config_name == 'testing':
        ...
        @test_err_bp.route('/api/test-exceptions/<string:exc_type>', methods=['GET', 'POST'])
        def trigger_exception(exc_type):
            ...
            if exc_type == 'business':
                raise BusinessRuleException("Regla de negocio rota", status_code=400, error_code="TEST_BUSINESS", details={"reason": "test"})
            elif exc_type == 'not-found':
                raise ResourceNotFoundException("Recurso no encontrado", resource_name="Animal", resource_id=123)
                ...
                raise ValidationError("Error de validación interna", errors=["Campo X es inválido"])
