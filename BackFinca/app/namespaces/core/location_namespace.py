...
class LocationReport(Resource):
    ...
    def post(self):
        ...
        if not finca_id:
            return APIResponse.error(message="Contexto de finca requerido para reportar ubicación", status_code=400)
        ...
        if not user:
            return APIResponse.error(message="Usuario no encontrado o sesión inválida", status_code=401)
        ...
        if not UserFinca.has_access(user_id, finca_id):
            return APIResponse.error(message="No tienes permiso para reportar ubicación en esta finca", status_code=403)
...
class LocationBatchReport(Resource):
    ...
    def post(self):
        ...
        if not finca_id:
            return APIResponse.error(message="Contexto de finca requerido para reportar ubicaciones", status_code=400)
        ...
        if not UserFinca.has_access(user_id, finca_id):
            return APIResponse.error(message="No tienes permiso para subir datos a esta finca", status_code=403)

        # Obtener los IDs de todos los usuarios con acceso activo a esta finca
        valid_user_ids = {m['user_id'] for m in UserFinca.get_finca_users(finca_id, active_only=True)}
...
class LatestLocations(Resource):
    ...
    def get(self):
        ...
        subquery = apply_tenant_filter(subquery_base, UserLocation, finca_id=finca_id).group_by(UserLocation.user_id).subquery()
        ...
        latest_locs = apply_tenant_filter(latest_locs_base, UserLocation, finca_id=finca_id).all()
