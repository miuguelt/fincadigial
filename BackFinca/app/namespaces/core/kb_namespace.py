...
kb_ns = Namespace('knowledge_base', description='Motor de Recomendaciones Agropecuarias (Offline-First)')
...
regla_model = kb_ns.model('KBRegla', {
...
recomendacion_model = kb_ns.model('KBRecomendacion', {
    'id': fields.Integer(),
...
calendario_model = kb_ns.model('KBCalendario', {
    'codigo': fields.String(),
...
categorias_model = kb_ns.model('KBCategorias', {
    'categoria': fields.String(),
...
class AnimalRecomendaciones(Resource):
    ...
    def get(self, animal_id):
        ...
        try:
            ...
            return APIResponse.success(resultados, "Recomendaciones generadas exitosamente")
        ...
        try:
            ...
            for animal in animales[:100]:  # Limitar a 100 para performance en VPS
                ...
                for cal in calendarios:
                    if cal.sexo.value not in ('Ambos', (animal.sex.value if animal.sex else 'Ambos')):
                        continue
            ...
            return APIResponse.success(proximos, f"{len(proximos)} eventos pendientes en el hato")
        except Exception as e:
            return APIResponse.error(str(e), status_code=500)
