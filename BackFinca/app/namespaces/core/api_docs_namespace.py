...
class PostmanCollectionResource(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            if not base_url:
                base_url = flask.current_app.config.get('API_BASE_URL_NO_VERSION') or flask.request.host_url.rstrip('/') or ''
...
class OpenAPISpecResource(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            base_url_config = flask.current_app.config.get('API_BASE_URL') or f"{flask.request.host_url.rstrip('/')}/api/v1"
            spec['servers'] = [
