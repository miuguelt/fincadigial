...
def _get_generic_ref_points():
    ...
    if entry and entry.extra and isinstance(entry.extra, list):
        return [(p['age'], p['weight']) for p in entry.extra if 'age' in p and 'weight' in p]
    return [
...
def _build_animal_growth(animal, controls):
    ...
    for c in sorted_controls:
        ...
        expected_w, min_w, expected_adg = _expected_weight(animal, age_months) if age_months is not None else (None, None, None)
        data_points.append({
            ...
            'deviation_pct': round((c.weight - expected_w) / expected_w * 100, 1) if (c.weight and expected_w) else None,
            'growth_stage': _get_growth_stage(age_months).value if age_months is not None else None,
        })
    ...
    if animal.birth_date and last_w:
        ...
        if current_expected:
            current_deviation = round((last_w - current_expected) / current_expected * 100, 1)
    ...
    if adg is not None and animal.birth_date:
        ...
        if age_now < 12:
            ...
            projected_weight_12m = round(last_w + adg * months_to_12 * 30.44, 1) if last_w else None
    ...
    return {
        ...
        'stats': {
            ...
            'growth_score': score,
            'growth_stage': _get_growth_stage((today - animal.birth_date).days / 30.44).value if animal.birth_date else None,
        },
    }
...
class GrowthProjection(Resource):
    ...
    def get(self, animal_id):
        ...
        if len(controls) < 2:
            return APIResponse.error('Se necesitan al menos 2 controles para proyectar', status_code=400)
        ...
        if len(weights) < 2:
            return APIResponse.error('Se necesitan al menos 2 controles con peso', status_code=400)
        ...
        current_age = (today - animal.birth_date).days / 30.44 if animal.birth_date else 0
        ...
        for target_age, label in milestones:
            if target_age > current_age:
                ...
                projections.append({
                    ...
                    'meets_target': projected_w >= (min_w or expected_w or 0) if expected_w else None,
                })
...
class AnimalBCS(Resource):
    ...
    def get(self, animal_id):
        ...
        bcs_records = BodyConditionScore.query.filter_by(animal_id=animal_id)\
            .order_by(BodyConditionScore.score_date.desc()).all()
...
class HerdBCS(Resource):
    ...
    def get(self):
        ...
        critical_animals = BodyConditionScore.query.filter(
            BodyConditionScore.finca_id == finca_id,
            BodyConditionScore.score <= 3
        ).order_by(BodyConditionScore.score.asc()).limit(20).all()
...
class BreedStandardsList(Resource):
    ...
    def get(self):
        ...
        standards = query.order_by(BreedGrowthStandard.breed_id, BreedGrowthStandard.age_months).all()
