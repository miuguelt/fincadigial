...
class MilkProductionAnimalTrend(Resource):
    @milk_ns.doc('get_animal_trend', description='Obtener tendencia de producción para un animal')
    @jwt_required()
    def get(self, animal_id):
        ...
        try:
            ...
            return APIResponse.success(data=trend, message='Tendencia de producción obtenida')
...
class MilkRevenueEstimate(Resource):
    ...
    def get(self):
        ...
        try:
            estimate = MilkProductionService.estimate_revenue(finca_id, price_per_liter, year, month)
