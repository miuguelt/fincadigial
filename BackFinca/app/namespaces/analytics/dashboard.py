...
class DashboardSummary(Resource):
    ...
    @jwt_required()
    @safe_cached(timeout=60, make_cache_key=lambda *args, **kwargs: _dashboard_cache_key('dashboard_basic'))
    def get(self):
...
class CompleteStats(Resource):
    @dashboard_ns.doc(security=['Bearer', 'Cookie'])
    @jwt_required()
    @safe_cached(timeout=120, make_cache_key=lambda *args, **kwargs: _dashboard_cache_key('dashboard_complete'))
    def get(self):
