...
ai_insight_model = legacy_ns.model('AIInsight', {'insight': fields.String(description='Contenido del análisis desde BD'), 'generated_at': fields.DateTime(description='Fecha de generación')})
...
class FieldHealthMap(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            default_status_map = {'Activo': 'healthy', 'Disponible': 'healthy', 'Ocupado': 'warning', 'Mantenimiento': 'resting', 'Restringido': 'critical', 'Dañado': 'critical'}
            status_entry = SystemContent.get_by_key('config.field_status_map')
            status_mapping = status_entry.extra if (status_entry and status_entry.extra) else default_status_map
            result = []
            for idx, field in enumerate(fields):
                ...
                result.append({'id': field.id, 'name': field.name, 'status': status, 'occupation': occupation, 'capacity': capacity, 'coords': coords, 'state': str(field.state.value), 'area': field.area})
