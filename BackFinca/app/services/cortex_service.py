...
"""
    ...
    def call_claude(prompt: str, role: PromptRole = PromptRole.ANALYST, max_tokens: int = 500) -> dict:
        ...
        logger.warning('No hay contenido en DB para key=%s y OLLAMA deshabilitado.', content_key)
        return None

    @staticmethod
    def _call_ollama(prompt: str, role: PromptRole, max_tokens: int) -> dict:
        if _circuit_is_open():
            logger.warning('Ollama circuit breaker activo — no hay contenido disponible')
            return {
