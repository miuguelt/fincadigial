...
class ExportService:
    ...
    def export_vaccinations_excel(params):
        ...
        if v := params.get('from_date'):
            ...
            except ValueError: logger.warning("Invalid from_date filter: %s", v)
        if v := params.get('to_date'):
            try: conditions.append(Vaccinations.vaccination_date <= date.fromisoformat(v))
            except ValueError: logger.warning("Invalid to_date filter: %s", v)
        if conditions:
    ...
    def export_reproduction_excel(params):
        ...
        if v := params.get('event_type'):
            ...
            except ValueError: logger.warning("Invalid event_type filter: %s", v)
    ...
    def export_bulk_health_pdf(animal_ids_str):
        if animal_ids_str == 'all':
            ...
            animal_ids = [a.id for a in Animals.query.filter_by(finca_id=finca_id).all()]
        else:
            try:
                animal_ids = [int(i.strip()) for i in animal_ids_str.split(',') if i.strip()]
            except (ValueError, TypeError):
                return None, "animal_ids debe ser una lista de enteros separados por coma o 'all'"
    ...
    def export_milk_production_excel(params):
        ...
        if v := params.get('from_date'):
            ...
            except ValueError: logger.warning("Invalid milk from_date filter: %s", v)
        if v := params.get('to_date'):
            try: query = query.filter(MilkProduction.date <= date.fromisoformat(v))
            except ValueError: logger.warning("Invalid milk to_date filter: %s", v)
        ...
        for r in records:
            ws.append([r.id, _fmt_date(r.date), r.animals.record if hasattr(r, 'animals') and r.animals else r.animal_id, r.liters, _fmt_enum(r.milking_session), r.fat_percentage, r.protein_percentage, r.somatic_cells, r.notes or ''])
    ...
    def export_multi_finca_general_pdf(user_id):
        ...
        return io.BytesIO(pdf_bytes), f"reporte_general_multi_finca_{date.today().isoformat()}.pdf"
    ...
    def export_finca_detail_pdf(user_id, finca_id):
        ...
        return io.BytesIO(pdf_bytes), f"reporte_finca_{finca.name.replace(' ', '_')}_{date.today().isoformat()}.pdf"
