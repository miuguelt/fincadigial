    ...
    for a_id in animal_ids:
        ...
        if controls:
            _add_compact_table(pdf, 'Últimos Controles de Salud', ['Fecha', 'Estado', 'Peso'],
                               [[c.checkup_date.isoformat(), c.health_status.value, f"{c.weight:.1f} kg" if c.weight is not None else "-"] for c in controls])
...
def _build_health_pdf(animal, controls, vaccinations, treatments, diseases):
    ...
    def resolve_image_path(filepath):
        ...
        if 'static/uploads/animals/' in filepath.replace('\\', '/'):
            ...
            animals_root = os.path.normpath(
                os.path.join(flask.current_app.root_path, '..', 'static', 'uploads', 'animals')
            )
    ...
    class PDF(FPDF):
        def header(self):
            ...
            self.cell(0, 5, 'FICHA GANADERA DE CONTROL SANITARIO', new_x='LMARGIN', new_y='NEXT')
            self.ln(7)
        ...
        def footer(self):
            ...
            self.ln(2)
            self.set_x(15)
            self.cell(0, 10, f'Página {self.page_no()} | Finca Villa Luz | Generado el {date.today().strftime("%d/%m/%Y")}', align='C')

    pdf = PDF()
    pdf.set_auto_page_break(auto=True, margin=20)
    pdf.add_page()
    pdf.set_margins(15, 28, 15)  # Margen superior incrementado para acomodar el banner superior
    ...
    info_rows = [
        [('Código:', animal.record), ('Raza:', animal.breed.name if animal.breed else '-')],
        [('Sexo:', _fmt_enum(animal.sex)), ('Edad:', f"{animal.age_in_months} meses" if animal.age_in_months else '-')],
        [('Peso:', f"{animal.weight:.1f} kg" if animal.weight is not None else "-"), ('Estado:', _fmt_enum(animal.status))],
    ...
    if primary_image and primary_image.filepath:
        ...
        if full_path:
            ...
            try:
                ...
                pdf.image(image_path, x=photo_x + 1, y=start_y + 1, w=photo_w - 2, h=card_height - 2)
                photo_rendered = True
    ...
    if animal.sex == Sex.Hembra or str(animal.sex) == 'Sex.Hembra' or str(animal.sex) == 'Hembra':
        ...
        calving_str = _fmt_date(animal.last_calving_date) if animal.last_calving_date else '-'
        ...
        purchase_str = _fmt_date(animal.purchase_date) if animal.purchase_date else 'Nacimiento en Finca'
    ...
    def section(title, rows, col_headers, col_widths):
        ...
        pdf.set_font('Helvetica', 'B', 11)
        pdf.set_text_color(27, 94, 32)
        pdf.cell(0, 8, f'{title} ({len(rows)} registros)', new_x='LMARGIN', new_y='NEXT')
        pdf.set_text_color(33, 33, 33)

        if not rows:
            pdf.set_font('Helvetica', 'I', 9)
            pdf.set_text_color(117, 117, 117)
            pdf.cell(0, 6, 'Sin registros históricos registrados.', new_x='LMARGIN', new_y='NEXT')
            pdf.set_text_color(33, 33, 33)
            pdf.ln(2.5)
            return
        ...
        for idx, row in enumerate(rows):
            ...
            for val, w in zip(row, col_widths):
                ...
                pdf.cell(w, 6, text, border='B', fill=fill, align='C' if '/' in text or 'kg' in text or 'cm' in text else 'L')
            pdf.ln()
        pdf.ln(3.5)
    ...
    section(
        ...
        [[_fmt_date(c.checkup_date), _fmt_enum(c.health_status),
          ...
          c.description or 'Sin observaciones'] for c in controls],
    ...
    section(
        ...
        [[_fmt_date(t.treatment_date),
          ...
          t.frequency or '-',
          t.dosis or '-',
          t.observations or '-'] for t in treatments],
...
def _build_multi_finca_general_pdf(fincas_data, user_fullname):
    ...
    class PDF(FPDF):
        def header(self):
            ...
            self.set_text_color(100)
            self.cell(0, 8, 'REPORTE COMPARATIVO GENERAL MULTI-FINCA', ln=True, align='C')
            self.ln(5)
        ...
        def footer(self):
            ...
            self.set_font('Helvetica', 'I', 8)
            self.set_text_color(150)
            self.cell(0, 10, f'Página {self.page_no()} | Generado el {datetime.now().strftime("%d/%m/%Y %H:%M")} por {user_fullname}', align='C')
    ...
    headers = ['Finca', 'Ubicación', 'Animales', 'Leche (L)', 'Ingresos ($)', 'Egresos ($)', 'Neto ($)']
    col_widths = [35, 35, 20, 22, 23, 23, 22]  # Sum = 180 (fits within margin of 15 each side of 210 A4 width)
...
def _build_finca_detail_pdf(finca, stats, user_fullname):
    ...
    class PDF(FPDF):
        def header(self):
            self.set_font('Helvetica', 'B', 16)
            self.set_text_color(31, 107, 53)
            self.cell(0, 10, f'REPORTE DETALLADO - {finca.name.upper()}', ln=True, align='C')
            self.set_font('Helvetica', 'I', 10)
            self.set_text_color(100)
