...
def register_api(app, limiter=None):
    ...
    authorizations['Bearer'] = {'type': 'apiKey', 'in': 'header', 'name': 'Authorization', 'description': 'JWT token. Formato: Bearer <token>'}
    authorizations['Cookie'] = {'type': 'apiKey', 'in': 'cookie', 'name': access_cookie_name, 'description': 'JWT token en cookie'}
    ...
    def custom_swagger_ui():
        return flask.render_template(
            ...
            cookie_config={
                ...
                'refresh': app.config.get('JWT_REFRESH_COOKIE_NAME', 'refresh_token_cookie'),
                'csrf_access': app.config.get('JWT_ACCESS_CSRF_COOKIE_NAME', 'csrf_access_token'),
                'csrf_refresh': app.config.get('JWT_REFRESH_CSRF_COOKIE_NAME', 'csrf_refresh_token'),
            },
        )
    ...
    if app.config.get('RATE_LIMIT_ENABLED', True) and limiter:
        try:
            from ..namespaces.users.auth_namespace import set_limiter as set_auth_limiter
            from ..namespaces.users.users_namespace import set_limiter as set_users_limiter
            from ..namespaces.core.activity_namespace import set_limiter as set_activity_limiter

            set_auth_limiter(limiter)
            set_users_limiter(limiter)
            set_activity_limiter(limiter)

            apply_rate_limit_exemptions(app, api, api_bp, limiter, ns_info['exempt_list'])
        except Exception:
    ...
    def serve_chat_file(filename):
        try:
            ...
            file_path = (pathlib.Path(app.root_path).parent / uploads_folder / filename).resolve()
            allowed_base = (pathlib.Path(app.root_path).parent / uploads_folder).resolve()

            if (not file_path.exists() or not str(file_path).startswith(str(allowed_base))) and filename.startswith('animals/'):
            ...
            if not file_path.exists() or not str(file_path).startswith(str(allowed_base)):
