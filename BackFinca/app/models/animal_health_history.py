...
class AnimalHealthHistory(BaseModel):
    ...
    animal = db.relationship('Animals', back_populates='health_history', lazy='selectin')
    finca = db.relationship('Finca', backref='health_history_records', lazy='selectin')
    performer = db.relationship('User', backref='health_events_performed', lazy='selectin')

    _namespace_fields = [
        'id', 'animal_id', 'finca_id', 'event_type', 'event_date',
        'weight', 'height', 'temperature', 'health_status',
        'description', 'performed_by', 'reference_id', 'created_at', 'updated_at'
    ]
    _namespace_relations = {
        'animal': {'fields': ['id', 'record', 'sex', 'status'], 'depth': 1},
    }
    _searchable_fields = ['description', 'health_status']
    _filterable_fields = ['animal_id', 'event_type', 'event_date', 'finca_id', 'health_status']
    ...
    def get_timeline(cls, animal_id, limit=50):
        ...
        return cls.query.filter_by(animal_id=animal_id) \
