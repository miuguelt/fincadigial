...
class ProductionTarget(BaseModel):
    ...
    period = db.Column(db.Enum(TargetPeriod), nullable=False, default=TargetPeriod.Daily)
    ...
    _namespace_fields = [
        'id', 'finca_id', 'animal_id', 'target_liters', 'period',
    ...
    def get_active_for_animal(cls, animal_id: int, finca_id: int, period: str = None):
        ...
        query = cls.query.filter_by(animal_id=animal_id, finca_id=finca_id, is_active=True)
        if period:
            query = query.filter_by(period=period)
