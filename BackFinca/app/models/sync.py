...
class Device(BaseModel):
    ...
    status = db.Column(db.Enum(DeviceStatus), nullable=False, default=DeviceStatus.ACTIVE)
    last_seen_at = db.Column(db.DateTime, nullable=True)
    finca_id = db.Column(db.Integer, db.ForeignKey("finca.id"), nullable=False)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=True)

    _namespace_fields = [
        "id", "device_id", "name", "platform", "status", "last_seen_at",
        "finca_id", "user_id", "created_at", "updated_at"
    ]
...
class SyncOperation(BaseModel):
    ...
    status = db.Column(db.Enum(SyncOperationStatus), nullable=False, default=SyncOperationStatus.PENDING)
    ...
    receipts = db.relationship("SyncOperationReceipt", back_populates="operation_ref", lazy="dynamic")

    _namespace_fields = [
        "id", "operation_id", "entity_type", "entity_id", "operation", "payload",
        "base_version", "logical_clock", "priority", "status", "origin_device_id",
        "author_user_id", "finca_id", "created_at_device", "applied_at", "created_at"
    ]
    _required_fields = ["operation_id", "entity_type", "operation", "origin_device_id", "finca_id"]
    _filterable_fields = ["entity_type", "entity_id", "operation", "status", "finca_id", "origin_device_id"]
...
class SyncSession(BaseModel):
    ...
    __table_args__ = (
        db.Index("ix_sync_sessions_finca_id", "finca_id"),
    )
    ...
    status = db.Column(db.Enum(SyncSessionStatus), nullable=False, default=SyncSessionStatus.OPEN)
    ...
    _namespace_fields = [
        "id", "session_id", "local_device_id", "peer_device_id", "transport", "status",
        "operations_sent", "operations_received", "conflicts_count", "started_at", "completed_at", "finca_id"
    ]
...
class SyncOperationReceipt(BaseModel):
    ...
    __table_args__ = (
        db.UniqueConstraint("operation_id", "device_id", name="uq_sync_receipt_operation_device"),
        db.Index("ix_sync_receipts_finca_id", "finca_id"),
    )

    id = db.Column(db.Integer, primary_key=True)
    operation_id = db.Column(db.String(128), db.ForeignKey("sync_operations.operation_id"), nullable=False)
    device_id = db.Column(db.String(128), nullable=False)
    ...
    _namespace_fields = ["id", "operation_id", "device_id", "received_at", "applied", "finca_id"]


class SyncConflict(BaseModel):
    __tablename__ = "sync_conflicts"
    __table_args__ = (
        db.Index("ix_sync_conflicts_finca_id", "finca_id"),
    )
    ...
    _namespace_fields = [
        "id", "operation_id", "entity_type", "entity_id", "local_payload", "incoming_payload",
        "resolution", "resolved_by", "resolved_at", "finca_id", "created_at"
    ]
...
class AttachmentBlob(BaseModel):
    ...
    _namespace_fields = [
        "id", "attachment_id", "entity_type", "entity_id", "filename", "content_type",
        "sha256", "total_size", "received_size", "storage_path", "is_complete",
        "finca_id", "uploaded_by", "created_at"
    ]
