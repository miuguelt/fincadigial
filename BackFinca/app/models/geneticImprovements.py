...
class GeneticImprovements(BaseModel):
    ...
    finca_id  = db.Column(db.Integer, db.ForeignKey('finca.id'), nullable=True, index=True)

    # Relaciones
    animals = db.relationship('Animals', back_populates='genetic_improvements', lazy='selectin')

    _namespace_fields = [
        'id', 'date', 'details', 'results', 'genetic_event_technique', 'animal_id', 'finca_id', 'created_at', 'updated_at'
    ]
