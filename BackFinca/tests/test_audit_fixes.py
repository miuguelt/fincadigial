...
from app.models import Finca, FarmType, Field, FoodType, Treatment, Animal, Species, Breed
...
class TestAuditFixes(unittest.TestCase):
    ...
    def setUp(self):
        with self.app.app_context():
            ...
            a = Animal.create(
                record='A1', sex='Hembra', birth_date=date(2020,1,1),
                weight=300,
                breeds_id=self.breed_id, finca_id=self.finca_a_id
            )
            db.session.commit()
            self.animal_id = a.id
    ...
    def test_fields_uniqueness_per_finca(self):
        ...
        with self.app.app_context():
            ...
            Field.create(name='Potrero 1', area='5ha', state='Disponible', finca_id=self.finca_a_id)
            db.session.commit()

            # Create same name in Finca B (Should SUCCEED)
            field_b = Field.create(name='Potrero 1', area='10ha', state='Disponible', finca_id=self.finca_b_id)
            self.assertIsNotNone(field_b.id)

            # Create same name in Finca A again (Should FAIL)
            with self.assertRaises(Exception):
                Field.create(name='Potrero 1', area='2ha', state='Disponible', finca_id=self.finca_a_id)
                db.session.commit()
    ...
    def test_food_types_uniqueness_per_finca(self):
        ...
        with self.app.app_context():
            ...
            # Create in Finca A
            FoodType.create(food_type='Pasto Estrella', finca_id=self.finca_a_id, **common_data)
            db.session.commit()

            # Create same name in Finca B (Should SUCCEED)
            ft_b = FoodType.create(food_type='Pasto Estrella', finca_id=self.finca_b_id, **common_data)
