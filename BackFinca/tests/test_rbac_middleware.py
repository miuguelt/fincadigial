...
class TestPropietario:
    ...
    def test_delete_animal(self, client, token_for):
        resp = client.delete(f"{BASE}/animals/999999", headers=token_for(self.ROLE, finca_type="Tradicional"))
        assert_rbac_allowed(resp)
...
class TestCapataz:
    ...
    @pytest.mark.critical
    def test_delete_animal_bloqueado(self, client, token_for):
        resp = client.delete(f"{BASE}/animals/1", headers=token_for(self.ROLE, finca_type="Tradicional"))
        assert_rbac_blocked(resp, "CAPATAZ_NO_DELETE")

    def test_put_animal_permitido(self, client, token_for):
        """/animals está en allowed_prefixes de Capataz → pasa RBAC."""
        resp = client.put(f"{BASE}/animals/999999", json={}, headers=token_for(self.ROLE, finca_type="Tradicional"))
        assert_rbac_allowed(resp)

    def test_put_vaccination_bloqueado(self, client, token_for):
        """/vaccinations no está en allowed_prefixes → RBAC bloquea PUT."""
        resp = client.put(f"{BASE}/vaccinations/1", json={}, headers=token_for(self.ROLE, finca_type="Tradicional"))
    ...
    def test_put_field_permitido(self, client, token_for):
        ...
        resp = client.put(f"{BASE}/fields/999999", json={}, headers=token_for(self.ROLE, finca_type="Tradicional"))
        assert_rbac_allowed(resp)
...
class TestVeterinario:
    ...
    def test_get_vaccination_permitido(self, client, token_for):
        resp = client.get(f"{BASE}/vaccinations", headers=token_for(self.ROLE, finca_type="Tradicional"))
        assert_rbac_allowed(resp)

    def test_get_treatment_permitido(self, client, token_for):
        resp = client.get(f"{BASE}/treatments", headers=token_for(self.ROLE, finca_type="Tradicional"))
        assert_rbac_allowed(resp)

    def test_post_vaccination_permitido(self, client, token_for):
        resp = client.post(f"{BASE}/vaccinations", json={}, headers=token_for(self.ROLE, finca_type="Tradicional"))
        assert_rbac_allowed(resp)
    ...
    def test_delete_vaccination_bloqueado(self, client, token_for):
        resp = client.delete(f"{BASE}/vaccinations/1", headers=token_for(self.ROLE, finca_type="Tradicional"))
        assert_rbac_blocked(resp, "VET_CREATE_ONLY")
...
class TestOperario:
    ...
    def test_put_animal_bloqueado(self, client, token_for):
        resp = client.put(f"{BASE}/animals/1", json={}, headers=token_for(self.ROLE, finca_type="Tradicional"))
    ...
    def test_post_animal_bloqueado(self, client, token_for):
        ...
        resp = client.post(f"{BASE}/animals", json={}, headers=token_for(self.ROLE, finca_type="Tradicional"))
        assert_rbac_blocked(resp, "OPERARIO_POST_LIMITED")

    def test_post_control_permitido(self, client, token_for):
        """/controls está en OPERARIO_POST_PATHS → pasa RBAC."""
        resp = client.post(f"{BASE}/controls", json={}, headers=token_for(self.ROLE, finca_type="Tradicional"))
        assert_rbac_allowed(resp)

    def test_post_animal_fields_permitido(self, client, token_for):
        """/animal-fields está en OPERARIO_POST_PATHS → pasa RBAC."""
        resp = client.post(f"{BASE}/animal-fields", json={}, headers=token_for(self.ROLE, finca_type="Tradicional"))
        assert_rbac_allowed(resp)
