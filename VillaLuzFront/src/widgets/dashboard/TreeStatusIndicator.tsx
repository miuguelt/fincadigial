...
export const TreeStatusIndicator: React.FC<TreeStatusIndicatorProps> = ({
  ...
  return (
    <div className={cn("flex items-center gap-4 text-sm", className)}>
      ...
      {hasMoreLevels && (
        <div className="text-xs text-info bg-info/5 px-2 py-1 rounded">
        ...
        <div className="text-xs text-muted-foreground">
          {type === "ancestors" ? (
            ...
            <span>
              ...
              {dependencyInfo.children_as_father &&
                ` (${dependencyInfo.children_as_father} como padre)`}
              {dependencyInfo.children_as_mother &&
                ` (${dependencyInfo.children_as_mother} como madre)`}
            </span>
          )}
        </div>
      )}
