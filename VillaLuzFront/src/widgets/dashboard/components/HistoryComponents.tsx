...
export const InfoField = ({
  ...
  return (
    <div className={`space-y-1.5 ${fullWidth ? "col-span-full" : ""}`}>
      ...
      {badge ? (
        <span
          className={`inline-flex px-2.5 py-1 rounded-md text-xs font-medium ${badgeVariant === "success" ? "bg-success/10 text-success dark:bg-green-900/30 dark:text-green-100" : badgeVariant === "destructive" ? "bg-destructive/10 text-destructive dark:bg-red-900/30 dark:text-red-100" : badgeVariant === "secondary" ? "bg-muted text-foreground" : "bg-info/10 text-info dark:bg-blue-900/30 dark:text-blue-100"}`}
        >
          {" "}
          {displayValue}{" "}
        </span>
