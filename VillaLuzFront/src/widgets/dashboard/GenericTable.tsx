  ...
  <T,>({
    ...
    const formatValue = (val: any) => {
      ...
      if (typeof val === "object") {
        if ("name" in val && typeof (val as any).name !== "object")
        ...
        <div className="flex items-center gap-1">
          {showViewButton && onView && (
            <Button
              variant="ghost"
              size="sm"
              className="h-8 w-8 p-0 hover:bg-info/5 hover:text-info transition-colors"
              onClick={() => onView(item)}
          ...
          {showEditButton && (
            <Button
              variant="ghost"
              size="sm"
              className="h-8 w-8 p-0 hover:bg-warning/5 hover:text-warning transition-colors"
              onClick={() => onEdit(item)}
              aria-label="Editar"
              title="Editar"
            >
          ...
          {showDeleteButton && onDelete && (
            <Button
              variant="ghost"
              size="sm"
              className="h-8 w-8 p-0 hover:bg-destructive/5 hover:text-destructive transition-colors"
              onClick={() => onDelete(item)}
              aria-label="Eliminar"
              title="Eliminar"
            >
    ...
    const startIndex = needsVirtualization
      ? Math.floor(scrollTop / virtualizationRowHeight)
      : 0;
    const endIndex = needsVirtualization
      ? Math.min(
          startIndex +
            Math.ceil(containerHeight / virtualizationRowHeight) +
    ...
    const slice = displayData.slice(
      Math.max(0, startIndex - virtualizationOverscan),
      endIndex,
    );
    const topPadding =
      Math.max(0, startIndex - virtualizationOverscan) *
    ...
    const bottomPadding =
      Math.max(0, displayData.length - endIndex) * virtualizationRowHeight;
      ...
      <div className="flex flex-col h-full space-y-3">
        ...
        >
          ...
          <Table
            ...
            className={refreshing ? "opacity-95 transition-opacity" : undefined}
          >
            ...
            <TableBody>
              {needsVirtualization ? (
                <>
                  ...
                  {slice.map((item: any) => (
                    <TableRow
                      key={String((item as any)[keyValue])}
                      style={{ height: virtualizationRowHeight }}
                    >
                      {columns.map((column) => {
                        const raw = getValue(item, column);
                        return (
                          <TableCell
                            key={`${(item as any)[keyValue]}-${column}`}
                          >
                            {formatValue(raw)}
                          </TableCell>
                        );
                ...
                displayData.map((item: any) => (
                  <TableRow key={String((item as any)[keyValue])}>
                    {columns.map((column) => {
                      const raw = getValue(item, column);
                      return (
                        <TableCell key={`${(item as any)[keyValue]}-${column}`}>
                          {formatValue(raw)}
                        </TableCell>
                      );
                    })}
