...
export const UserActionsMenu: React.FC<UserActionsMenuProps> = ({ user }) => {
  ...
  const [animalLabelById, setAnimalLabelById] = useState<Map<number, string>>(
    new Map(),
  );
  ...
  const loadListData = useCallback(async () => {
    ...
    try {
      ...
      const animalsList = await animalsService
      ...
      switch (openModal) {
        case "animal_disease": {
          const adResult = await animalDiseasesService
            .getAll({ page: 1, limit: 1000 })
            .catch(() => []);
          const allDiseases = Array.isArray(adResult)
            ? adResult
            : (adResult as any)?.data || (adResult as any)?.items || [];
          data = allDiseases.filter((item: any) => isUserFkRecord(item));
          break;
        }

        case "vaccination": {
          const vResult = await vaccinationsService
            .getAll({ page: 1, limit: 1000 })
            .catch(() => []);
          const allVaccinations = Array.isArray(vResult)
            ? vResult
            : (vResult as any)?.data || (vResult as any)?.items || [];
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="text-muted-foreground">
                {item.animal_id ? getAnimalLabelForId(item.animal_id) : "-"}
              </span>
            </div>
            <div className="flex justify-between">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    animal_id: parseInt(e.target.value),
                  })
                }
  ...
  return (
    <>
      ...
      <GenericModal
        ...
        description={
          modalMode === "list"
            ? `Registros del usuario ${user.fullname || user.id}`
            : undefined
        }
        size="2xl"
        enableBackdropBlur
