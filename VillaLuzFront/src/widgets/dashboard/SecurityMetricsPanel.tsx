...
export const SecurityMetricsPanel = () => {
  ...
  if (loading) {
    return (
      <div className="text-xs text-muted-foreground">Cargando seguridad...</div>
    );
  }
    ...
    <Card className="border-border">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {" "}
            <RefreshCcw className="h-3 w-3 mr-1" /> Refrescar
          </Button>
          <Button size="sm"
            onClick={triggerScan}
            disabled={scanning}
      ...
      <CardContent className="space-y-4">
        {error && (
          <div className="text-xs bg-destructive text-destructive-foreground border border-destructive rounded p-2">
            {error}
          </div>
        )}
        ...
        <div>
          <h3 className="text-xs font-semibold mb-2 flex items-center gap-1">
            <ActivitySquare className="h-3.5 w-3.5" /> Alertas recientes
          </h3>
          <ul className="space-y-2 max-h-56 overflow-auto pr-1">
            {alerts.length ? (
              alerts.slice(0, 8).map((al, index) => (
                <li
                  key={al.id || `alert-${index}-${al.title}`}
                  className="p-2 rounded border bg-muted text-[11px]"
                >
              ...
              <li key="no-alerts" className="text-[11px] text-muted-foreground">
