...
export const UserHistoryModal = ({ user, onClose }: UserHistoryModalProps) => {
  ...
  const userDiseases = safeAnimalDiseases.filter(
    (disease: any) =>
      disease?.instructor?.identification === user.identification,
  );
  ...
  const userTreatments = safeTreatments.filter(
    (treatment: any) =>
      treatment?.animals?.user?.identification === user.identification,
  );

  const userControls = safeControls.filter(
    (control: any) =>
      control?.animals?.user?.identification === user.identification,
  );
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  {stats.recentActivities} últimos 30 días
                </p>
              </CardContent>
            </Card>
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="animal_created">
                    Animales registrados
                  </SelectItem>
                  <SelectItem value="disease_treated">
                    Enfermedades tratadas
                  </SelectItem>
                  <SelectItem value="control_performed">
                    Controles realizados
                  </SelectItem>
                  <SelectItem value="treatment_applied">
                    Tratamientos aplicados
                  </SelectItem>
                  <SelectItem value="genetic_improvement">
                    Mejoras genéticas
                  </SelectItem>
                  <SelectItem value="field_management">
                    Gestión de Potreros
                  </SelectItem>
                </SelectContent>
              </Select>
                ...
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    <Badge variant="secondary">
                      {filteredTimelineActivities.length} actividades
                    </Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {filteredTimelineActivities.length > 0 ? (
                    <div className="space-y-4">
                      {filteredTimelineActivities.map((activity) => (
                        <div
                          key={activity.id}
                          className="flex items-start space-x-4 pb-4 border-b border-border/50 last:border-b-0"
                        >
                          <div className="flex-shrink-0">
                            <div
                              className={`w-10 h-10 rounded-full flex items-center justify-center ${
                                activity.type === "disease_treated"
                                  ? "bg-destructive/10 text-destructive"
                                  : activity.type === "treatment_applied"
                                    ? "bg-success/10 text-success"
                                    : activity.type === "control_performed"
                                      ? "bg-purple-100 text-purple-600"
                                      : activity.type === "animal_created"
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-muted-foreground">
                        No se encontraron actividades que coincidan con los
                        filtros.
                      </p>
                    </div>
                  )}
                ...
                <CardContent>
                  {userAnimals.length > 0 ? (
                        ...
                        <TableBody>
                          {userAnimals.map((animal) => (
                            <TableRow key={animal.idAnimal}>
                              ...
                              <TableCell>
                                {animal.breed?.name || "No especificada"}
                              </TableCell>
                              <TableCell>
                                {animal.breed?.species?.name ||
                                  "No especificada"}
                              </TableCell>
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-muted-foreground">
                        No hay animales asignados a este usuario.
                      </p>
                    </div>
                  )}
                </CardContent>
                        ...
                        {userDiseases.slice(0, 5).map((disease: any) => (
                          ...
                          >
                            <div className="flex justify-between items-start">
                              ...
                              <Badge
                                variant={
                                  disease.status ? "default" : "secondary"
                                }
                              >
                          ...
                          >
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  Animal: {control.animals?.record}
                                </p>
                                <p className="text-xs text-muted-foreground">
                                  {formatDate(control.checkup_date)}
                                </p>
                              </div>
                              <Badge
                                variant={
                                  control.healt_status === "Exelente"
                                    ? "default"
                                    : "secondary"
                                }
                              >
                ...
                <CardContent>
                  {userFields.length > 0 ? (
                        ...
                        <TableBody>
                          {userFields.map((field: any) => (
                            <TableRow key={field.id}>
                              ...
                              <TableCell>
                                {formatDate(field.treatment_date)}
                              </TableCell>
                              <TableCell>
                                {field.end_date
                              ...
                              <TableCell>
                                {field.duration || "En curso"}
                              </TableCell>
                              <TableCell>
                                <Badge
                                  variant={
                                    field.end_date ? "secondary" : "default"
                                  }
                                >
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-muted-foreground">
                        No hay registros de gestión de Potreros.
                      </p>
                    </div>
                  )}
                </CardContent>
