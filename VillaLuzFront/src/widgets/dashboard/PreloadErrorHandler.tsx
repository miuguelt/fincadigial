...
export const PreloadErrorHandler: React.FC<PreloadErrorHandlerProps> = ({
  ...
  const errorsByType = filteredErrors.reduce(
    (acc, error) => {
      if (!acc[error.type]) {
        acc[error.type] = [];
      }
    ...
    switch (type) {
      case PreloadErrorType.DASHBOARD_CRITICAL:
        return {
          ...
          description:
            "No se pudieron cargar los datos esenciales del dashboard. La aplicación puede no funcionar correctamente.",
          icon: AlertTriangle,
          color: "destructive",
          critical: true,
      ...
      case PreloadErrorType.ANIMAL_MODULE:
        return {
          ...
          description:
            "No se pudieron cargar los datos del módulo Animal. Algunas funciones pueden no estar disponibles.",
          icon: Database,
          color: "default",
          critical: false,
      ...
      case PreloadErrorType.USER_MODULE:
        return {
          ...
          description:
            "No se pudieron cargar los datos del módulo Usuario. Algunas funciones pueden no estar disponibles.",
          icon: Database,
          color: "default",
          critical: false,
          actions: ["retry-module", "details"],
      ...
      case PreloadErrorType.NETWORK_ERROR:
        return {
          title: "Error de red",
          description:
            "Problemas de conectividad al intentar cargar los datos. Verifica tu conexión a internet.",
          icon: WifiOff,
          color: "default",
          critical: false,
          actions: ["retry", "details"],
      ...
      case PreloadErrorType.AUTHENTICATION_ERROR:
        return {
          ...
          description:
            "Problemas con la sesión de usuario. Es posible que necesites iniciar sesión nuevamente.",
          icon: Shield,
          color: "destructive",
          critical: true,
          actions: ["retry", "details"],
  ...
  const renderErrorDetails = (error: PreloadError) => {
    ...
    return (
      <Collapsible
        open={isExpanded}
        onOpenChange={() => toggleErrorExpansion(errorId)}
      >
        <CollapsibleContent className="mt-3">
          <div className="bg-muted/50 dark:bg-card p-3 rounded-md text-sm">
            <div className="space-y-2">
              ...
              {error.context && (
                <div>
                  ...
                  <pre className="mt-1 text-xs bg-muted dark:bg-foreground p-2 rounded overflow-auto max-h-32">
                ...
                <div className="mt-2">
                  ...
                  {typeErrors.length > 1 && (
                    <div className="mt-2 text-xs text-muted-foreground">
                      {typeErrors.length - 1} error(es) adicional(es) de este
                      tipo
                    </div>
                  )}
                </div>
    ...
    <div className="w-full max-w-md mx-auto p-6 bg-card rounded-lg shadow-lg">
      <div className="text-center mb-6">
        ...
        <p className="text-muted-foreground">
          No se pudo cargar información esencial para el funcionamiento de la
          aplicación.
        </p>
      </div>
...
export const NonCriticalErrorBanner: React.FC<{
  ...
  return (
    <div className="w-full bg-warning/5 border-l-4 border-yellow-400 p-4 mb-4">
      <div className="flex items-center">
        <AlertTriangle className="h-5 w-5 text-yellow-400 mr-2" />
        <div className="flex-1">
          <p className="text-sm text-warning">
            Algunos datos no pudieron cargarse. La aplicación seguirá
            funcionando, pero algunas características pueden no estar
            disponibles.
          </p>
        </div>
        <div className="flex space-x-2 ml-4">
          {onRetry && (
            <button
              onClick={onRetry}
              className="text-warning hover:text-yellow-900 text-sm underline"
            >
              Reintentar
            </button>
          )}
