...
export const BreedActionsMenu: React.FC<BreedActionsMenuProps> = ({
  breed,
}) => {
  ...
  const loadAnimals = async () => {
    ...
    try {
      ...
      const animalsData = Array.isArray(result) ? result : [];

      // Filtrar animales por breed_id
      const filteredAnimals = animalsData.filter(
        (animal: any) => animal.breed_id === breed.id,
      );
      ...
      return (
        <div className="py-10 text-center">
          <p className="text-muted-foreground">
            No hay animales registrados para esta raza
          </p>
        </div>
      );
    }
