...
const FarmStatistics: React.FC = () => {
  ...
  useEffect(() => {
    if (animals && animals.length > 0) {
      const alive = animals.filter(
        (a) =>
          (a as any).status === "Sano" ||
          (a as any).status === "Enfermo" ||
          (a as any).status === "En tratamiento" ||
          (a as any).status === "En observación" ||
          (a as any).status === "Cuarentena" ||
          a.status === "Vivo",
      ).length;
  ...
  useEffect(() => {
    if (controls && treatments && vaccinations) {
      ...
      const healthCounts = controls.reduce(
        (acc, control) => {
      ...
      const healthDistribution = Object.entries(healthCounts).map(
        ([status, count]) => ({
          name: status,
          value: count,
          color:
                  ...
                  <PieChart>
                    <Pie
                      ...
                      label={({
                        name,
                        percent,
                      }: import("@/shared/types/rechartsLabel.types").RechartsLabelProps) =>
              ...
              value={
                users?.filter((u: any) =>
                  typeof u.status === "boolean"
                    ? u.status === true
                    : String(u.status) === "1" ||
                      String(u.status).toLowerCase() === "true" ||
                      String(u.status).toLowerCase() === "activo",
                ).length || 0
              }
              ...
              value={
                users?.filter((u: any) =>
                  typeof u.status === "boolean"
                    ? u.status === false
                    : String(u.status) === "0" ||
                      String(u.status).toLowerCase() === "false" ||
