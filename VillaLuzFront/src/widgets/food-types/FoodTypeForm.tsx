...
const FoodTypeForm: React.FC<FoodTypeFormProps> = ({ initialData, onSubmit, loading }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md mx-auto p-4 bg-card rounded shadow">
      <label>
        Nombre (food_type):
        <input
          type="text"
      ...
      <label>
        ...
        <input
          ...
          className={`input input-bordered w-full ${errors.sowing_date ? 'border-destructive' : ''}`}
        />
        {errors.sowing_date && <p className="text-destructive text-xs mt-1">{errors.sowing_date}</p>}
      </label>
      <label>
        ...
        <input
          ...
          max={getTodayColombia()}
          className={`input input-bordered w-full ${errors.harvest_date ? 'border-destructive' : ''}`}
        />
        {errors.harvest_date && <p className="text-destructive text-xs mt-1">{errors.harvest_date}</p>}
      </label>
      <label>
        Área (m²):
        <input
