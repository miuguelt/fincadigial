      ...
      if (responseData && responseData.data && responseData.data.images) {
            ...
            try {
              if (typeof window !== 'undefined') {
                if (cleanPath.startsWith('/public/images')) {
                  return addVersionParam(`/api/v1${cleanPath}`, updatedAt);
                }
                if (cleanPath.startsWith('/static/uploads/')) {
                  return addVersionParam(
                    `/api/v1/public/images/${cleanPath.slice('/static/uploads/'.length)}`,
                    updatedAt
                  );
                }
        ...
        responseData.data.images = responseData.data.images.map((image: AnimalImage) => {
          if (image.url && (image.url.startsWith('http://') || image.url.startsWith('https://'))) {
            try {
              const absoluteUrl = new URL(image.url);
              if (
                absoluteUrl.pathname.startsWith('/public/images') ||
                absoluteUrl.pathname.startsWith('/static/uploads/')
              ) {
                return {
                  ...image,
                  url: buildStaticAssetUrl(absoluteUrl.pathname, image.updated_at),
                };
              }
            } catch {
              // Si URL no parsea, seguir con el valor original.
            }
