...
export const AnimalTagCard: React.FC<AnimalTagCardProps> = ({
  ...
  <div className="animal-tag bg-[#0f172a]/60 border border-white/10 rounded-[2.5rem] p-6 flex flex-col items-center gap-6 shadow-[0_20px_40px_rgba(0,0,0,0.4)] hover:border-emerald-500/30 transition-all group relative overflow-hidden">
    <div className="tag-header w-full flex justify-between items-start z-10">
      <Badge className="badge text-[9px] font-black border-emerald-500/20 text-emerald-400 bg-emerald-500/10 px-2.5 py-0.5 rounded-full uppercase tracking-widest">
        VILLA LUZ
      </Badge>
    ...
    <div className="text-center w-full z-10">
      ...
      <p className="sub-text text-[10px] font-black text-indigo-300/40 uppercase tracking-widest mt-2 opacity-80">
        {breedLabel || "EJEMPLAR BOVINO"}
      </p>
    </div>
    <div className="details-container flex gap-3 w-full z-10 mt-1">
      <div className="details-item flex-1 h-10 rounded-xl bg-slate-950 border border-white/5 flex items-center justify-center text-[10px] font-semibold text-sm text-indigo-200/40 italic shadow-inner">
        {gender || "S/G"}
      </div>
      <div className="details-item flex-1 h-10 rounded-xl bg-slate-950 border border-white/5 flex items-center justify-center text-[10px] font-semibold text-sm text-indigo-200/40 italic shadow-inner">
        {birthDate || "S/F"}
