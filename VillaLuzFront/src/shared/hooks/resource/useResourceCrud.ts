    ...
    try {
      const result = await safeExecuteMutation(async () => {
        ...
        if (createdId && createdId !== 'undefined') {
          ...
          const now = Date.now();
          for (const [id, timestamp] of Array.from((recentlyCreatedTimestamps.current as Map<number | string, number>).entries())) {
      ...
      const result = await safeExecuteMutation(async () => {
        try {
          ...
          if (ok) {
            ...
            const now = Date.now();
            for (const [delId, timestamp] of Array.from((recentlyDeletedTimestamps.current as Map<number | string, number>).entries())) {
