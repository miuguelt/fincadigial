...
export class PWAApiClient<T = any> extends BaseService<T> {
  ...
  async getWithETag(params?: Record<string, any>): Promise<PaginatedResponse<T>> {
    ...
    try {
      ...
      return {
        ...
        total_pages: response.data?.total_pages || response.data?.totalPages || response.data?.pages || 1,
        has_next_page: response.data?.has_next_page ?? response.data?.hasNextPage ?? false,
        has_previous_page: response.data?.has_previous_page ?? response.data?.hasPreviousPage ?? false,
      };
    } catch (error) {
   ...
   */
  async fullSync(): Promise<{ data: T[]; total_items: number }> {
    if (__DEV__) {
