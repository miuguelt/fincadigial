...
export const CorralPanel: React.FC<{ onClose?: () => void }> = ({ onClose }) => {
  ...
  const formContent = (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {/* Paso 1: Animal */}
      <div className="bg-blue-50/50 p-5 rounded-lg border border-blue-100 shadow-sm">
        <Label className="text-xl font-bold text-blue-900 mb-3 block flex items-center gap-2">
        ...
        >
          <SelectTrigger className="h-16 text-xl bg-white shadow-sm border-2 border-blue-200 rounded-xl w-full">
            <SelectValue placeholder={loadingAnimals ? "Cargando lista..." : "Toque aquí para elegir animal"} />
          </SelectTrigger>
          <SelectContent className="max-h-[300px]">
            {animals.map((a) => (
              <SelectItem key={a.value} value={a.value.toString()} className="text-xl p-4 border-b border-gray-100 last:border-0 cursor-pointer">
                {a.label}
              </SelectItem>
            ))}
          </SelectContent>
      ...
      {animalId && (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-emerald-50/50 p-5 rounded-lg border border-emerald-100 shadow-sm">
            <Label className="text-xl font-bold text-emerald-900 mb-4 block flex items-center gap-2">
              <span>2.</span> 📊 Datos de hoy
            </Label>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {/* Leche - Solo si es hembra */}
              <div className={`space-y-2 p-5 rounded-xl bg-white border-2 transition-all ${isFemale ? 'border-emerald-200 shadow-sm' : 'opacity-40 grayscale pointer-events-none'}`}>
                ...
                <div className="relative">
                  <Input 
                    ...
                    step="0.1" 
                    min="0"
                    className="h-16 text-3xl text-center font-bold rounded-xl pr-12 border-gray-300 focus:border-emerald-500 focus:ring-emerald-500" 
                    placeholder="0.0" 
                    value={milkLiters}
                    onChange={(e) => setMilkLiters(e.target.value)}
                  />
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 font-medium text-lg">L</span>
                </div>
                {!isFemale && <p className="text-sm text-gray-500 text-center mt-2">No aplica (Macho)</p>}
              </div>
              
              {/* Peso */}
              <div className="space-y-2 p-5 rounded-xl bg-white border-2 border-emerald-200 shadow-sm">
                <Label className="text-lg font-semibold text-gray-700 flex items-center gap-2 mb-2">
                  ⚖️ ¿Cuánto pesa hoy?
                </Label>
                <div className="relative">
                  <Input 
                    ...
                    step="1" 
                    min="0"
                    className="h-16 text-3xl text-center font-bold rounded-xl pr-14 border-gray-300 focus:border-emerald-500 focus:ring-emerald-500" 
                    placeholder="0" 
                    value={weight}
                    onChange={(e) => setWeight(e.target.value)}
                  />
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 font-medium text-lg">Kg</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Paso 3: Salud */}
          <div className="bg-amber-50/50 p-5 rounded-lg border border-amber-100 shadow-sm">
            <Label className="text-xl font-bold text-amber-900 mb-4 block flex items-center gap-2">
              <span>3.</span> 🩺 ¿Cómo lo ve de salud?
            </Label>
            
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { id: 'Excelente', label: 'Muy Bien', icon: '🌟', color: 'bg-green-100 border-green-400 text-green-800 ring-green-400' },
                { id: 'Sano', label: 'Normal', icon: '✅', color: 'bg-emerald-100 border-emerald-400 text-emerald-800 ring-emerald-400' },
                { id: 'Regular', label: 'Decaído', icon: '⚠️', color: 'bg-yellow-100 border-yellow-400 text-yellow-800 ring-yellow-400' },
                { id: 'Malo', label: 'Enfermo', icon: '🚨', color: 'bg-red-100 border-red-400 text-red-800 ring-red-400' }
              ].map((status) => {
                ...
                return (
                  <div 
                    ...
                    className={`cursor-pointer rounded-xl border-2 p-4 flex flex-col items-center justify-center gap-2 transition-all duration-200 active:scale-95 ${isSelected ? `${status.color} shadow-md scale-105 font-bold ring-2 ring-offset-2` : 'border-gray-200 bg-white hover:bg-gray-50 text-gray-600'}`}
                  >
            ...
            {showTreatment && (
              <div className="mt-5 p-5 bg-red-50 rounded-xl border-2 border-red-200 animate-in fade-in slide-in-from-top-2">
                <Label className="text-lg font-bold text-red-800 mb-3 block flex items-center gap-2">
                  💊 Tratamiento (Opcional)
                </Label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <Label className="text-gray-700">¿Qué remedio le aplicó?</Label>
                    <Input 
                      placeholder="Ej: Vitamina, Antibiótico..." 
                      value={treatmentDesc}
                      onChange={(e) => setTreatmentDesc(e.target.value)}
                      className="h-14 text-lg bg-white border-red-100 focus:border-red-400"
                    />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-gray-700">¿Qué cantidad (Dosis)?</Label>
                    <Input 
          ...
          {isFemale && (
            <div className="bg-purple-50/50 p-5 rounded-lg border border-purple-100 shadow-sm">
              <div className="flex justify-between items-center mb-4">
                <Label className="text-xl font-bold text-purple-900 flex items-center gap-2 cursor-pointer" onClick={() => setShowRepro(!showRepro)}>
                ...
                <Button 
                  ...
                  className={`rounded-full px-4 ${showRepro ? 'bg-purple-600 hover:bg-purple-700' : 'text-purple-700 border-purple-300 bg-white'}`}
                >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 animate-in fade-in slide-in-from-top-2">
                  {[
                    ...
                    <div 
                      ...
                      className={`cursor-pointer rounded-xl border-2 p-3 flex flex-col items-center justify-center gap-1 transition-all ${reproEvent === evt.id ? 'border-purple-500 bg-purple-100 font-bold text-purple-900 shadow-sm' : 'border-purple-200 bg-white hover:bg-purple-50 text-gray-600'}`}
                    >
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-4 pt-4 border-t border-gray-100">
            {onClose && (
              <Button 
                ...
                variant="outline" 
                onClick={onClose} 
                className="w-full sm:w-1/3 h-16 text-xl rounded-lg border-2 border-gray-200 text-gray-600 hover:bg-gray-100"
              >
                Cancelar
              </Button>
            )}
  ...
  if (onClose) {
    return (
      <div className="p-2 sm:p-6 bg-white rounded-xl">
        <h2 className="text-3xl font-extrabold text-gray-800 mb-6 text-center tracking-tight">Registro de Corral</h2>
        {formContent}
      </div>
    );
  }
  ...
  return (
    <Card className="max-w-3xl mx-auto shadow-xl border-0 rounded-xl overflow-hidden bg-white">
