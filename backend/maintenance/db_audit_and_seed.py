#!/usr/bin/env python3
"""
Script de Auditoría de Base de Datos VillaLuz
=============================================

Este script audita el estado de todas las tablas. El modo predeterminado
nunca crea movimientos: el poblamiento sintético queda reservado a una base
de pruebas y requiere autorización explícita.

Uso:
    python db_audit_and_seed.py [--audit-only]
    ALLOW_SIMULATION_SCRIPTS=true python db_audit_and_seed.py --seed-only
...
def seed_milk_production(app, db):
    ...
    animals = (
        Animals.query.filter(
            Animals.sex == "Hembra", Animals.status == AnimalStatus.Vivo
        )
        .limit(5)
        .all()
    )
...
def seed_reproductive_events(app, db):
    ...
    males = (
        Animals.query.filter(
            Animals.sex == "Macho", Animals.status == AnimalStatus.Vivo
        )
        .limit(3)
        .all()
    )
    ...
    for user in users:
        for finca in fincas:
            ...
            existing = UserFinca.query.filter_by(
                user_id=user.id, finca_id=finca.id
            ).first()
            if not existing:
                try:
                    UserFinca.assign(
                    ...
                    print(
                        f"  ⚠ Error asignando usuario {user.id} a finca {finca.id}: {e}"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Auditoría y Poblamiento de BD VillaLuz"
    )
    parser.add_argument(
        "--audit-only", action="store_true", help="Solo auditar, no poblar"
    )
    parser.add_argument(
        "--seed-only", action="store_true", help="Solo poblar, no auditar inicial"
    )
    ...
    if args.table:
        ...
        report_path = PROJECT_ROOT / "test-results" / "backend" / "db_audit_report.json"
        report_path.parent.mkdir(parents=True, exist_ok=True)
        with open(report_path, "w", encoding="utf-8") as f:
            json.dump(status, f, indent=2, ensure_ascii=False)
        print(f"\n💾 Reporte guardado en: {report_path}")
    else:
        # La auditoría nunca debe convertir tablas vacías en movimientos
        # inventados. El poblamiento queda reservado a --seed-only con
        # autorización explícita para una base de pruebas.
        print("=" * 70)
        print("🔍 AUDITORÍA (MODO SEGURO, SIN POBLAMIENTO)")
        print("=" * 70)
        status = get_db_status()
        print_audit_report(status)
        print(
            "\nℹ️ No se poblaron tablas: los movimientos deben registrarse desde la operación real."
        )
