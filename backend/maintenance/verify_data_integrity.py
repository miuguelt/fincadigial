...
def verify_milk_production():
    ...
    with app.app_context():
        ...
        if dead_with_milk > 0:
            issues.append(
                f"⚠ {dead_with_milk} animales muertos/vendidos con producción"
            )
...
def verify_inventory_movements():
    ...
    with app.app_context():
        ...
        negative = InventoryMovement.query.filter(
...
def verify_animal_images():
    ...
    with app.app_context():
        ...
        if primary_counts:
            issues.append(
                f"⚠ {len(primary_counts)} animales con múltiples imágenes primarias"
            )
...
def main():
    ...
    if args.table:
        ...
        if args.table in verifiers:
            ...
            print(
                f"Tabla {args.table} no encontrada. Disponibles: {', '.join(verifiers.keys())}"
            )
    else:
