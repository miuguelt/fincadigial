...
with app.app_context():
    ...
    for f in fincas:
        ...
        incomes = (
            ...
            .filter_by(finca_id=f.id, transaction_type="Income")
            .scalar()
            or 0
        )
        expenses = (
            db.session.query(db.func.sum(Transaction.amount))
            .filter_by(finca_id=f.id, transaction_type="Expense")
            .scalar()
            or 0
        )
        ...
        total_l = (
            ...
            .filter_by(finca_id=f.id)
            .scalar()
            or 0
        )
        total_e = (
            db.session.query(db.func.count(MilkProduction.id))
            .filter_by(finca_id=f.id)
            .scalar()
            or 0
        )
