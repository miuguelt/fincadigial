...
with app.app_context():
    params = [
        ...
        (
            "param.coef_aprovechamiento",
            "0.7",
            "Coeficiente de aprovechamiento de pastura",
        ),
    ...
    texts = [
        (
            "recommendation.weather.heat",
            "Proporcione sombra adicional y agua fresca. Evite actividades físicas intensas con el ganado.",
        ),
        (
            "recommendation.weather.cold",
            "Proporcione refugio y alimento adicional. Monitoree especialmente a crías y animales enfermos.",
        ),
        (
            "recommendation.weather.storm",
            "Refugie al ganado en zonas seguras. Evite estar bajo árboles aislados.",
        ),
        (
            "recommendation.weather.rain",
            "Vigile zonas de drenaje. Evite mover ganado por terrenos resbaladizos.",
        ),
        (
            "recommendation.weather.wind",
            "Asegure estructuras ligeras. Monitoree cercas y techos.",
        ),
        (
            "recommendation.weather.hail",
            "Refugie al ganado inmediatamente. Proteja cultivos sensibles si es posible.",
        ),
    ]
    ...
    for key, value in texts:
        ...
        if not entry:
            SystemContent.create(
                key=key, content=value, category="recommendation", content_type="text"
            )
            print(f"  ✅ {key}")
        else:
            print(f"  ⏭ {key} ya existe")
