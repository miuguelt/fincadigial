...
with app.app_context():
    ...
    if not Infrastructure.query.filter_by(finca_id=finca.id).first():
        ...
        infra = [
            Infrastructure(
                name="Tanque de Leche Principal",
                type=InfrastructureType.TANQUE,
                next_maintenance=date.today() + timedelta(days=5),
                finca_id=finca.id,
            ),
            Infrastructure(
                name="Cerca Eléctrica Potrero 1",
                type=InfrastructureType.CERCA,
            ...
            Infrastructure(
                ...
                type=InfrastructureType.MAQUINARIA,
    ...
    if not AnimalGroup.query.filter_by(finca_id=finca.id).first():
        ...
        group = AnimalGroup(
            name="Novillas de Ceba",
            description="Lote principal de engorde",
            finca_id=finca.id,
        )
    ...
    if field and not PastureAforo.query.filter_by(field_id=field.id).first():
        ...
        aforo = PastureAforo(
            field_id=field.id, entry_height=25, exit_height=4, finca_id=finca.id
        )
