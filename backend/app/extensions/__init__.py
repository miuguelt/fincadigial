...
def init_extensions(app):
    ...
    if redis_url:
        try:
            ...
            logger.info(
                "Redis native client inicializado y registrado en app.extensions"
            )
    ...
    try:
        ...
        if cache_config["CACHE_TYPE"] == "redis":
            ...
            cache_redis_url = app.config.get("CACHE_REDIS_URL") or app.config.get(
                "REDIS_URL"
            )
            if redis_client is not None and cache_redis_url == app.config.get(
                "REDIS_URL"
            ):
