...
def get_client_info():
    ...
    return {
        "ip": flask.request.environ.get(
            "HTTP_X_FORWARDED_FOR", flask.request.environ.get("REMOTE_ADDR")
        ),
...
def log_authentication_attempt(user_identifier, success, additional_info=None):
    ...
    safe_identifier = (
        user_identifier[:2] + "*" * (len(str(user_identifier)) - 2)
        if user_identifier
        else "unknown"
    )
...
def log_jwt_token_event(event_type, user_id=None, token_info=None):
    ...
    if token_info:
        safe_token_info = {
            ...
            "new_access_token_created": token_info.get(
                "new_access_token_created", False
            ),
