...
class PresenceTracker:
    ...
    def register_connection(user_id: int) -> str:
        ...
        if redis_client:
            try:
                ...
                logger.debug(f"[PresenceTracker] Registrada conexión Redis para user={user_id} conn={conn_id}")
        ...
        with _local_lock:
            ...
            logger.debug(f"[PresenceTracker] Registrada conexión local para user={user_id} conn={conn_id}")
    ...
    def remove_connection(user_id: int, conn_id: str):
        ...
        if redis_client:
            try:
                ...
                logger.debug(f"[PresenceTracker] Removida conexión Redis para user={user_id} conn={conn_id}")
            except Exception as e:
        ...
        with _local_lock:
            if user_id in _local_connections:
                ...
                logger.debug(f"[PresenceTracker] Removida conexión local para user={user_id} conn={conn_id}")
    ...
    def is_user_online(user_id: int) -> bool:
        ...
        with _local_lock:
            return user_id in _local_connections and len(_local_connections[user_id]) > 0
