...
def seed_admin_user(app, logger: logging.Logger | None = None) -> None:
    ...
    - phone: "3000000000"
    - password: VILLALUZ_SEED_ADMIN_PASSWORD (hashed)
        ...
        with app.app_context():
            try:
                ...
                if "user" not in table_names or "finca" not in table_names:
                    _logger.info(
                        "Skipping seed admin: tabla user o finca no existe aún"
                    )
                    return
                ...
                seed_password = os.getenv("VILLALUZ_SEED_ADMIN_PASSWORD") or os.getenv(
                    "ADMIN_PASSWORD"
                )
                if not seed_password:
                    if app.config.get("CONFIG_NAME") == "testing":
                        ...
                        _logger.warning(
                            "Se omite el usuario administrador semilla: "
                            "define VILLALUZ_SEED_ADMIN_PASSWORD o ADMIN_PASSWORD."
                        )
                        return
                ...
                db.session.commit()
                _logger.info(
                    "Usuario administrador semilla creado (identification=99999999)"
                )
            except Exception as e:
                db.session.rollback()
                _logger.warning(
                    "Error verificando/creando usuario semilla: %s", e, exc_info=True
                )
    except Exception as e:
        _logger.warning(
            "No se pudo crear usuario administrador semilla: %s", e, exc_info=True
        )
...
def warmup_initial_caches(app, logger: logging.Logger | None = None) -> None:
    ...
    include_relations_enabled = bool(
        app.config.get("CACHE_WARMUP_INCLUDE_RELATIONS", False)
    )
    per_page_default = int(
        app.config.get("CACHE_WARMUP_LIMIT", 5)
    )  # reducir lote para warmup
        ...
        try:
            ...
            def _warm_model_list(model_cls, args: dict[str, Any]) -> None:
                try:
                    ...
                    per_page = int(
                        args.get("limit", args.get("per_page", per_page_default))
                    )
                    include_rel = bool(
                        str(args.get("include_relations", "false")).lower() == "true"
                    )
                    ...
                    payload = {
                        ...
                        "meta": {
                            "pagination": {
                                ...
                                "total_items": data_struct.get(
                                    "total_items", data_struct.get("total", 0)
                                ),
                                "total_pages": data_struct.get(
                                    "total_pages", data_struct.get("pages", 1)
                                ),
                                "has_next_page": data_struct.get(
                                    "has_next_page", data_struct.get("has_next", False)
                                ),
                                "has_previous_page": data_struct.get(
                                    "has_previous_page",
                                    data_struct.get("has_prev", False),
                                ),
                            }
                        },
                    }
                    ...
                    _logger.info(
                        "Cache warmup listo para %s key=%s",
                        model_cls.__name__,
                        cache_key,
                    )
                except Exception as e:
                    _logger.warning(
                        "Fallo warmup para %s: %s",
            ...
            with app.app_context():
                ...
                if (
                    ...
                    try:
                        ...
                        data_struct = Animals.get_paginated_response(
                            query, include_relations=False
                        )
                        payload = {
                            "success": True,
                            ...
                            "meta": {
                                "pagination": {
                                    ...
                                    "has_next_page": data_struct.get(
                                        "has_next_page",
                                        data_struct.get("has_next", False),
                                    ),
                                    "has_previous_page": data_struct.get(
                                        "has_previous_page",
                                        data_struct.get("has_prev", False),
                        ...
                        _logger.info(
                            "Cache warmup mínimo con relaciones listo para %s key=%s",
                            Animals.__name__,
                            cache_key,
                        )
                    except Exception as e:
                        _logger.warning(
                            "Fallo warmup con relaciones: %s", e, exc_info=True
                        )
        except Exception as e:
    ...
    try:
        if async_enabled:
            ...
            th.start()
            _logger.info(
                "Warmup de caché disparado en segundo plano (async=%s)", async_enabled
            )
        else:
            _do_warmup()
