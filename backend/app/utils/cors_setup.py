...
def init_cors(app, logger: logging.Logger = None):
    ...
    try:
        app.config["CORS_ORIGINS"] = _sanitize_origins(
            app.config.get("CORS_ORIGINS", []) or []
        )
    except Exception:
        pass
    ...
    def _handle_cors_preflight():
        if flask.request.method == "OPTIONS":
            ...
            allowed_methods = flask.current_app.config.get(
                "CORS_METHODS",
            ...
            res.headers.add(
                ...
                str(flask.current_app.config.get("CORS_MAX_AGE", 86400)),
            )
    ...
    try:
        ...
        logger.info(
            f"CORS habilitado. Orígenes permitidos (solo .env): {allowed_origins}"
        )
        logger.info(
