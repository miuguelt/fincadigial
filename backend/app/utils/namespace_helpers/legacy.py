...
def _field_definitions_for_model(
    model_class, exclude: list[str]
) -> dict[str, fields.Raw]:
    defs = {}
    ...
    for column in model_class.__table__.columns:
        ...
        kwargs = {
            ...
            "required": (column.name in model_required)
            or (
                not column.nullable
...
def _build_models(ns: Namespace, model_class: type):
    ...
    input_fields = _field_definitions_for_model(
        model_class, exclude=["id", "created_at", "updated_at"]
    )
    ...
    if "password" in model_class.__table__.columns:
        ...
        input_fields["password"] = fields.String(
            description="Password (raw, will be hashed)", required=is_required
        )
    ...
    for fname, col in model_class.__table__.columns.items():
        ...
        is_enum = (
            hasattr(col.type, "enums")
            or str(col.type).startswith("ENUM")
            or hasattr(col.type, "enum_class")
            or (
                hasattr(model_class, "_enum_fields")
                and fname in model_class._enum_fields
            )
        )
        ...
        if is_enum:
            ...
            if fname in input_fields:
                ...
                input_fields[fname] = fields.String(
                    description=description,
                    required=getattr(input_fields[fname], "required", False),
                )
            if fname in response_fields:
                response_fields[fname] = fields.String(
                    description=response_fields[fname].description, required=False
                )
    ...
    list_model = ns.model(
        f"{model_class.__name__}List",
        {
            ...
            "data": fields.List(
                fields.Nested(response_model), description="Lista paginada de elementos"
            ),
            "meta": fields.Nested(
                ns.model("ListMeta", {"pagination": fields.Nested(pagination_model)}),
    ...
    *,
    ...
    def _validation_error_response(errors):
        ...
        status_code = (
            ...
            if validation_error_status
            else (400 if is_public_create else 422)
        )
    ...
    def _handle_integrity_error(
        ie: IntegrityError, action_name: str, record_id: int | None = None
    ):
        db.session.rollback()
        ...
        key_name = None
        m = re.search(
            r"Duplicate entry '(.+?)' for key '(.+?)'", msg, flags=re.IGNORECASE
        )
        if m:
            value = m.group(1)
            key_name = m.group(2)
            ...
            if m2:
                ...
                m3 = re.search(
                    r"llave duplicada viola restricción de unicidad «(.+?)»",
                    msg,
                    flags=re.IGNORECASE,
                )
                if m3:
    ...
    ns.doc(
        params={
            ...
            "sort_order": "asc o desc (alias: sort_dir, order)",
    ...
    class ModelListResource(Resource):
        @ns.doc(
            "list_" + name,
            description="Listar registros con filtros y paginación (caché ligera / export / selección de campos)",
        )
        ...
        def get(self):  # List
            try:
                ...
                if (
                    allow_cache
                    and cached_payload
                    and (prefer_cache or not cache_is_stale)
                ):
                    ...
                    try:
                        if cached_payload.get("data"):
                            upd_vals = [
                                item.get("updated_at")
                                for item in cached_payload["data"]
                    ...
                    cache_headers = _generate_cache_headers(
                        model_class, max_updated_cached
                    )
                    cache_headers["X-Cache-Status"] = (
                        "STALE" if cache_is_stale else "HIT"
                    )
                    if cache_is_stale:
                        cache_headers["Warning"] = (
                            '110 - "Contenido en caché expirado usado por prefer_cache/offline"'
                        )

                    if _check_conditional_request(
                        etag, cache_headers.get("Last-Modified")
                    ):
                ...
                max_page_size = int(
                    flask.current_app.config.get("API_MAX_PAGE_SIZE", 500)
                )
                ...
                # Aceptar los alias usados por las distintas generaciones del
                # cliente: sort -> sort_by y sort_dir/order -> sort_order.
                ...
                try:
                    if not flask.request.args.get(
                        "include_relations"
                    ) and search_type in ("dates", "all"):
                        include_rel = True
                except Exception:
                    pass
                ...
                if filters:
                    logger.debug(
                        f"Filtros aplicados en {model_class.__name__}: {filters}"
                    )
                ...
                if since_param:
                    try:
                        ...
                        filters["_since"] = (
                            since_date  # Usar '_since' especial para diferenciarlo
                        )
                        logger.debug(f"Delta sync enabled: since={since_date}")
                    except (ValueError, TypeError) as e:
                        logger.warning(
                            f"Parámetro 'since' inválido: {since_param} - {e}"
                        )
                ...
                per_page_val = data_struct.get(
                    "limit", data_struct.get("per_page", len(items))
                )
                total_val = data_struct.get(
                    "total_items", data_struct.get("total", len(items))
                )
                max_updated = None
                try:
                    if items:
                ...
                try:
                    ...
                    st = (
                        flask.request.args.get("search_type", default="auto", type=str)
                        or "auto"
                    ).lower()
                    ...
                    if fields_param and (
                        st in ("dates", "all") or (st == "auto" and is_date_like)
                    ):
                        fields_param = None
                except Exception:
                ...
                if export_fmt and export_fmt.lower() == "csv":
                    ...
                    resp.headers["Content-Disposition"] = (
                        f"attachment; filename={model_class.__name__.lower()}_export.csv"
                    )
                    if max_updated:
                        resp.headers["ETag"] = (
                            f"W/{_scoped_etag(model_class, data_struct['total'], max_updated)}"
                        )
                    return resp
                ...
                logger.debug(
                    f"List response payload for {model_class.__name__}: {response_payload}"
                )
                ...
                if allow_cache and _check_conditional_request(
                    etag, pwa_headers.get("Last-Modified")
                ):
                ...
                logger.error(
                    f"Error listando {model_class.__name__}: {e}", exc_info=True
                )
                from app.utils.response_handler import APIResponse
                ...
                if cached_payload and stale_if_error > 0:
                    try:
                        cached_meta = cached_payload.get("meta", {}).get(
                            "pagination", {}
                        )
                        cached_total = cached_meta.get("total_items", 0)
                        max_updated_cached = None
                        ...
                        etag = _scoped_etag(
                            model_class, cached_total, max_updated_cached
                        )
                        cache_headers = _generate_cache_headers(
                            model_class, max_updated_cached
                        )
                        ...
                        cache_headers["Warning"] = (
                            '111 - "Respuesta en caché servida por error de backend"'
                        )
                        ...
                        logger.debug(
                            "No se pudo usar fallback de caché tras error de backend",
                            exc_info=True,
                        )

                resp_body, status = APIResponse.error(
                    "Error interno del servidor",
                    details={
                        "error": str(e),
                        "context": f"list {model_class.__name__}",
                    },
                    status_code=500,
                )
        ...
        @ns.doc(
            "head_list_" + name,
            description="HEAD listado: devuelve solo headers (ETag, status) sin cuerpo",
        )
        ...
        def post(self):  # Create
            try:
                ...
                logger.info(
                    f"POST payload keys for {model_class.__name__}: {list(payload.keys())}"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"{model_class.__name__} created successfully with ID: {instance_id}"
                    )
                except Exception as e:
                    logger.error(
                        f"Error serializing {model_class.__name__} after create: {e}",
                        exc_info=True,
                    )
                    # Fallback: re-query desde BD
                    logger.debug(
                        f"Re-querying {model_class.__name__} ID {instance.id} from DB..."
                    )
                    ...
                    if instance:
                        ...
                        instance_id = instance.id
                        logger.info(
                            f"{model_class.__name__} re-queried and serialized successfully with ID: {instance_id}"
                        )
                    else:
                        raise Exception(
                            f"Failed to serialize and re-query {model_class.__name__}"
                        )
                ...
                try:
                    ...
                    logger.debug(
                        "No se pudo registrar activity_log en create", exc_info=True
                    )
                try:
                ...
                try:
                    if model_class.__name__ == "User":
                        ...
                        user_name = (
                            getattr(user, "fullname", "Un usuario") or "Un usuario"
                        )
                    ...
                    logger.warning(
                        f"No se pudo enviar notificación de nuevo usuario: {e}"
                    )
                ...
                response = APIResponse.created(
                    result, message=f"{model_class.__name__} creado exitosamente"
                )
                if isinstance(response, tuple) and len(response) >= 2:
                    ...
                    logger.debug(
                        f"Response prepared for {model_class.__name__} ID {instance_id}"
                    )
                    return resp
                ...
                db.session.rollback()
                logger.warning(
                    f"Validation error creating {model_class.__name__}: {ve.message}"
                )
                errors = _format_validation_errors(ve)
                conflict_detected = False
                try:
                    conflict_detected = getattr(ve, "code", "").lower() in (
                        "conflict",
                        "unique",
                    )
                    if not conflict_detected:
                        ...
                        conflict_detected = any(
                            isinstance(err, str) and "ya existe" in err.lower()
                ...
                if conflict_detected:
                    return APIResponse.conflict(
                        "Violación de unicidad", details={"validation_errors": errors}
                    )
                ...
                db.session.rollback()
                logger.error(
                    f"Error creando {model_class.__name__}: {e}", exc_info=True
                )
                return APIResponse.error(
                    "Error interno del servidor",
                    details={
                        "error": str(e),
                        "context": f"create {model_class.__name__}",
                    },
                    status_code=500,
                )
    ...
    class ModelDetailResource(Resource):
        @ns.doc(
            "get_" + name,
            description="Obtener detalle por ID (soporta include_relations)",
        )
        @require_permission(entity, "read")
        @_maybe_rate_limit
        def get(self, record_id: int):  # Retrieve
            try:
                ...
                if (
                    ...
                    if cache_is_stale:
                        cache_headers["Warning"] = (
                            '110 - "Detalle en caché expirado usado por prefer_cache/offline"'
                        )
                    if _check_conditional_request(
                        etag, cache_headers.get("Last-Modified")
                    ):
                ...
                include_relations = (
                    flask.request.args.get("include_relations", "false").lower()
                    == "true"
                )
                instance = model_class.get_by_id(
                    record_id, include_relations=include_relations
                )
                if not instance:
                ...
                fields_param = flask.request.args.get("fields")
                data_obj = instance.to_namespace_dict(
                ...
                body, status = APIResponse.success(
                    data=data_obj, message=f"{name.capitalize()} obtenido exitosamente"
                )
                ...
                try:
                    etag = _scoped_etag(
                        model_class,
                        record_id,
                        data_obj.get("updated_at")
                        if isinstance(data_obj, dict)
                        else None,
                    )
                    pwa_headers = _generate_cache_headers(
                        model_class,
                        data_obj.get("updated_at")
                        if isinstance(data_obj, dict)
                        else None,
                    ...
                    if cache_enabled:
                        _detail_cache_set(
                            model_class.__name__, record_id, body, model_class
                        )
                except Exception:
                    logger.debug(
                        "No se pudo cachear/etiquetar respuesta de detalle",
                        exc_info=True,
                    )
                ...
                logger.error(
                    f"Error obteniendo {model_class.__name__} ID {record_id}: {e}",
                    exc_info=True,
                )
                if cached_payload and stale_if_error > 0:
                    try:
                        ...
                        cache_headers["X-Cache-Status"] = "STALE-FALLBACK"
                        cache_headers["Warning"] = (
                            '111 - "Detalle en caché servido por error de backend"'
                        )
                        cache_headers["X-Offline-Fallback"] = "true"
                        resp = flask_make_response(jsonify(cached_payload), 200)
                        resp.headers["ETag"] = etag
                        ...
                        logger.debug(
                            "No se pudo usar fallback de caché para detalle tras error",
                            exc_info=True,
                        )
                body, status = APIResponse.error(
                    "Error interno del servidor",
                    details={"error": str(e), "context": f"get {model_class.__name__}"},
                    status_code=500,
                )
                resp = flask_make_response(jsonify(body), status)
                return resp
        ...
        @ns.doc(
            "update_" + name, description="Actualizar registro (reemplazo completo)"
        )
        ...
        def put(self, record_id: int):  # Update
            try:
                ...
                if expected_version is not None and expected_version != getattr(
                ...
                try:
                    logger.debug(
                        f"Serializing updated {model_class.__name__} ID {record_id}..."
                    )
                    result = instance.to_namespace_dict()
                    logger.info(
                        f"{model_class.__name__} ID {record_id} updated successfully"
                    )
                except Exception as e:
                    logger.error(
                        f"Error serializing {model_class.__name__} after update: {e}",
                        exc_info=True,
                    )
                    # Fallback: re-query desde BD
                    ...
                    if instance:
                        result = instance.to_namespace_dict()
                        logger.info(
                            f"{model_class.__name__} ID {record_id} re-queried and serialized"
                        )
                    else:
                        raise Exception(
                            f"Failed to serialize and re-query {model_class.__name__} ID {record_id}"
                        )
                ...
                try:
                    ...
                    logger.debug(
                        "No se pudo registrar activity_log en update", exc_info=True
                    )
                try:
                    from flask import current_app
                ...
                response = APIResponse.success(
                    data=result, message=f"{name.capitalize()} actualizado exitosamente"
                )
                if isinstance(response, tuple) and len(response) >= 2:
                    resp_body, status_code = response[0], response[1]
                    resp = make_response(jsonify(resp_body), status_code)
                    ...
                    resp.headers["Expires"] = "0"
                    resp.headers["ETag"] = (
                        f'"{instance.id}-{instance.updated_at}"'
                ...
                return APIResponse.error(
                    ...
                    details={
                        "error": str(e),
                        "context": f"update {model_class.__name__}",
                    },
                    status_code=500,
                )
            ...
            def patch(self, record_id: int):  # Partial update
                try:
                    ...
                    if not isinstance(payload, dict):
                        return APIResponse.validation_error(
                            {"payload": "Se requiere un objeto JSON."}
                        )
                    ...
                    try:
                        logger.debug(
                            f"Serializing patched {model_class.__name__} ID {record_id}..."
                        )
                        result = instance.to_namespace_dict()
                        logger.info(
                            f"{model_class.__name__} ID {record_id} patched successfully"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error serializing {model_class.__name__} after patch: {e}",
                            exc_info=True,
                        )
                        # Fallback: re-query desde BD
                        instance = model_class.query.get(record_id)
                    ...
                    try:
                        ...
                        description = (
                            f"Campos actualizados: {updated_fields}"
                            if updated_fields
                            else "Actualizacion parcial desde API"
                        )
                        log_activity_event(
                            action="update",
                            entity=model_class.__name__.lower(),
                        ...
                        logger.debug(
                            "No se pudo registrar activity_log en patch", exc_info=True
                        )
                    try:
                        from flask import current_app
                    ...
                    response = APIResponse.success(
                        data=result,
                        message=f"{name.capitalize()} actualizado parcialmente",
                    )
                    if isinstance(response, tuple) and len(response) >= 2:
                        resp_body, status_code = response[0], response[1]
                        resp = make_response(jsonify(resp_body), status_code)
                    ...
                    return APIResponse.error(
                        ...
                        details={
                            "error": str(e),
                            "context": f"patch {model_class.__name__}",
                        },
                        status_code=500,
                    )
        ...
        def delete(self, record_id: int):  # Delete
            try:
                ...
                db.session.rollback()
                logger.error(
                    f"Error eliminando {model_class.__name__} id={record_id}: {e}",
                    exc_info=True,
                )
                body, status = APIResponse.error(
                    "Error interno del servidor",
                    details={
                        "error": str(e),
                        "context": f"delete {model_class.__name__}",
                    },
                    status_code=500,
                )
                return flask_make_response(jsonify(body), status)
    ...
    class ModelDependenciesResource(Resource):
        @ns.doc(
            "get_dependencies_" + name,
            description="Verificar dependencias de un registro antes de eliminar",
        )
        @_maybe_rate_limit
        def get(self, record_id: int):
            ...
            try:
                ...
                logger.error(
                    f"Error verificando dependencias de {model_class.__name__} ID {record_id}: {e}",
                    exc_info=True,
                )
                body, status = APIResponse.error(
    ...
    class ModelBatchDependenciesResource(Resource):
        @ns.doc(
            "post_batch_dependencies_" + name,
            description="Verificación batch de dependencias para múltiples registros",
        )
        @_maybe_rate_limit
        def post(self):
            ...
            try:
                ...
                record_ids = (
                    data.get("ids") or data.get(f"{name}_ids") or data.get("record_ids")
                )

                if not record_ids:
                    return APIResponse.error(
                        message="Se requiere lista de ids", status_code=400
                    )

                if not isinstance(record_ids, list):
                    return APIResponse.error(
                        message="ids debe ser una lista", status_code=400
                    )
                ...
                logger.error(
                    f"Error en verificación batch de {model_class.__name__}: {e}",
                    exc_info=True,
                )
                return APIResponse.error(
                    message=f"Error en verificación batch: {str(e)}", status_code=500
                )
    ...
    class ModelMetadataResource(Resource):
        @ns.doc(
            "metadata_" + name,
            description="Obtener metadatos del recurso (total, last_modified) sin body completo - optimizado para PWA",
        )
        @_maybe_rate_limit
        def get(self):
            ...
            try:
                ...
                if result and result.last_modified:
                    max_updated = (
                        ...
                        if hasattr(result.last_modified, "isoformat")
                ...
                logger.error(
                    f"Error obteniendo metadata de {model_class.__name__}: {e}",
                    exc_info=True,
                )
                body, status = APIResponse.error(
                    "Error interno del servidor",
    ...
    if enable_bulk:
        ...
        class ModelBulkResource(Resource):
            @ns.doc(
                "bulk_create_" + name,
                description="Crear múltiples registros (soporta shared data)",
            )
            @require_permission(entity, "create")
            @_maybe_rate_limit
            def post(self):
                try:
                    ...
                    if not isinstance(payload, list) or not payload:
                        return APIResponse.validation_error(
                            {"items": "Se requiere lista de objetos no vacía"}
                        )

                    logger.debug(
                        f"Bulk creating {len(payload)} {model_class.__name__} instances..."
                    )
                    ...
                    db.session.rollback()
                    logger.error(
                        f"Error bulk create {model_class.__name__}: {e}", exc_info=True
                    )
                    return APIResponse.error(
                        "Error interno del servidor",
                        details={"error": str(e)},
            ...
            @ns.doc(
                "bulk_update_" + name,
                description="Actualizar múltiples registros (requiere ID en cada item)",
            )
            @require_permission(entity, "update")
            @_maybe_rate_limit
            def put(self):
                try:
                    ...
                    if not isinstance(payload, list) or not payload:
                        return APIResponse.validation_error(
                            {"items": "Se requiere lista de objetos con ID"}
                        )

                    logger.debug(
                        f"Bulk updating {len(payload)} {model_class.__name__} instances..."
                    )
                    ...
                    db.session.rollback()
                    logger.error(
                        f"Error bulk update {model_class.__name__}: {e}", exc_info=True
                    )
                    return APIResponse.error(
                        "Error interno del servidor",
                        details={"error": str(e)},
            ...
            @ns.doc(
                "bulk_delete_" + name, description="Eliminar múltiples registros por ID"
            )
            @require_permission(entity, "delete")
            @_maybe_rate_limit
            def delete(self):
                try:
                    ...
                    db.session.rollback()
                    logger.error(
                        f"Error bulk delete {model_class.__name__}: {e}", exc_info=True
                    )
                    return APIResponse.error(
                        "Error interno del servidor",
                        details={"error": str(e)},
    ...
    if enable_stats and hasattr(model_class, "get_stats"):
        ...
        class ModelStatsResource(Resource):
            @ns.doc(
                "stats_" + name, description="Obtener estadísticas básicas del modelo"
            )
            @_maybe_rate_limit
            def get(self):
                try:
                    ...
                    body, status = APIResponse.success(
                        stats, message="Estadísticas obtenidas"
                    )
                    resp = flask_make_response(jsonify(body), status)
                    return resp
                except Exception as e:
                    logger.error(
                        f"Error obteniendo stats {model_class.__name__}: {e}",
                        exc_info=True,
                    )
                    body, status = APIResponse.error(
                        "Error interno del servidor",
