...
def register_docs_routes(api_bp):
            ...
            for _finder, name, _ispkg in pkgutil.iter_modules(pkg.__path__):
                try:
                    ...
                    logging.getLogger(__name__).debug(
                        "No se pudo importar modelo: %s", name, exc_info=True
                    )
        except Exception:
            logging.getLogger(__name__).debug(
                "app.models no es un paquete importable", exc_info=True
            )
                    ...
                    for p in models_dir.iterdir():
                        if (
                            ...
                            try:
                                ...
                                logging.getLogger(__name__).debug(
                                    "No se pudo importar modelo desde archivo: %s",
                                    p.name,
                                    exc_info=True,
                                )
            except Exception:
        ...
        def serialize_model(cls):
            ...
            try:
                ...
                for col in cls.__table__.columns:
                    ...
                    if not col.primary_key and col.name not in (
                        "created_at",
                        "updated_at",
                    ):
                        if col.name in data["required"]:
                            ...
                            create_example[col.name] = (
                                "ENUM_VALUE"
                                if col.name in getattr(cls, "_enum_fields", {})
                                else _example_value(col_type)
                            )
                ...
                for rel_name, rel_cfg in getattr(
                    cls, "_namespace_relations", {}
                ).items():
                ...
                data["examples"] = {
                    "create": {
                        k: _sanitize_value(v) for k, v in create_example.items()
                    },
                    "update": {
                        k: _sanitize_value(v) for k, v in update_example.items()
                    },
                }
            except Exception:
                return data
    ...
    def docs_examples():
        ...
        for m in schema_resp["data"]["models"]:
            ...
            field_types = {
                f["name"]: f.get("type", "").lower() for f in (m.get("fields") or [])
            }
    ...
    def docs_frontend_guide():
        try:
            ...
            return flask.send_file(
                str(md_path), mimetype="text/markdown; charset=utf-8"
            )
        except Exception as e:
            return flask.jsonify(
                {
                    "success": False,
                    "message": "Error sirviendo la guía",
                    "details": str(e),
                }
            ), 500
