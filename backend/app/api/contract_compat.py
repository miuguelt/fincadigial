...
def serialize_animal(animal):
    ...
    return {
        ...
        "created_at": animal.created_at.isoformat() + "Z"
        if animal.created_at
        else datetime.now(UTC).replace(tzinfo=None).isoformat() + "Z",
    }
...
def serialize_control(ctrl):
    tipo = "preventivo"
    if ctrl.health_status and ctrl.health_status.value in [
        "preventivo",
        "curativo",
        "rutina",
    ]:
    ...
    return {
        ...
        "tipo": tipo,
        "diagnostico": ctrl.health_status.value
        if hasattr(ctrl.health_status, "value")
        else str(ctrl.health_status),
        "observaciones": ctrl.description or "",
        "created_at": ctrl.created_at.isoformat() + "Z"
        if ctrl.created_at
        else datetime.now(UTC).replace(tzinfo=None).isoformat() + "Z",
    }
...
def get_animales():
    ...
    return jsonify(
        {"data": data, "total": pagination.total, "page": page, "per_page": per_page}
    )
...
def get_animal(id):
    ...
    return jsonify({"data": serialize_animal(animal)})
...
def post_animal():
    ...
    try:
        ...
        return jsonify(
            {"errors": {"fecha_nacimiento": "Formato inválido (YYYY-MM-DD)"}}
        ), 422
    ...
    return jsonify({"data": serialize_animal(animal)}), 201
...
def put_animal(id):
    ...
    if "fecha_nacimiento" in data:
        try:
            animal.birth_date = datetime.strptime(
...
def get_potrero(id):
    ...
    return jsonify({"data": serialize_potrero(field)})
...
def post_potrero():
    ...
    db.session.commit()
    return jsonify({"data": serialize_potrero(field)}), 201
...
def get_control(id):
    ...
    return jsonify({"data": serialize_control(ctrl)})
...
def post_control():
    ...
    db.session.commit()
    return jsonify({"data": serialize_control(ctrl)}), 201
...
def get_reporte_ica():
    ...
    for a in animals:
        ...
        animales_list.append(
            {
                "arete": arete,
                "especie": a.breed.species.name.lower()
                if (a.breed and a.breed.species)
                else "bovino",
    ...
    return jsonify(
        {
            ...
            "fecha_generacion": datetime.now(UTC).replace(tzinfo=None).isoformat()
            + "Z",
