...
class ActiveLactationCycle(Resource):
    @lactation_ns.doc(
        "get_active_lactation",
        description="Obtener ciclo de lactancia activo de un animal",
    )
    @jwt_required()
    def get(self, animal_id):
        ...
        if not cycle:
            return APIResponse.success(
                data=None, message="No hay ciclo de lactancia activo"
            )
...
class LactationHistory(Resource):
    @lactation_ns.doc(
        "get_lactation_history",
        description="Obtener historial de lactancias de un animal",
    )
    @jwt_required()
    def get(self, animal_id):
        finca_id = get_current_finca_id()
...
class UpdateLactationProduction(Resource):
    @lactation_ns.doc(
        "update_lactation_production",
        description="Actualizar métricas de producción del ciclo",
    )
    @jwt_required()
    def post(self, cycle_id):
        ...
        if cycle.status != LactationStatus.Active:
            return APIResponse.error(
                "Solo se pueden actualizar ciclos activos", code=400
            )

        total_liters = (
            db.session.query(db.func.sum(MilkProduction.liters))
            .filter(
        ...
        peak_record = (
            db.session.query(MilkProduction.date, MilkProduction.liters)
            .filter(
                MilkProduction.animal_id == cycle.animal_id,
                MilkProduction.finca_id == finca_id,
                MilkProduction.date >= cycle.calving_date,
            )
            .order_by(MilkProduction.liters.desc())
            .first()
        )
...
class DryOffCycle(Resource):
    @lactation_ns.doc(
        "dry_off_cycle", description="Registrar secado de un ciclo de lactancia"
    )
    @jwt_required()
    def post(self, cycle_id):
        finca_id = get_current_finca_id()
        ...
        if dry_off_date < cycle.calving_date:
            return APIResponse.error(
                "Fecha de secado no puede ser anterior al parto", code=400
            )
