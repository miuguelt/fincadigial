...
movement_post_model = animal_movements_ns.model(
    "AnimalMovementPost",
    {
        "animal_id": fields.Integer(required=True, description="ID del animal a mover"),
        "tipo_movimiento": fields.String(
            required=True,
            choices=["Traslado_Interno", "Venta_Traslado_Externo", "Venta_En_Predio"],
            description="Tipo de movilización",
        ),
        "fecha_movimiento": fields.String(
            required=True, description="Fecha del movimiento (YYYY-MM-DD)"
        ),
        "finca_destino_id": fields.Integer(
            description="ID de la finca destino para traslados internos"
        ),
        "finca_destino_externa": fields.String(
            description="Nombre del predio externo de destino"
        ),
        "rpp_destino_externo": fields.String(
            description="Código RPP ICA del predio externo de destino (12 dígitos)"
        ),
        ...
        "guia_movilizacion": fields.String(
            description="Guía Sanitaria de Movilización Interna (GSMI) ICA"
        ),
        "ruv_vacunacion": fields.String(
            description="Registro Único de Vacunación (RUV) contra Aftosa"
        ),
        "placa_vehiculo": fields.String(description="Placa del vehículo transportador"),
        "nombre_conductor": fields.String(
            description="Nombre del transportador/conductor"
        ),
        "cedula_conductor": fields.String(
            description="Cédula del transportador/conductor"
        ),
        "precinto_seguridad": fields.String(
            description="Precinto de seguridad oficial colocado por el ICA"
        ),
...
class AnimalMovementsResource(Resource):
    ...
    @animal_movements_ns.doc(
        "list_movements",
        description="Listar el historial de movimientos de la finca activa",
        security=["Bearer"],
    )
    @jwt_required()
    def get(self):
        try:
            ...
            query = AnimalMovement.query.filter(
                ...
                AnimalMovement.fecha_movimiento.desc(), AnimalMovement.created_at.desc()
            )
    ...
    @animal_movements_ns.doc(
        "register_movement",
        description="Registrar una venta y/o traslado de animal bajo normativas del ICA",
        security=["Bearer"],
    )
        ...
        try:
            ...
            if not animal_id:
                return APIResponse.validation_error(
                    {"animal_id": "Se requiere el ID del animal"}
                )
            if not tipo_mov_str or tipo_mov_str not in [
                "Traslado_Interno",
                "Venta_Traslado_Externo",
                "Venta_En_Predio",
            ]:
                return APIResponse.validation_error(
                    {"tipo_movimiento": "Tipo de movimiento inválido"}
                )
            if not fecha_mov_str:
                return APIResponse.validation_error(
                    {"fecha_movimiento": "Se requiere la fecha del movimiento"}
                )
            ...
            try:
                ...
                return APIResponse.validation_error(
                    {"fecha_movimiento": "Formato de fecha inválido. Usar YYYY-MM-DD"}
                )
            ...
            if animal.finca_id != finca_id:
                return APIResponse.error(
                    "El animal no pertenece a la finca activa", status_code=403
                )

            # 3. Check animal status (must be Vivo)
            if animal.status != AnimalStatus.Vivo:
                return APIResponse.validation_error(
                    f"El animal no se puede mover/vender porque su estado actual es {animal.status.value}"
                )
            ...
            if tipo_movimiento == MovementType.Traslado_Interno:
                if not finca_destino_id:
                    return APIResponse.validation_error(
                        {
                            "finca_destino_id": "Se requiere la finca de destino para un traslado interno"
                        }
                    )
                if finca_destino_id == finca_id:
                    return APIResponse.validation_error(
                        {
                            "finca_destino_id": "La finca de destino no puede ser la misma finca de origen"
                        }
                    )
                ...
                if not finca_destino or not finca_destino.is_active:
                    return APIResponse.error(
                        "La finca de destino no existe o está inactiva", status_code=400
                    )

                if not guia_movilizacion:
                    return APIResponse.validation_error(
                        {
                            "guia_movilizacion": "La Guía Sanitaria de Movilización (GSMI) del ICA es obligatoria para trasladar animales"
                        }
                    )
                ...
                if not finca_destino_externa:
                    return APIResponse.validation_error(
                        {
                            "finca_destino_externa": "Se requiere el nombre del predio pecuario de destino"
                        }
                    )
                if not guia_movilizacion:
                    return APIResponse.validation_error(
                        {
                            "guia_movilizacion": "La Guía Sanitaria de Movilización (GSMI) del ICA es obligatoria para transporte por carretera"
                        }
                    )
                if rpp_destino_externo:
                    ...
                    if not rpp_clean.isdigit() or len(rpp_clean) != 12:
                        return APIResponse.validation_error(
                            {
                                "rpp_destino_externo": "El código RPP del predio destino ante el ICA debe contener exactamente 12 dígitos numéricos"
                            }
                        )
            ...
            try:
                ...
                active_fields = AnimalFields.query.filter_by(
                ...
                if arete_sinigan:
                    ...
                    reg = SiniganRegistrations.query.filter_by(
                        animal_id=animal.id
                    ).first()
                    if reg:
                        ...
                        SiniganRegistrations.create(
                            ...
                            predio_origen=animal.finca.name
                            if animal.finca
                            else "Origen",
                            guia_movilizacion=guia_movilizacion,
                            notes=data.get("notes")
                            or "Registro automático por movimiento",
                        )
                ...
                movement = AnimalMovement(
                    ...
                    finca_destino_externa=finca_destino_externa
                    if tipo_movimiento == MovementType.Venta_Traslado_Externo
                    else None,
                    rpp_destino_externo=rpp_destino_externo
                    if tipo_movimiento == MovementType.Venta_Traslado_Externo
                    else None,
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to write movement to ActivityLog: {log_err}"
                    )
...
class AnimalMovementsByAnimalResource(Resource):
    ...
    @animal_movements_ns.doc(
        "list_animal_movements",
        description="Listar el historial de movimientos de un animal específico",
        security=["Bearer"],
    )
    @jwt_required()
    def get(self, animal_id):
        try:
            ...
            query = (
                AnimalMovement.query.filter_by(animal_id=animal_id)
                .filter(
                    (AnimalMovement.finca_origen_id == finca_id)
                    | (AnimalMovement.finca_destino_id == finca_id)
                )
                .order_by(
                    AnimalMovement.fecha_movimiento.desc(),
                    AnimalMovement.created_at.desc(),
                )
            )
            ...
            return APIResponse.error(
                f"Error al obtener movimientos del animal: {str(e)}"
            )
