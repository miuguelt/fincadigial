...
class FincaTargetProgress(Resource):
    @target_ns.doc(
        "get_finca_progress", description="Obtener progreso de metas de la finca"
    )
    @jwt_required()
    def get(self):
        ...
        if not targets:
            return APIResponse.success(
                data=None, message="No hay metas activas para este período"
            )
        ...
        progress_pct = (
            (actual / target.target_liters * 100) if target.target_liters > 0 else 0
        )
...
class TargetProgress(Resource):
    @target_ns.doc(
        "get_target_progress", description="Obtener progreso de una meta específica"
    )
    @jwt_required()
    def get(self, target_id):
        ...
        target = ProductionTarget.query.filter_by(
            id=target_id, finca_id=finca_id
        ).first()
        if not target:
