            ...
            if animal:
                if event.event_type == EventType.Diagnostico:
                    ...
                    animal.update(
                        is_pregnant=False,
                        is_lactating=True,
                        last_calving_date=event.event_date,
                    )
            db.session.commit()
...
class ReproductionSummary(Resource):
    ...
    def get(self):
        ...
        total_females = (
            ...
            .filter_by(sex=Sex.Hembra)
            .count()
        )
        total_events = apply_tenant_filter(
            ReproductiveEvent.query, ReproductiveEvent
        ).count()

        insem_count = (
            apply_tenant_filter(ReproductiveEvent.query, ReproductiveEvent)
            .filter_by(event_type=EventType.Inseminacion)
            .count()
        )
        heat_count = (
            apply_tenant_filter(ReproductiveEvent.query, ReproductiveEvent)
            .filter_by(event_type=EventType.Celo)
            .count()
        )
        diag_count = (
            apply_tenant_filter(ReproductiveEvent.query, ReproductiveEvent)
            .filter_by(event_type=EventType.Diagnostico)
            .count()
        )
        positive_count = (
            apply_tenant_filter(ReproductiveEvent.query, ReproductiveEvent)
        ...
        parto_count = (
            apply_tenant_filter(ReproductiveEvent.query, ReproductiveEvent)
            .filter_by(event_type=EventType.Parto)
            .count()
        )
        ...
        conception_rate = (
            round(positive_count / insem_count * 100, 1) if insem_count else None
        )

        total_alive = (
            apply_tenant_filter(
                db.session.query(func.sum(ReproductiveEvent.alive_count)),
                ReproductiveEvent,
            )
            .filter(ReproductiveEvent.event_type == EventType.Parto)
            .scalar()
            or 0
        )
        total_dead = (
            apply_tenant_filter(
                db.session.query(func.sum(ReproductiveEvent.dead_count)),
                ReproductiveEvent,
            )
            .filter(ReproductiveEvent.event_type == EventType.Parto)
            .scalar()
    ...
    def get(self):
        ...
        all_events_in_period = (
            ...
            .filter(
                ...
                ReproductiveEvent.event_type.in_(
                    [EventType.Inseminacion, EventType.Diagnostico, EventType.Parto]
                ),
            )
        ...
        inseminations_period = [
            e for e in all_events_in_period if e.event_type == EventType.Inseminacion
        ]
        ...
        for insem in inseminations_period:
            ...
            next_event = None
            for ev in animal_events:
                if ev.event_date > insem.event_date or (
                    ev.event_date == insem.event_date and ev.id > insem.id
                ):
                    next_event = ev
                    break
            ...
            if next_event:
                if (
                    next_event.event_type == EventType.Diagnostico
                    and next_event.diagnosis_result == DiagnosisResult.Positivo
                ) or next_event.event_type == EventType.Parto:
                    successful_inseminations.append(insem)

        conception_rate = (
            round(len(successful_inseminations) / len(inseminations_period) * 100, 1)
            if inseminations_period
            else 0
        )
        ...
        natural_insem = [
            e
            for e in inseminations_period
            if e.technique == InseminationTechnique.Natural
        ]
        artificial_insem = [
            e
            for e in inseminations_period
            if e.technique == InseminationTechnique.Artificial
        ]

        natural_success = [
            e
            for e in successful_inseminations
            if e.technique == InseminationTechnique.Natural
        ]
        artificial_success = [
            e
        ...
        natural_rate = (
            round(len(natural_success) / len(natural_insem) * 100, 1)
            if natural_insem
            else 0
        )
        artificial_rate = (
            round(len(artificial_success) / len(artificial_insem) * 100, 1)
            if artificial_insem
            else 0
        )
        ...
        for animal_id in set(p.animal_id for p in partos):
            animal_partos = sorted(
                [p for p in partos if p.animal_id == animal_id],
                key=lambda x: x.event_date,
            )
            for i in range(1, len(animal_partos)):
                interval = (
                    animal_partos[i].event_date - animal_partos[i - 1].event_date
                ).days
        ...
        mortality_rate = (
            round((sum(p.dead_count or 0 for p in partos) / total_born) * 100, 1)
            if total_born > 0
            else 0
        )
        ...
        females_db = (
            apply_tenant_filter(Animals.query, Animals)
            .filter(Animals.id.in_(animal_ids))
            .all()
            if animal_ids
            else []
        )
...
class SirePerformance(Resource):
    ...
    def get(self):
        ...
        offspring_list = (
            apply_tenant_filter(Offspring.query, Offspring)
            .filter(Offspring.birth_event_id.in_(parto_ids))
            .all()
            if parto_ids
            else []
        )
        ...
        for parto in partos_period:
            ...
            for ev in reversed(animal_events):
                if (
                    ev.event_type == EventType.Inseminacion
                    and ev.sire_id is not None
                    and (
                        ev.event_date < parto.event_date
                        or (ev.event_date == parto.event_date and ev.id < parto.id)
                    )
                ):
                    matching_insem = ev
                    break
        ...
        sires_db = (
            Animals.query.filter(Animals.id.in_(sire_ids)).all() if sire_ids else []
        )
        ...
        for sire_id, stats in sire_stats.items():
            ...
            conception_rate = (
                round(stats["positive"] / stats["inseminations"] * 100, 1)
                if stats["inseminations"] > 0
                else 0
            )
            avg_birth_weight = (
                round(sum(stats["birth_weights"]) / len(stats["birth_weights"]), 1)
                if stats["birth_weights"]
                else 0
            )
        ...
        try:
            ...
            if not animal_ids or not isinstance(animal_ids, list):
                return APIResponse.validation_error(
                    {"animal_ids": "Se requiere lista de IDs"}
                )
            if not event_type:
                return APIResponse.validation_error(
                    {"event_type": "Se requiere tipo de evento"}
                )
            if not event_date:
                return APIResponse.validation_error(
                    {"event_date": "Se requiere fecha de evento"}
                )
            ...
            db.session.rollback()
            return APIResponse.error(
                message=f"Error en registro reproductivo masivo: {str(e)}"
            )
