...
class FarmWallet(Resource):
    ...
    def get(self):
        ...
        income = float(
            db.session.query(func.coalesce(func.sum(FarmExpenses.amount), 0))
            .filter_by(finca_id=finca_id, is_income=True)
            .scalar()
            or 0
        )
        expenses = float(
            db.session.query(func.coalesce(func.sum(FarmExpenses.amount), 0))
            .filter_by(finca_id=finca_id, is_income=False)
            .scalar()
            or 0
        )
        ...
        query = FarmExpenses.query.filter_by(finca_id=finca_id).order_by(
            FarmExpenses.expense_date.desc()
        )
