...
image_model = finca_images_ns.model('FincaImage', {
...
class FincaImageUpload(Resource):
    ...
    def post(self):
        try:
            ...
            if not finca_id:
                return APIResponse.error(message='El campo finca_id es requerido', status_code=400)

            finca = Finca.query.get(finca_id)
            if not finca:
                return APIResponse.error(message=f'Finca con ID {finca_id} no encontrada', status_code=404)
            if not can_manage_finca_images(get_jwt_identity(), finca_id):
                return APIResponse.forbidden('No tienes permiso para administrar las fotos de esta finca')

            files = flask.request.files.getlist('files')
            if not files or len(files) == 0:
                return APIResponse.error(message='No se proporcionaron archivos', status_code=400)
            ...
            for file in files:
                try:
                    ...
                    if not allowed_file(file.filename):
                        errors.append({'filename': file.filename, 'error': 'Tipo de archivo no permitido'})
                        continue
                    ...
                    try:
                        webp_bytes, thumb_bytes = optimize_image_to_webp(raw_bytes, generate_thumb=True)
                        optimized = True
                    except Exception as img_err:
                        logger.warning("No se pudo convertir a WebP (%s): %s", file.filename, img_err)
                        optimized = False
                    ...
                    if optimized:
                        ...
                        file_info = save_finca_image(
                            io.BytesIO(webp_bytes), finca_id,
                    ...
                    logger.error("Error procesando archivo %s: %s", file.filename, str(e))
                    errors.append({'filename': file.filename, 'error': str(e)})

            if uploaded_images:
                db.session.commit()

                if FincaImages.query.filter_by(finca_id=finca_id).count() == len(uploaded_images):
            ...
            return APIResponse.error(message=f'Error al subir imágenes: {str(e)}', status_code=500)
...
class FincaImagesList(Resource):
    ...
    def get(self, finca_id):
        try:
            ...
            return APIResponse.error(message=f'Error al obtener imágenes: {str(e)}', status_code=500)
...
class FincaImageDetail(Resource):
    ...
    def delete(self, image_id):
        try:
            ...
            if not image:
                return APIResponse.error(message=f'Imagen con ID {image_id} no encontrada', status_code=404)
            ...
            if was_primary:
                next_image = FincaImages.query.filter_by(finca_id=finca_id)\
            ...
            return APIResponse.error(message=f'Error al eliminar imagen: {str(e)}', status_code=500)
...
class SetPrimaryFincaImage(Resource):
    ...
    def put(self, image_id):
        try:
            ...
            FincaImages.query.filter_by(finca_id=image.finca_id)\
            ...
            return APIResponse.error(message=f'Error al establecer imagen principal: {str(e)}', status_code=500)
