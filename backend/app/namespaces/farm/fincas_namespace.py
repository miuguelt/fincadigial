...
finca_stats_model = fincas_ns.model(
    "FincaStats",
    {
...
class FincaStatistics(Resource):
    @fincas_ns.doc(
        "get_finca_statistics",
        description="Estadísticas globales de fincas",
        security=["Bearer"],
    )
    @jwt_required()
    def get(self):
        try:
            ...
            if role not in ["Administrador", "Propietario"]:
                ...
                query_base = query_base.join(UserFinca).filter(
                    UserFinca.user_id == user_id, UserFinca.is_active == True
                )

            type_stats = db.session.query(Finca.type, func.count(Finca.id)).select_from(
                Finca
            )
            if role not in ["Administrador", "Propietario"]:
                type_stats = type_stats.join(UserFinca).filter(
                    UserFinca.user_id == user_id, UserFinca.is_active == True
                )
            type_stats = type_stats.group_by(Finca.type).all()

            status_stats = db.session.query(
                Finca.is_active, func.count(Finca.id)
            ).select_from(Finca)
            if role not in ["Administrador", "Propietario"]:
                status_stats = status_stats.join(UserFinca).filter(
                    UserFinca.user_id == user_id, UserFinca.is_active == True
                )
            ...
            return APIResponse.success(
                data=payload, message="Estadísticas de fincas obtenidas"
            )
        except Exception as e:
            logger.error(
                "Error obteniendo estadísticas de fincas: %s", e, exc_info=True
            )
...
class FincaGlobalActivity(Resource):
    @fincas_ns.doc(
        "get_finca_global_activity",
        description="Historial de movimientos global de fincas",
        security=["Bearer"],
    )
    @jwt_required()
    def get(self):
        try:
            ...
            logger.error(
                "Error obteniendo actividad global de fincas: %s", e, exc_info=True
            )
            return APIResponse.error(
                "Error interno del servidor", details={"error": str(e)}, status_code=500
            )
...
def _membership_flags(finca_ids: list[int]) -> dict[int, dict]:
    ...
    """
    flags = {
        finca_id: {"is_member": False, "already_requested": False}
    ...
    member_rows = (
        db.session.query(UserFinca.finca_id)
        .filter(
    ...
    pending_rows = (
        db.session.query(JoinRequest.finca_id)
        .filter(
...
class PublicFincasList(Resource):
    @fincas_ns.doc(
        "public_fincas_list",
        description="Listar fincas públicas disponibles para solicitar membresía. No requiere autenticación.",
    )
...
def _public_livestock_stats(finca_id: int) -> dict:
    ...
    rows = (
        db.session.query(Animals.sex, Animals.status, func.count(Animals.id))
        .filter(Animals.finca_id == finca_id)
        .group_by(Animals.sex, Animals.status)
        .all()
    )
...
class PublicFincaDetail(Resource):
    @fincas_ns.doc(
        "public_finca_detail",
        description="Obtener detalles públicos de una finca específica",
    )
    @jwt_required(optional=True)
    def get(self, finca_id):
        try:
            ...
            if not finca.is_active:
                return APIResponse.error(
                    "Esta finca no está disponible", status_code=400
                )
            ...
            logger.error(
                "Error obteniendo finca pública %s: %s", finca_id, e, exc_info=True
            )
            return APIResponse.error("Error interno del servidor", status_code=500)


finca_location_model = fincas_ns.model(
    "FincaLocationUpdate",
    {
...
class FincaLocationUpdate(Resource):
    @fincas_ns.doc(
        "update_finca_location",
        description="Actualizar coordenadas GPS de una finca",
        security=["Bearer"],
    )
    ...
    def patch(self, finca_id: int):
        try:
            ...
            if lat is None or lon is None:
                return APIResponse.error(
                    "latitude y longitude son obligatorios", status_code=400
                )
            ...
            try:
                ...
                return APIResponse.error(
                    "latitude y longitude deben ser numéricos", status_code=400
                )

            if not (-90 <= lat <= 90) or not (-180 <= lon <= 180):
                return APIResponse.error(
                    "Coordenadas fuera de rango geográfico", status_code=400
                )
            ...
            db.session.rollback()
            logger.error(
                "Error actualizando ubicación de finca %s: %s",
                finca_id,
                e,
                exc_info=True,
            )
