...
class InventorySummary(Resource):
    ...
    def get(self):
        ...
        total_lots = InventoryLot.query.count()
        med_lots = InventoryLot.query.filter_by(product_type=ProductType.Medicamento).count()
        vac_lots = InventoryLot.query.filter_by(product_type=ProductType.Vacuna).count()

        expired = InventoryLot.query.filter(InventoryLot.expiry_date < today).count()
        expiring_soon = InventoryLot.query.filter(
            and_(InventoryLot.expiry_date >= today, InventoryLot.expiry_date <= expiry_threshold)
        ).count()

        low_stock_lots = [
            lot for lot in InventoryLot.query.all()
            if lot.is_low_stock and not lot.is_expired
        ]

        total_value = db.session.query(
            func.sum(InventoryLot.current_quantity * InventoryLot.unit_cost)
        ).filter(InventoryLot.unit_cost.isnot(None)).scalar() or 0
...
class InventoryAlerts(Resource):
    ...
    def get(self):
        ...
        expired = InventoryLot.query.filter(
            InventoryLot.expiry_date < today
        ).order_by(InventoryLot.expiry_date).all()

        expiring = InventoryLot.query.filter(
            and_(InventoryLot.expiry_date >= today, InventoryLot.expiry_date <= expiry_threshold)
        ).order_by(InventoryLot.expiry_date).all()

        low_stock = [lot for lot in InventoryLot.query.all() if lot.is_low_stock and not lot.is_expired]
