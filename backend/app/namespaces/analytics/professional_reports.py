...
from app.services.pdf_report_service import generate_animal_cv_pdf, generate_financial_statement_pdf
from app.services.analytics.farming_intelligence_service import FarmingIntelligenceService
from app.models.animals import Animals
from app.models.reproduction import ReproductiveEvent
from app.models.user_finca import UserFinca
from app.models.finca import Finca

ns = Namespace('reports_pro', description='Reportes profesionales en PDF para bancos y compradores')
...
class AnimalCVReport(Resource):
    ...
    def get(self, animal_id):
        ...
        repro_events = ReproductiveEvent.query.filter_by(animal_id=animal.id).order_by(ReproductiveEvent.event_date.desc()).all()
        ...
        response.headers['Content-Disposition'] = f'attachment; filename=hoja_vida_{animal.record}.pdf'
        return response
...
class FinancialStatementReport(Resource):
    ...
    def get(self):
        ...
        # Aquí llamaríamos al servicio de economía para obtener los datos
        # Por ahora simulamos la estructura que espera el servicio de PDF
        from app.models.production_finance import FarmExpenses
        items = FarmExpenses.query.filter_by(finca_id=finca.id).order_by(FarmExpenses.expense_date.desc()).all()
        ...
        response = make_response(pdf_content)
        response.headers['Content-Type'] = 'application/pdf'
        response.headers['Content-Disposition'] = 'attachment; filename=estado_cuenta_campesino.pdf'
        return response
