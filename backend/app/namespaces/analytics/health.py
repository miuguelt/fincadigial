...
health_analytics_ns = Namespace(
    "analytics/health", description="🏥 Analytics - Salud animal"
)
    ...
    def get(self):
        ...
        try:
            ...
            treatment_q = _tf(db.session.query(Treatments), Treatments).filter(
                Treatments.treatment_date >= start_date, Treatments.is_deleted == False
            )
            ...
            if animal_id:
                ...
                vaccination_q = vaccination_q.filter(
                    Vaccinations.animal_id == animal_id
                )

            treatments_by_month_query = _tf(
                db.session.query(
            ...
            if animal_id:
                treatments_by_month_query = treatments_by_month_query.filter(
                    Treatments.animal_id == animal_id
                )
            treatments_by_month = (
                treatments_by_month_query.group_by(
            ...
            vaccinations_by_month_query = _tf(
                db.session.query(
            ...
            if animal_id:
                vaccinations_by_month_query = vaccinations_by_month_query.filter(
                    Vaccinations.animal_id == animal_id
                )
            vaccinations_by_month = (
                vaccinations_by_month_query.group_by(
            ...
            if animal_id:
                health_status_query = health_status_query.filter(
                    Control.animal_id == animal_id
                )
            health_status_dist = health_status_query.group_by(
                Control.health_status
            ).all()

            common_diseases_query = _tf(
                db.session.query(
            ...
            if animal_id:
                common_diseases_query = common_diseases_query.filter(
                    Treatments.animal_id == animal_id
                )
            common_diseases = (
                common_diseases_query.group_by(Treatments.description)
                .order_by(desc(func.count(Treatments.id)))
                .limit(10)
                .all()
            )

            medication_usage_query = (
                _tf(
                    db.session.query(
                        Medications.name,
                ...
                .join(
                    Treatments,
                    Treatments.description.ilike(
                        func.concat("%", Medications.name, "%")
                    ),
                )
                .filter(
            ...
            if animal_id:
                medication_usage_query = medication_usage_query.filter(
                    Treatments.animal_id == animal_id
                )
            medication_usage = (
                medication_usage_query.group_by(Medications.name)
                .order_by(desc(func.count(Treatments.id)))
                .limit(10)
                .all()
            )

            health_total = sum(count for _, count in health_status_dist)
            healthy_statuses = {"Excelente", "Bueno", "Sano"}
            healthy_controls = sum(
                count
                for status, count in health_status_dist
                if getattr(status, "value", status) in healthy_statuses
            )
            healthy_control_rate = (
                round((healthy_controls / health_total) * 100, 1)
                if health_total
                else None
            )
            ...
            return APIResponse.success(
                data={
                    ...
                    "healthy_control_rate": healthy_control_rate,
                    "medication_usage": [
                    ...
                    "summary": {
                        ...
                        "total_vaccinations": sum(
                            c for _, _, c in vaccinations_by_month
                        ),
