...
calendar_ns = Namespace(
    "analytics/calendar", description="📅 Analytics - Calendario global de eventos"
)
...
class GlobalCalendar(Resource):
    @calendar_ns.doc(
        ...
        params={
            ...
            "alert_mode": {
                "description": "summary (por día) o details",
                "type": "string",
            },
            "alert_limit": {
                "description": "Máximo de detalles de alerta (1-200)",
                "type": "integer",
            },
            "only_alerts": {
                "description": "Omitir otros eventos al consultar el detalle diario",
                "type": "boolean",
            },
        },
    ...
    def get(self):
        ...
        try:
            ...
            alert_mode = (
                (flask.request.args.get("alert_mode") or "summary").strip().lower()
            )
            ...
            alert_limit = max(
                1,
                min(alert_limit or DEFAULT_ALERT_DETAIL_LIMIT, MAX_ALERT_DETAIL_LIMIT),
            )
            ...
            if not only_alerts:
                repro_events = (
                    _tf(
                        ReproductiveEvent.query.options(
                ...
                append_regular_event(
                    {
                        ...
                        or f"Evento reproductivo: {ev.event_type.value}",
                    }
                )
                # Añadir fecha estimada de parto como evento futuro si existe
                if (
                    ev.expected_birth_date
                    and start_date <= ev.expected_birth_date <= end_date
                ):
                    append_regular_event(
                        {
            ...
            if alert_mode == "summary":
                ...
                alerts = (
                    _tf(AnimalAlert.query, AnimalAlert)
                    .filter(*alert_filters)
                    .order_by(
                        priority_order,
            ...
            return APIResponse.success(
                data={
                    ...
                    "alerts": {
                        ...
                        "truncated": alert_mode == "details"
                        and loaded_alerts < alert_total,
                        "limit": alert_limit,
                    },
                }
            )
            ...
            return APIResponse.error(
                message="Error al cargar calendario", details={"error": str(e)}
            )
