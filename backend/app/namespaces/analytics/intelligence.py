...
from app.services.analytics.regional_intelligence import RegionalBenchmarkingService, DigitalAdvisorService
from app.models.user_finca import UserFinca

ns = Namespace('intelligence', description='Inteligencia ganadera y KPIs para el productor')
...
class RegionalComparison(Resource):
    @jwt_required()
    def get(self):
        """Compara la finca con el promedio regional."""
        user_id = get_jwt_identity()
        user_finca = UserFinca.query.filter_by(user_id=user_id, is_active=True).first()
        # En producción calcularíamos estos KPIs reales de la finca
        farm_kpis = {"avg_milk_per_cow": 12.5, "avg_open_days": 115}
...
class DigitalAdvisor(Resource):
    ...
    def get(self):
        ...
        user_id = get_jwt_identity()
        user_finca = UserFinca.query.filter_by(user_id=user_id, is_active=True).first()
        recommendations = DigitalAdvisorService.generate_recommendations(user_finca.finca_id)
