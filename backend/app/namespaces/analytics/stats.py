...
animal_stats_model = analytics_ns.model(
    "AnimalStats",
    {
        "by_status": fields.Raw(description="Por estado"),
        "by_sex": fields.Raw(description="Por sexo"),
        "by_sex_active": fields.Raw(description="Por sexo, solo animales vivos"),
...
class AnimalStatistics(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            age_config = _get_stats_config(
                "config.age_groups",
                {
                    "Terneros (0-1 año)": {"min": 0, "max": 1},
                    "Jóvenes (1-2 años)": {"min": 1, "max": 2},
                    "Adultos (2-5 años)": {"min": 2, "max": 5},
                    "Maduros (5+ años)": {"min": 5, "max": 999},
                },
            )
            ...
            weight_config = _get_stats_config(
                "config.weight_ranges",
                {
                    "0-200 kg": 200,
                    "201-400 kg": 400,
                    "401-600 kg": 600,
            ...
            by_sex_active_dict = {}
            for sex, count in active_sex_stats:
                if sex is not None:
                    key = sex.value if hasattr(sex, "value") else str(sex)
                    by_sex_active_dict[key] = count
            ...
            return APIResponse.success(
                data={
                    ...
                    "by_sex": by_sex_dict,
                    "by_sex_active": by_sex_active_dict,
            ...
            return APIResponse.error(
                message="Error obteniendo estadísticas", details={"error": str(e)}
            )
