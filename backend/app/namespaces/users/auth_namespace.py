...
# Crear el namespace
auth_ns = Namespace(
    "auth", description="🔐 Autenticación y Autorización Optimizada", path="/auth"
)
    ...
    try:
        ...
        def login_key_func():
            try:
                ...
                ip = (
                    (flask.request.remote_addr or "127.0.0.1")
                    if flask.request
                    else "127.0.0.1"
                )
            ident = None
        ...
        try:
            LoginResource.post = limiter.limit(login_limit, key_func=login_key_func)(
                LoginResource.post
            )
        ...
        try:
            RefreshTokenResource.post = limiter.limit(
                refresh_limit, key_func=get_user_id
            )(RefreshTokenResource.post)
            RefreshTokenResource.post._rate_limit_applied = True
        except Exception:
            logger.exception(
                "No se pudo aplicar rate limit a RefreshTokenResource.post"
            )
        try:
            LogoutResource.post = limiter.limit(logout_limit, key_func=get_user_id)(
                LogoutResource.post
            )
        ...
        try:
            ChangePasswordResource.post = limiter.limit(
                change_limit, key_func=get_user_id
            )(ChangePasswordResource.post)
            ChangePasswordResource.post._rate_limit_applied = True
        except Exception:
            logger.exception(
                "No se pudo aplicar rate limit a ChangePasswordResource.post"
            )
        try:
            PasswordRecoverResource.post = limiter.limit(
                recover_limit, key_func=get_remote_address_with_forwarded
            )(PasswordRecoverResource.post)
            PasswordRecoverResource.post._rate_limit_applied = True
        except Exception:
            logger.exception(
                "No se pudo aplicar rate limit a PasswordRecoverResource.post"
            )
        try:
            PasswordResetResource.post = limiter.limit(
                reset_limit, key_func=get_remote_address_with_forwarded
            )(PasswordResetResource.post)
            PasswordResetResource.post._rate_limit_applied = True
        except Exception:
            logger.exception(
                "No se pudo aplicar rate limit a PasswordResetResource.post"
            )
        ...
        try:
            ...
            logger.exception(
                "No se pudo configurar la exención de rate limit para CurrentUserResource.get"
            )
    except Exception as e:
        logger.warning(
            f"No se pudo configurar/aplicar rate limiting en auth_namespace: {e}",
            exc_info=True,
        )
...
login_model = auth_ns.model(
    "Login",
    {
        ...
        "identifier": fields.Raw(
            required=False,
            description="Email o número de identificación (campo principal) (string | number)",
            example="12345678",
        ),
        "identification": fields.Raw(
            required=False,
            description="Alias: número de identificación si no se envía identifier (string | number)",
            example="12345678",
        ),
        "email": fields.String(
            required=False,
            description="Alias: email si no se envía identifier",
            example="user@example.com",
        ),
        "password": fields.String(
            required=True, description="Contraseña", example="<TEST_PASSWORD>"
        ),
    },
)
...
password_change_model = auth_ns.model(
    "PasswordChange",
    {
        "current_password": fields.String(
            required=True, description="Contrasena actual", example="<CURRENT_PASSWORD>"
        ),
        "new_password": fields.String(
            required=True,
            description="Nueva contrasena (minimo 8 caracteres, compleja)",
            example="<NEW_PASSWORD>",
        ),
    },
)

password_recover_model = auth_ns.model(
    "PasswordRecoverRequest",
    {
        "identifier": fields.Raw(
            required=False,
            description="Email o numero de identificacion",
            example="user@example.com",
        ),
        "email": fields.String(
            required=False,
            description="Alias: email del usuario",
            example="user@example.com",
        ),
        "identification": fields.Raw(
            required=False,
            description="Alias: numero de identificacion",
            example="12345678",
        ),
    },
)
...
password_reset_model = auth_ns.model(
    "PasswordReset",
    {
        "reset_token": fields.String(
            required=True, description="Token de recuperacion generado en /auth/recover"
        ),
        "new_password": fields.String(
            required=True,
            description="Nueva contrasena valida",
            example="<NEW_PASSWORD>",
        ),
    },
)
...
class LoginResource(Resource):
    @auth_ns.doc(
        "login_user",
        description="Autenticar usuario y generar tokens JWT. Acepta campos: identifier | identification | email",
    )
    ...
    def post(self):
        ...
        identifier = (
            data.get("identifier") or data.get("identification") or data.get("email")
        )
        identifier_str = None
        ...
        logger.info(
            f"Login attempt for identifier: {identifier_str} (Path: {flask.request.path})"
        )
        ...
        try:
            ...
            if not user:
                ...
                log_authentication_attempt(
                    identifier_str, False, {"reason": "invalid_credentials"}
                )
            ...
            if not pwd_ok:
                logger.warning(
                    "Login failed: Incorrect password for user %s", user.identification
                )
                log_authentication_attempt(
                    identifier_str, False, {"reason": "invalid_credentials"}
                )
            ...
            if not user.status:
                log_authentication_attempt(
                    identifier_str, False, {"reason": "user_inactive"}
                )
            ...
            user_claims = {
                ...
                if user.finca and user.finca.type
                else "Educativa",
            }
            ...
            expires_in = (
                int(expires_conf.total_seconds())
                if isinstance(expires_conf, timedelta)
                else None
            )
            ...
            api_response_dict, status_code = APIResponse.success(
                message="Login exitoso", data=response_data
            )
            ...
            log_authentication_attempt(
                identifier_str, False, {"reason": "server_error", "error": str(e)}
            )
            return APIResponse.error(
...
class RefreshTokenResource(Resource):
    ...
    def _perform_refresh():
        ...
        try:
            ...
            logger.error(
                "Unexpected error validating refresh token: %s", e, exc_info=True
            )
            return APIResponse.error(
                "Error al validar token", status_code=500, details={"error": str(e)}
            )
        ...
        try:
            ...
            return APIResponse.error(
                "Error al renovar token", status_code=500, details={"error": str(e)}
            )
...
class LogoutResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            access_cookie_name = flask.current_app.config.get(
                "JWT_ACCESS_COOKIE_NAME", "access_token_cookie"
            )
            refresh_cookie_name = flask.current_app.config.get(
            ...
            try:
                ...
                logger.warning(
                    "No se pudo revocar el access token en logout: %s", decode_err
                )
            ...
            try:
                ...
                logger.warning(
                    "No se pudo revocar el refresh token en logout: %s", decode_err
                )
            ...
            api_response_dict, status_code = APIResponse.success(
                message="Sesión cerrada exitosamente", data={"should_clear_auth": True}
            )
            resp = flask.jsonify(api_response_dict)
            unset_jwt_cookies(resp)
            resp.status_code = status_code
...
class CurrentUserResource(Resource):
    @auth_ns.doc(
        "get_current_user", description="Obtener información del usuario autenticado."
    )
    @jwt_required()
    def get(self):
        ...
        try:
            ...
            logger.error(
                f"Error obteniendo usuario actual (ID: {get_jwt_identity()}): {e}",
                exc_info=True,
            )
            return APIResponse.error(
                "Error interno del servidor", details={"error": str(e)}, status_code=500
...
class ChangePasswordResource(Resource):
    @auth_ns.doc(
        "change_password",
        description="Actualizar la contrasena del usuario autenticado.",
    )
    ...
    def post(self):
        ...
        if not current_password or not new_password:
            return APIResponse.validation_error(
                {"current_password": "Requerido", "new_password": "Requerido"}
            )
        ...
        try:
            ...
            if user.check_password(new_password):
                return APIResponse.validation_error(
                    {
                        "new_password": "La nueva contrasena no puede ser igual a la actual"
                    }
                )
            ...
            try:
                ...
                logger.debug(
                    "No se pudo revocar tokens en cambio de contrasena: %s", revoke_err
                )
            ...
            try:
                ...
                logger.debug(
                    "No se pudo registrar log de cambio de contrasena", exc_info=True
                )
            ...
            logger.error(
                f"Error actualizando contrasena para usuario {get_jwt_identity()}: {e}",
                exc_info=True,
            )
            db.session.rollback()
            return APIResponse.error(
...
class PasswordRecoverResource(Resource):
    @auth_ns.doc(
        "recover_password",
        description="Enviar correo con enlace de recuperacion y generar token temporal para recuperar la contrasena.",
    )
    @auth_ns.expect(password_recover_model)
    def post(self):
        data = flask.request.get_json() or {}
        identifier = (
            data.get("identifier") or data.get("email") or data.get("identification")
        )
        if isinstance(identifier, (int, float)):
        ...
        if not identifier:
            return APIResponse.validation_error(
                {"identifier": "Se requiere email o identificacion"}
            )
        ...
        if not reset_link:
            logger.error(
                "No se pudo construir enlace de recuperacion para usuario %s", user.id
            )
            return APIResponse.error(
                "No se pudo generar enlace de recuperacion", status_code=500
            )
        ...
        if not sent:
            ...
            return APIResponse.error(
                "No se pudo enviar el correo de recuperacion", status_code=500
            )
        ...
        try:
            ...
            logger.debug(
                "No se pudo registrar log de solicitud de recuperacion", exc_info=True
            )
        ...
        return APIResponse.success(
            message="Correo de recuperacion enviado", data=response_data
        )
...
class PasswordResetResource(Resource):
    @auth_ns.doc(
        "reset_password",
        description="Restablecer contrasena usando el token generado en /auth/recover.",
    )
    ...
    def post(self):
        ...
        if not reset_token or not new_password:
            return APIResponse.validation_error(
                {"reset_token": "Requerido", "new_password": "Requerido"}
            )
        ...
        try:
            ...
            return APIResponse.unauthorized(
                "Token de recuperacion invalido o expirado", details={"error": str(e)}
            )
        except Exception as e:
            logger.error(
                f"Error decodificando token de recuperacion: {e}", exc_info=True
            )
            return APIResponse.error(
                "Error al validar token de recuperacion",
                status_code=500,
                details={"error": str(e)},
            )

        if (
            decoded.get("type") != "refresh"
            or decoded.get("purpose") != "password_reset"
        ):
            return APIResponse.unauthorized(
                "Token de recuperacion invalido",
                details={"reason": decoded.get("type")},
            )
        ...
        try:
            ...
            db.session.rollback()
            logger.error(
                f"Error guardando nueva contrasena para usuario {user_id}: {e}",
                exc_info=True,
            )
            return APIResponse.error(
                "Error al guardar la nueva contrasena",
                status_code=500,
                details={"error": str(e)},
            )
        ...
        try:
            ...
            logger.debug(
                "No se pudo marcar token de recuperacion como usado", exc_info=True
            )
        ...
        try:
            log_admin_action(
                ...
                "User",
                user.id,
                changes={"via": "recover_endpoint"},
            )
        except Exception:
            logger.debug(
                "No se pudo registrar log de reset de contrasena", exc_info=True
            )
...
@auth_ns.route("/dev-users")
class DevUsersResource(Resource):
    @auth_ns.doc(
        "dev_users_list",
        responses={
            200: "Lista de usuarios de prueba para desarrollo",
            403: "No disponible en entorno de producción",
        },
    )
    def get(self):
        """Lista de usuarios y roles de prueba disponibles (Solo en desarrollo/pruebas)."""
        import os

        env = os.environ.get("FLASK_ENV", "development").lower()
        if env == "production":
            return APIResponse.forbidden("Endpoint deshabilitado en entorno productivo")

        dev_ids = [1098, 55555555, 66666666, 11111111, 22222222, 33333333, 44444444]
        users = User.query.filter(User.identification.in_(dev_ids)).all()

        data = [
            {
                "id": str(u.identification),
                "label": u.fullname
                or (u.role.value if hasattr(u.role, "value") else str(u.role)),
        ...
        return APIResponse.success(
            message="Usuarios de desarrollo recuperados", data={"dev_users": data}
        )
