...
from app import db
from app.models.join_request import JoinRequest, JoinRequestStatus, JoinRequestType, InvitationMethod
...
invitations_ns = Namespace('invitations', description='📨 Sistema de Invitaciones con Tokens')

invite_model = invitations_ns.model('CreateInvitation', {
    ...
    'method': fields.String(description='Método de envío', enum=['email', 'link', 'qr', 'code']),
    'max_uses': fields.Integer(description='Usos máximos del token', default=1),
})

token_accept_model = invitations_ns.model('AcceptByToken', {
    'token': fields.String(required=True, description='Token de invitación'),
})

respond_model = invitations_ns.model('RespondToInvitation', {
...
class CreateInvitation(Resource):
    ...
    def post(self):
        ...
        req, token = InvitationService.create_invitation(
            admin_user_id=current_user_id, finca_id=finca_id,
            target_user=target_user, target_email=email,
            role=role, notes=notes, expires_hours=expires_hours,
            method=method, max_uses=max_uses
        )
        ...
        try:
            target = target_user or (User.query.filter_by(email=email).first() if email else None)
            if target:
                PushNotificationService.send_to_user(
                    user_id=target.id,
                    title='Nueva Invitación',
                    body=f'Has sido invitado a unirte a una finca como {role}.',
                    data={'type': 'invitation', 'request_id': req.id, 'url': f'/invite/{token}'}
                )
...
class RespondToInvitation(Resource):
    ...
    def post(self, request_id):
        ...
        req = InvitationService.respond_to_invitation(request_id, current_user_id, approve)
...
class FarmSearchResource(Resource):
    @invitations_ns.doc('search_farms', description='Buscar fincas por nombre para solicitar unirse', security=['Bearer'])
    @jwt_required()
    def get(self):
        try:
...
class RequestJoinFarm(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            existing = UserFinca.query.filter_by(user_id=current_user_id, finca_id=farm_id).first()
            if existing:
                return APIResponse.validation_error({'finca_id': 'Ya eres miembro de esta finca'})
            ...
            if pending:
                return APIResponse.validation_error({'finca_id': 'Ya tienes una solicitud pendiente para esta finca'})
...
class PendingInvitations(Resource):
    ...
    def get(self):
        ...
        admin_memberships = UserFinca.query.filter_by(
            ...
            UserFinca.role.in_(['Administrador', 'Propietario', 'Capataz'])
        ).all()
