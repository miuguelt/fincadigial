...
def set_limiter(app_limiter):
    ...
    try:
        ...
        create_limit = (RATE_LIMIT_CONFIG.get("users", {}) or {}).get(
            "create", "10 per hour"
        )
...
class UserAvatarResource(Resource):
        ...
        try:
            ...
            if current_user_id != user_id and get_jwt().get("role") != "Administrador":
                return APIResponse.forbidden(
                    "No puede cambiar el avatar de otro usuario"
                )

            if "file" not in flask.request.files:
                return APIResponse.validation_error(
                    {"file": "No se proporcionó archivo"}
                )
            ...
            return APIResponse.success(
                data={"avatar_url": avatar_url}, message="Avatar actualizado"
            )
        except Exception as e:
            db.session.rollback()
    ...
    def delete(self, user_id):
        try:
            ...
            if current_user_id != user_id and get_jwt().get("role") != "Administrador":
                return APIResponse.forbidden(
                    "No puede eliminar el avatar de otro usuario"
                )
...
class UserStatistics(Resource):
    ...
    def get(self):
        try:
            ...
            return APIResponse.success(
                data=payload, message="Estadísticas completas de usuarios"
            )
        except Exception as e:
            logger.error(
                "Error obteniendo estadísticas de usuarios: %s", e, exc_info=True
            )
...
class UserFincas(Resource):
    ...
    def get(self, user_id):
        try:
            ...
            logger.error(
                "Error obteniendo fincas del usuario %s: %s", user_id, e, exc_info=True
            )
            return APIResponse.error(
                "Error interno del servidor", details={"error": str(e)}, status_code=500
            )
...
class UserApprovalStatus(Resource):
    ...
    def patch(self, user_id):
        try:
            ...
            old_status = (
                str(target_user.approval_status.value)
                if target_user.approval_status
                else "None"
            )
...
class UserStatusStats(Resource):
    ...
    def get(self):
        try:
            return APIResponse.success(
                data=get_user_status_stats(),
                message="Estadísticas de estado de usuarios",
            )
        except Exception as e:
            logger.error(
                "Error obteniendo estadísticas de estado: %s", e, exc_info=True
            )
            return APIResponse.error(
                "Error interno del servidor", details={"error": str(e)}, status_code=500
            )
...
class UserPublicCreate(Resource):
    ...
    def post(self):
        try:
            ...
            if (
                password_confirmation is not None
                and password_confirmation != password_raw
            ):
                return APIResponse.validation_error(
            ...
            try:
                ...
                logger.info(
                    f"Enviando alertas de registro a {len(target_user_ids)} destinatarios (Admin global)"
                )
            ...
            if m:
                ...
                # SQLite
                m2 = re.search(
                    r"UNIQUE constraint failed: (.+)", msg, flags=re.IGNORECASE
                )
                if m2:
                    key_name = m2.group(1)
                else:
