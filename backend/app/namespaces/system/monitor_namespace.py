...
monitor_ns = Namespace('system-monitor', description='Monitoreo del sistema y ecosistema', path='/system')

LOG_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'logs'))
DOCS_DIRS = [
    os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'docs')),
    os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'backend', 'docs')),
    os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'frontend', 'docs')),
]
    ...
    def get(self):
        ...
        for f in sorted(os.listdir(LOG_DIR), reverse=True):
            ...
            if os.path.isfile(fp):
                ...
                category = 'json' if ext == '.json' else 'log' if ext == '.log' else 'other'
                logs.append({
                    'name': f,
                    'size_kb': size_kb,
                    'modified': mtime,
...
class EcosystemStatus(Resource):
    def get(self):
        ...
        try:
            import json
            config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', '..', '_core', 'GLOBAL_CONFIG.json'))
        ...
        # Puertos de proyecto
        proj_vl_back = g_ports.get("projects", {}).get("villaluz", {}).get("backend_port", 8092)
        proj_vl_front = g_ports.get("projects", {}).get("villaluz", {}).get("frontend_port", 3005)
        ...
        for port, name in known_ports.items():
            ...
            sock.close()
            ports_info[str(port)] = {
                'name': name, 'open': result == 0, 'port': port
            }
                ...
                for f in sorted(files):
                    if f.endswith('.md'):
                        ...
                        guides.append({
                            'name': f,
                            'path': rel,
                            'directory': os.path.basename(root) if root != base_dir else '',
                            'category': os.path.basename(os.path.dirname(os.path.join(root, f))) if root != base_dir else 'general',
