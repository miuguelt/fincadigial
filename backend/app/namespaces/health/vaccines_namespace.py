...
vaccine_model = vaccines_ns.model(
    "Vaccine",
    {
...
class VaccinesWithRouteAdmin(Resource):
    @jwt_required()
    @vaccines_ns.doc(
        "get_vaccines_with_route_administration",
        description="Listado paginado de vacunas incluyendo la relación de ruta de administración. Ejemplos: ?page=1&limit=50&search=anti&fields=id,name,manufacturer",
        params={
            "page": "Página (int)",
            "limit": "Elementos por página (int)",
            "search": "Texto de búsqueda simple por nombre",
            "fields": "Lista separada por comas de campos a incluir (ej: id,name,manufacturer)",
        },
    )
    def get(self):
        ...
        for v in pagination.items:
            ...
            base = (
                v.to_namespace_dict(include_relations=False, fields=selected)
...
class VaccinesByRoute(Resource):
    @jwt_required()
    @vaccines_ns.doc(
        "get_vaccines_by_route",
        description="Listado paginado de vacunas filtradas por ruta de administración. Ejemplos: ?page=1&limit=50&fields=id,name,route_administration_id",
        params={
            "route_id": "ID de la ruta de administración (path)",
            "page": "Página (int)",
            "limit": "Elementos por página (int)",
            "search": "Texto de búsqueda por nombre",
