...
nav_structure_model = nav_ns.model(
    "NavigationStructure",
    {
...
class NavigationStructure(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            structure = {"version": "1.0", "base_url": "/api/v1", "groups": []}
            ...
            for cat_key, cat_data in sorted(
                categories.items(), key=lambda x: x[1]["order"]
            ):
                if cat_data["groups"]:
                    structure["groups"].extend(cat_data["groups"])

            return APIResponse.success(
                data=structure, message="Estructura de navegación generada exitosamente"
            )
