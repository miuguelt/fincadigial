...
postman_collection_model = docs_ns.model('PostmanCollection', {
    'info': fields.Nested(docs_ns.model('PostmanInfo', {
...
mobile_guide_model = docs_ns.model('MobileGuide', {
    'title': fields.String(),
...
def generate_postman_collection(base_url: str = 'http://localhost:5000') -> Dict[str, Any]:
...
class PostmanCollectionResource(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            # Agregar metadata
            collection['info']['_postman_id'] = f'finca-villa-luz-{datetime.now().strftime("%Y%m%d")}'
...
'''
                ...
                'ios_swift': {
                    'auth': '''
// Login
let response = try await api.login(identifier: "99999999", password: "password123")
...
'''
            ...
            'endpoints_importantes': [
                {'method': 'POST', 'path': '/auth/login', 'desc': 'Login y obtener tokens'},
                ...
                {'method': 'GET', 'path': '/multi-finca/my-fincas', 'desc': 'Fincas del usuario'},
                {'method': 'POST', 'path': '/multi-finca/switch', 'desc': 'Cambiar finca'},
                {'method': 'GET', 'path': '/analytics/dashboard', 'desc': 'Dashboard stats'},
            ]
        }

        return APIResponse.success(
            message='Guía de integración móvil',
            data=guide
        )
...
response = requests.get('http://localhost:5000/api/v1/animals', headers=headers)
...
curl -X POST http://localhost:5000/api/v1/auth/login \\
  ...
  -d '{"identifier":"99999999","password":"password123"}'
''',
                'list_animals': '''
    ...
    def get(self):
        ...
        try:
            ...
            spec['servers'] = [
                ...
                {'url': 'https://api.fincavillaluz.com/api/v1', 'description': 'Production'}
            ]
            spec['externalDocs'] = {
