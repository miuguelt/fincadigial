...
alert_config_model = alerts_ns.model(
    "AlertConfig",
    {
        ...
        "alert_type": fields.String(
            ...
            enum=[e.value for e in AlertType],
        ),
        "dimension": fields.String(
            required=True, description="Dimensión (Peso, Tiempo, Estado)"
        ),
        "condition_value": fields.String(
            required=True, description="Valor de la condición (ej: < 150, 90 days)"
        ),
...
class AlertConfigList(Resource):
    ...
    def get(self):
        ...
        limit = min(
            max(flask.request.args.get("limit", default=50, type=int) or 50, 1), 200
        )
...
class AlertConfigResource(Resource):
    ...
    def put(self, id):
        ...
        config.update(**data)
        return APIResponse.success(
            config.to_namespace_dict(), "Configuración actualizada"
        )
    ...
    def post(self):
        ...
        if not animal_ids:
            ...
            if field_id:
                ...
                animal_ids = [
                    af.animal_id
                    for af in AnimalFields.query.filter_by(
        ...
        if not animal_ids:
            return APIResponse.error(
                "No se proporcionaron animales para configurar", status_code=400
            )
        ...
        for aid in animal_ids:
            try:
                ...
                logging.getLogger(__name__).warning(
                    f"Error en bulk config para animal {aid}: {e}"
                )

        return APIResponse.success(
            {"count": created_count},
            f"Configuración aplicada a {created_count} animales",
        )
...
class AlertList(Resource):
    ...
    def get(self):
        ...
        pagination = query.order_by(AnimalAlert.triggered_at.desc()).paginate(
        ...
        if page == 1 and (is_read is None or is_read.lower() == "false"):
            try:
                        ...
                        virtual_alerts.append(
                            {
                                ...
                                "priority": "Alta",
                                "is_read": False,
                                "triggered_at": u.created_at.isoformat()
                                if u.created_at
                ...
                logging.getLogger(__name__).warning(
                    f"Error inyectando alertas virtuales de usuarios: {e}"
                )
    ...
    def post(self):
        ...
        try:
            ...
            return APIResponse.success(
                alert.to_namespace_dict(), "Alerta creada", status_code=201
            )
        except Exception as e:
            return APIResponse.error(str(e), status_code=400)
...
class AlertRead(Resource):
    ...
    def post(self, id):
        ...
        if id >= 1000000:
            ...
            return APIResponse.success(
                None, "Alerta de usuario virtual marcada como leída"
            )
        ...
        return APIResponse.success(
            alert.to_namespace_dict(), "Alerta marcada como leída"
        )
