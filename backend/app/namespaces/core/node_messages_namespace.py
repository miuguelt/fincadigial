...
node_messages_ns = Namespace(
    "node-messages", description="Mensajeria persistente entre nodos"
)
    ...
    def post(self):
        ...
        if not finca_id or not content:
            return APIResponse.validation_error(
                {"finca_id": "requerido", "content": "requerido"}
            )
        ...
        msg = NodeMessage(
            ...
            priority=int(
                ...
                or (10 if msg_type == NodeMessageType.ALERT else 100)
            ),
            finca_id=finca_id,
        )
        db.session.add(msg)
        ...
        return APIResponse.success(
            _message_dict(msg), message="Mensaje encolado", status_code=201
        )
