...
security_ns = Namespace(
    "security", description="🔐 Monitoreo y Dashboard de Seguridad", path="/security"
)

logger = logging.getLogger(__name__)

security_metrics_model = security_ns.model(
    "SecurityMetrics",
    {
        "total_login_attempts": fields.Integer(
        ...
        "suspicious_activities": fields.Integer(
            description="Actividades sospechosas detectadas"
        ),
        "rate_limit_violations": fields.Integer(
            description="Violaciones de rate limit"
        ),
        "malicious_content_blocks": fields.Integer(
            description="Contenido malicioso bloqueado"
        ),
    ...
    def get(self):
        try:
            ...
            return APIResponse.success(
                data=metrics, message="Métricas de seguridad obtenidas exitosamente"
            )
        except Exception as e:
            logger.error("Error obteniendo métricas de seguridad: %s", e, exc_info=True)
            return APIResponse.error(
                "Error obteniendo métricas de seguridad",
                details={"error": str(e)},
                status_code=500,
            )
...
class SecurityAuditResource(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            return APIResponse.success(
                data=alerts, message="Logs de auditoría obtenidos"
            )
        except Exception as e:
            return APIResponse.error(
                "Error en auditoría", details={"error": str(e)}, status_code=500
            )
...
class SecurityAlertsResource(Resource):
    @security_ns.doc(
        "get_security_alerts", description="Obtener alertas de seguridad recientes"
    )
    @require_permission("security", "read")
    def get(self):
        ...
        try:
            ...
            return APIResponse.success(
                data=alerts, message="Alertas de seguridad obtenidas exitosamente"
            )
...
class SecurityScanResource(Resource):
    @security_ns.doc(
        "run_security_scan", description="Ejecutar escaneo de seguridad manual"
    )
    ...
    def post(self):
        ...
        try:
            ...
            return APIResponse.success(
                data=scan_results, message="Escaneo de seguridad completado"
            )
...
def analyze_security_logs():
    ...
    metrics = {
        ...
        "active_sessions": 0,
        "last_security_scan": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
    }
    ...
    try:
        ...
        if os.path.exists(security_log_path):
            ...
            if metrics["total_login_attempts"] > 0:
                success_attempts = (
                    metrics["total_login_attempts"] - metrics["failed_login_attempts"]
                )
                metrics["success_rate"] = (
                    success_attempts / metrics["total_login_attempts"]
                ) * 100
        ...
        if os.path.exists(security_log_path):
            ...
            for line in reversed(lines[-1000:]):  # Últimas 1000 líneas
                try:
                    if any(
                        keyword in line
                        for keyword in ["WARNING", "ERROR", "SUSPICIOUS"]
                    ):
...
def check_logging_configuration():
    ...
    if not os.path.exists(security_log_path):
        ...
        result["recommendation"] = (
            "Verificar que el logging de seguridad esté funcionando"
        )
