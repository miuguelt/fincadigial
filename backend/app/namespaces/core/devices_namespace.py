...
devices_ns = Namespace(
    "devices", description="Dispositivos autorizados para sincronizacion rural"
)
...
class DevicesResource(Resource):
    @jwt_required()
    def get(self):
        finca_id = get_current_finca_id() or flask.request.args.get(
            "finca_id", type=int
        )
        if not finca_id:
        ...
        query = Device.query.filter_by(finca_id=finca_id).order_by(
...
class DeviceRegisterResource(Resource):
    ...
    def post(self):
        ...
        if not device:
            device = Device(
                finca_id=finca_id, device_id=device_id, name=name, user_id=user_id
            )
        ...
        return APIResponse.success(
            _device_dict(device), message="Dispositivo registrado", status_code=201
        )
