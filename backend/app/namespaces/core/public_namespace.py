...
# Crear namespace
public_ns = Namespace(
    "public", description="Registro público de nuevas fincas", path="/public"
)

# Modelos para documentación
finca_model = public_ns.model(
    "FincaInput",
    {
        "name": fields.String(
            required=True, description="Nombre de la finca", example="Mi Finca"
        ),
        "type": fields.String(
            required=True,
...
owner_model = public_ns.model(
    "OwnerInput",
    {
        "identification": fields.Integer(
            required=True, description="Número de identificación", example=123456789
        ),
        "fullname": fields.String(
            required=True, description="Nombre completo", example="Juan Pérez"
        ),
        "email": fields.String(
            required=True, description="Correo electrónico", example="juan@example.com"
        ),
        "phone": fields.String(
            required=True, description="Teléfono", example="3001234567"
        ),
...
register_model = public_ns.model(
    ...
    {
        "finca": fields.Nested(
            finca_model, required=True, description="Datos de la finca"
        ),
        "owner": fields.Nested(
            owner_model,
            required=True,
...
finca_response = public_ns.model(
    "FincaResponse",
    {
        "id": fields.Integer,
        "name": fields.String,
...
register_response = public_ns.model(
    "RegisterResponse",
    {
...
class RegisterFincaResource(Resource):
    ...
    @public_ns.doc(
        "register_finca",
        description="Registrar una nueva finca con su administrador/propietario.",
        security=[],
    )
    ...
    def post(self):
        ...
        try:
            validate_password(
                owner_data.get("password", ""), field_name="owner.password"
            )
        except Exception as e:
            return APIResponse.validation_error({"owner.password": str(e)})
        ...
        try:
            ...
            access_token = create_access_token(
                identity=str(user.id), additional_claims=user_claims
            )
...
class PublicInfoResource(Resource):
    ...
    @public_ns.doc(
        "public_info",
        description="Obtener información pública del sistema.",
        security=[],
    )
    def get(self):
