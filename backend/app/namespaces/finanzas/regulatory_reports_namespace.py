...
# Namespace
regulatory_ns = Namespace(
    "regulatory-reports", description="Reportes regulatorios ICA/SENA"
)
...
inventory_report_model = regulatory_ns.model(
    "InventoryReportRequest",
    {
        "date_from": fields.String(
            description="Fecha inicio (YYYY-MM-DD)", example="2024-01-01"
        ),
        "date_to": fields.String(
            description="Fecha fin (YYYY-MM-DD)", example="2024-12-31"
        ),
...
movements_report_model = regulatory_ns.model(
    "MovementsReportRequest",
    {
        ...
        "format": fields.String(description="Formato: csv, pdf, json", example="csv"),
        "type": fields.String(
            description="Tipo: births, deaths, sales, transfers, all", example="all"
        ),
    },
)

health_report_model = regulatory_ns.model(
    "HealthReportModel",
    {
        "date_from": fields.String(description="Fecha inicio", example="2024-01-01"),
        "date_to": fields.String(description="Fecha fin", example="2024-12-31"),
        "format": fields.String(description="Formato: csv, pdf, json", example="csv"),
        "type": fields.String(
            description="Tipo: vaccinations, treatments, controls, all", example="all"
        ),
    },
)
        ...
        try:
            ...
            if finca_type not in ["Tradicional", "Educativa"]:
                return APIResponse.forbidden(
                    "Los reportes regulatorios solo aplican a fincas tradicionales o educativas"
                )
            ...
            query = (
                ...
                .filter(
                    Animal.finca_id == finca_id, Animal.status.in_(["Vivo", "Vendido"])
                )
            )
                ...
                inventory_data.append(
                    {
                        ...
                        "sexo": str(animal.sex.value)
                        if hasattr(animal.sex, "value")
                        else str(animal.sex),
                        "fecha_nacimiento": animal.birth_date.isoformat()
                        if animal.birth_date
                        else "",
                        "edad_meses": age_months or "",
                        "peso_kg": f"{animal.weight:.1f}"
                        if animal.weight is not None
                        else "",
                        "ubicacion": location,
                        "estado": str(animal.status.value)
                        if hasattr(animal.status, "value")
                        else str(animal.status),
                        "fecha_ingreso": animal.entry_date.isoformat()
                        if animal.entry_date
                        else "",
                        "fecha_compra": animal.purchase_date.isoformat()
            ...
            if format_type == "pdf":
                ...
                owner_membership = UserFinca.query.filter_by(
                    finca_id=finca_id, role="Propietario"
                ).first()
                ...
                finca_info = {
                    "name": finca.name,
                    "ubication": f"{finca.municipality}, {finca.department}"
                    if finca.municipality
                    else "N/A",
                    "owner": owner_membership.user.fullname
                    if owner_membership
                    else "N/A",
                }
                pdf_content = pdf_report_service.generate_inventory_pdf(
                    ...
                    finca_info,
                    inventory_data,
                    finca_type=str(finca.type.value),
                )
                return Response(
                    pdf_content,
            ...
            return APIResponse.error(
                "Error al generar reporte", details={"error": str(e)}, status_code=500
            )
        ...
        try:
            ...
            if movement_type in ["births", "all"]:
                ...
                if date_from:
                    date_from_parsed = parse_date(date_from)
                    if date_from_parsed:
                        births_query = births_query.filter(
                            Animal.birth_date >= date_from_parsed.date()
                        )
                ...
                if date_to:
                    ...
                    if date_to_parsed:
                        births_query = births_query.filter(
                            Animal.birth_date <= date_to_parsed.date()
                        )
            ...
            if movement_type in ["deaths", "all"]:
                ...
                if date_from:
                    date_from_parsed = parse_date(date_from)
                    if date_from_parsed:
                        deaths_query = deaths_query.filter(
                            Animal.exit_date >= date_from_parsed.date()
                        )
                ...
                if date_to:
                    ...
                    if date_to_parsed:
                        deaths_query = deaths_query.filter(
                            Animal.exit_date <= date_to_parsed.date()
                        )
            ...
            if movement_type in ["sales", "all"]:
                ...
                if date_from:
                    ...
                    if date_from_parsed:
                        sales_query = sales_query.filter(
                            Animal.sale_date >= date_from_parsed.date()
                        )
                ...
                if date_to:
                    ...
                    if date_to_parsed:
                        sales_query = sales_query.filter(
                            Animal.sale_date <= date_to_parsed.date()
                        )
            ...
            # Ordenar por fecha
            movements_data.sort(
                key=lambda x: x["fecha"] if x["fecha"] else "", reverse=True
            )
            ...
            summary = {
                "nacimientos": len(
                    [m for m in movements_data if m["tipo_movimiento"] == "NACIMIENTO"]
                ),
                "muertes": len(
                    [m for m in movements_data if m["tipo_movimiento"] == "MUERTE"]
                ),
                "ventas": len(
                    [m for m in movements_data if m["tipo_movimiento"] == "VENTA"]
                ),
            }
        ...
        try:
            ...
            if health_type in ["vaccinations", "all"]:
                ...
                vacc_query = (
                    ...
                    .join(Vaccines, VacTable.vaccine_id == Vaccines.id)
                    .join(Animals, VacTable.animal_id == Animals.id)
                    .outerjoin(
                        RouteAdministration,
                        Vaccines.route_administration_id == RouteAdministration.id,
                    )
                    .outerjoin(User, VacTable.performed_by == User.id)
                    .filter(VacTable.finca_id == finca_id)
                )

                if date_from:
                    date_from_parsed = parse_date(date_from)
                    if date_from_parsed:
                        vacc_query = vacc_query.filter(
                            VacTable.vaccination_date >= date_from_parsed.date()
                        )
                ...
                if date_to:
                    ...
                    if date_to_parsed:
                        vacc_query = vacc_query.filter(
                            VacTable.vaccination_date <= date_to_parsed.date()
                        )
                ...
                for vacc in vaccinations:
                    health_data.append(
                        {
                            "fecha": vacc.vaccination_date.isoformat()
                            if vacc.vaccination_date
                            else "",
                            ...
                            "observaciones": (
                                f"Plan Sanitario Nacional: {vacc.national_plan}. "
                                if vacc.national_plan
                                else ""
                            )
                            + (vacc.notes or ""),
                        }
            ...
            if health_type in ["treatments", "all"]:
                ...
                if date_from:
                    ...
                    if date_from_parsed:
                        treat_query = treat_query.filter(
                            Treatments.treatment_date >= date_from_parsed.date()
                        )
                ...
                if date_to:
                    ...
                    if date_to_parsed:
                        treat_query = treat_query.filter(
                            Treatments.treatment_date <= date_to_parsed.date()
                        )
                ...
                for treat in treatments:
                    ...
                    if treat.withdrawal_days and treat.withdrawal_days > 0:
                        observaciones_parts.append(
                            f"Tiempo de retiro: {treat.withdrawal_days} días"
                        )
                    if treat.cost:
                    ...
                    health_data.append(
                        {
                            "fecha": treat.treatment_date.isoformat()
                            if treat.treatment_date
                            else "",
            ...
            if health_type in ["controls", "all"]:
                ...
                if date_from:
                    ...
                    if date_from_parsed:
                        control_query = control_query.filter(
                            Control.checkup_date >= date_from_parsed.date()
                        )
                ...
                if date_to:
                    ...
                    if date_to_parsed:
                        control_query = control_query.filter(
                            Control.checkup_date <= date_to_parsed.date()
                        )
            ...
            summary = {
                "vacunaciones": len(
                    [h for h in health_data if h["tipo_registro"] == "VACUNACION"]
                ),
                "tratamientos": len(
                    [h for h in health_data if h["tipo_registro"] == "TRATAMIENTO"]
                ),
                "controles": len(
                    [
                        h
                        for h in health_data
                        if h["tipo_registro"] == "CONTROL_SANITARIO"
                    ]
                ),
            }
