...
from app.models import UserFinca, User, Finca, Role
...
class InviteUserResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            current_finca_id = jwt_data.get('finca_id')

            # Solo Administradores y Propietarios pueden invitar
            if current_role not in ['Administrador', 'Propietario']:
                return APIResponse.error(
                    'Solo Administradores y Propietarios pueden invitar usuarios',
                    status_code=403
                )

            # Verificar que el usuario autenticado pertenezca a la finca destino
            if current_finca_id != target_finca_id:
                if not UserFinca.has_access(current_user_id, target_finca_id):
                    return APIResponse.error(
                        'No tiene acceso a esta finca',
                        status_code=403
                    )
    ...
    def get(self):
        ...
        try:
            ...
            for f in fincas:
                ...
                total_animals = Animals.query.filter_by(finca_id=f_id, is_active=True).count()
                total_milk = db.session.query(func.sum(MilkProduction.amount_liters)).filter_by(finca_id=f_id).scalar() or 0.0
                
                results.append({
                    'finca_id': f_id,
                    'finca_name': Finca.query.get(f_id).name,
                    'kpis': {
