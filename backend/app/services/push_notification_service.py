...
class PushNotificationService:
    ...
    @classmethod
    def send_to_subscription(cls, subscription: PushSubscription, payload: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            vapid_claims = {
                ...
                'exp': int(datetime.now(timezone.utc).timestamp()) + 12 * 3600  # 12 horas
            }
    ...
    @classmethod
    def send_to_user(cls, user_id: int, title: str, body: str,
                     icon: Optional[str] = None, data: Optional[Dict] = None,
        ...
        """
        subscriptions = PushSubscription.get_user_subscriptions(user_id, active_only=True)
        ...
        logger.info(f'Enviadas {success_count}/{len(subscriptions)} notificaciones a usuario {user_id}')
        return success_count

    @classmethod
    def send_to_users(cls, user_ids: List[int], title: str, body: str,
                      icon: Optional[str] = None, data: Optional[Dict] = None) -> Dict[int, int]:
        """
    ...
    @classmethod
    def send_to_finca(cls, finca_id: int, title: str, body: str,
                      roles: Optional[List[str]] = None, icon: Optional[str] = None,
        ...
        logger.info(f'Enviadas {total_sent} notificaciones a {len(users)} usuarios de finca {finca_id}')
        return total_sent
    ...
    def send_alert_notification(cls, alert_id: int, user_id: int) -> bool:
        ...
        count = cls.send_to_user(user_id, title, body, data=data, tag=f'alert-{alert_id}')
        return count > 0
    ...
    @classmethod
    def send_membership_request_notification(cls, request_id: int, finca_id: int, user_name: str) -> int:
        """
        ...
        return cls.send_to_finca(
            ...
            data={'type': 'membership_request', 'request_id': request_id, 'url': '/admin/membership'}
        )
    ...
    @classmethod
    def broadcast_to_all(cls, title: str, body: str,
                         icon: Optional[str] = None, data: Optional[Dict] = None) -> int:
        """
        ...
        payload = {
            ...
            'icon': icon or '/assets/icon-192x192.png',
            'badge': '/assets/badge-72x72.png',
            'tag': f'broadcast-{datetime.now(timezone.utc).timestamp()}',
        ...
        logger.info(f'Broadcast enviado a {success_count}/{len(subscriptions)} dispositivos')
        return success_count
