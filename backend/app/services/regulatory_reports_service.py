...
class RegulatoryReportsService:
    ...
    def get_movements_report(
        cls, finca_id, date_from_str, date_to_str, format_type, movement_type
    ):
        date_from = cls._parse_date(date_from_str)
        date_to = cls._parse_date(date_to_str)
        movements_data = []
        ...
        if movement_type in ["births", "all"]:
            ...
            if date_from:
                births_query = births_query.filter(
                    Animal.birth_date >= date_from.date()
                )
            if date_to:
        ...
        if format_type == "csv":
            ...
            return {
                ...
                "headers": headers,
                "data": movements_data,
                "filename": f"movimientos_ganado_{finca_id}_{datetime.now().strftime('%Y%m%d')}",
            }

        if format_type == "pdf":
            finca = Finca.query.get(finca_id)
            owner_membership = (
                UserFinca.query.filter_by(finca_id=finca_id, role="Propietario").first()
                or UserFinca.query.filter_by(
                    finca_id=finca_id, role="Administrador"
                ).first()
            )
            finca_info = {
            ...
            return {
                "type": "pdf",
                "content": pdf_content,
                "filename": f"movimientos_{finca_id}.pdf",
            }
    ...
    def get_health_report(
        cls, finca_id, date_from_str, date_to_str, format_type, health_type
    ):
        date_from = cls._parse_date(date_from_str)
        date_to = cls._parse_date(date_to_str)
        health_data = []
        ...
        if health_type in ["vaccinations", "all"]:
            ...
            if date_from:
                vacc_query = vacc_query.filter(
                    VacTable.vaccination_date >= date_from.date()
                )
            if date_to:
                vacc_query = vacc_query.filter(
                    VacTable.vaccination_date <= date_to.date()
                )
        ...
        if format_type == "csv":
            ...
            return {
                ...
                "headers": headers,
                "data": health_data,
                "filename": f"sanidad_ganado_{finca_id}_{datetime.now().strftime('%Y%m%d')}",
            }
        ...
        if format_type == "pdf":
            ...
            return {
                "type": "pdf",
                "content": pdf_content,
                "filename": f"sanidad_{finca_id}.pdf",
            }
        ...
        summary = {
            ...
            "controles": len(
                [h for h in health_data if h["tipo_registro"] == "CONTROL_SANITARIO"]
            ),
        }
