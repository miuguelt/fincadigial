    ...
    for ev in rows:
        ...
        if ev.expected_birth_date and start <= ev.expected_birth_date <= end:
            out.append(
                _base_event(
                    f"birth_est_{ev.id}",
                    f"PARTO ESTIMADO: {rec}",
                    str(ev.expected_birth_date),
                    "future_birth",
                    ev.animal_id,
                    rec,
...
def treatment_events(start: date, end: date) -> list[dict]:
    rows = (
        apply_tenant_filter(
            Treatments.query.options(joinedload(Treatments.animals)), Treatments
        )
        .filter(
    ...
    for t in rows:
        ...
        out.append(
            _base_event(
                f"treatment_{t.id}",
                f"TX: {t.description[:20]}... - {rec}",
                str(t.treatment_date),
                "health",
                t.animal_id,
                rec,
                t.description,
...
def vaccination_events(start: date, end: date) -> list[dict]:
    rows = (
        apply_tenant_filter(
            Vaccinations.query.options(joinedload(Vaccinations.animals)), Vaccinations
        )
        .filter(
...
def control_events(start: date, end: date) -> list[dict]:
    rows = (
        apply_tenant_filter(Control.query.options(joinedload(Control.animals)), Control)
        .filter(
...
def task_events(start: date, end: date) -> list[dict]:
    rows = (
        apply_tenant_filter(Tasks.query.options(joinedload(Tasks.animal)), Tasks)
        .filter(
...
def alert_events(start: date, end: date) -> list[dict]:
    rows = (
        apply_tenant_filter(
            AnimalAlert.query.options(joinedload(AnimalAlert.animal)), AnimalAlert
        )
        .filter(
