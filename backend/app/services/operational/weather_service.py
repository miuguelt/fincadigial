    ...
    def get_forecast(lat: float, lon: float) -> dict[str, Any]:
        ...
        try:
            ...
            if temp is None or wind is None or code is None:
                logger.warning(
                    "Open-Meteo response incompleta: temp=%s, wind=%s, code=%s",
                    temp,
                    wind,
                    code,
                )
            ...
            if temp < 10:
                alerts.append(
                    {
                        "type": "cold",
                        "message": "Temperatura baja: Riesgo de hipotermia en terneros jóvenes.",
                    }
                )
            elif temp > 32:
                alerts.append(
                    {
                        "type": "heat",
                        "message": "Calor extremo: Provea suficiente agua y sombra.",
                    }
                )
                ...
                alerts.append(
                    {
                        ...
                        "message": f"Condiciones de {condition} activas.",
                    }
                )

            return {
                "current": {"temp": temp, "condition": condition, "wind": wind},
                "alerts": alerts,
            }
