...
class MembershipService:
    ...
    def _check_admin_access(user_id: int, finca_id: int) -> UserFinca:
        ...
        if not membership:
            raise ForbiddenException(
                "No tienes permisos de administrador en esta finca"
            )
        return membership
    ...
    def create_invitation(
        ...
        existing = JoinRequest.query.filter_by(
            user_id=target_user_id, finca_id=finca_id, status=JoinRequestStatus.PENDING
        ).first()
        if existing:
            raise BusinessRuleException(
    ...
    @staticmethod
    def create_request(
        user_id: int, finca_id: int, role: str = "Operario", notes: str | None = None
    ) -> JoinRequest:
    ...
    @staticmethod
    def approve_direct(
        request_id: int, admin_user_id: int, role: str | None = None
    ) -> JoinRequest:
        req = MembershipService._get_join_request_or_404(request_id)
        if req.status != JoinRequestStatus.PENDING:
        ...
        UserFinca.assign(
            user_id=req.user_id, finca_id=req.finca_id, role=final_role, commit=False
        )
    ...
    def audit_memberships(user_id: int) -> dict:
        ...
        total_members = UserFinca.query.filter_by(
            finca_id=user.finca_id, is_active=True
        ).count()

        duplicate_check = (
            db.session.query(UserFinca.user_id, func.count(UserFinca.id).label("count"))
            .filter(UserFinca.finca_id == user.finca_id, UserFinca.is_active == True)
            .group_by(UserFinca.user_id)
            .having(func.count(UserFinca.id) > 1)
            .all()
        )
        ...
        duplicates = [
            {
                "user_id": user_id_dup,
                "user_fullname": db.session.get(User, user_id_dup).fullname
                if db.session.get(User, user_id_dup)
                else "N/A",
                "count": count,
            }
        ...
        multi_finca_users = (
            ...
            .having(func.count(UserFinca.finca_id) > 1)
            .all()
        )
        ...
        all_fincas = (
            db.session.query(
                Finca.id, Finca.name, func.count(UserFinca.id).label("member_count")
            )
            .outerjoin(
                UserFinca,
                (Finca.id == UserFinca.finca_id) & (UserFinca.is_active == True),
            )
            .group_by(Finca.id, Finca.name)
            .order_by(func.count(UserFinca.id).desc())
            .all()
        )

        fincas_summary = [
            {"finca_id": f_id, "finca_name": name, "member_count": count}
        ...
        return {
            ...
            "summary": {
                ...
                "total_users_with_access": db.session.query(
                    func.count(func.distinct(UserFinca.user_id))
                )
                .filter(UserFinca.is_active == True)
                .scalar()
                or 0,
