...
def _get_clinical_param(key: str) -> int | None:
...
class MedicalAnalyticsService:
    ...
    def get_upcoming_events(days_ahead: int = 30) -> dict:
        ...
        if not finca_id:
            return {'generated_at': datetime.now(UTC).isoformat(), 'horizon_days': days_ahead, 'summary': {'total': 0}}
        ...
        if females_ids:
            ...
            for g in gestations:
                ...
                gestacion_days = _get_clinical_param('gestacion_dias')
                expected_birth = g.date + timedelta(days=gestacion_days)
        ...
        for v in recent_icas:
            ...
            ica_interval = _get_clinical_param('ica_vacuna_intervalo')
            days_remaining = ica_interval - days_since
        ...
        for c in recent_controls:
            ...
            ctrl_min = _get_clinical_param('control_dias_min')
            ctrl_max = _get_clinical_param('control_dias_max')
            if ctrl_min <= days_since_ctrl <= ctrl_max:
                controls_due.append({
