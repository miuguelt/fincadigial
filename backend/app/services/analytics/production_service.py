...
class ProductionAnalyticsService:
        ...
        return [
            {
                ...
                'avg_weight': float(t.avg_weight) if isinstance(t.avg_weight, decimal.Decimal) else t.avg_weight,
                'sample_size': t.count
            } for t in trends
        ]
    ...
    def get_animal_distribution(group_by='breed'):
        ...
        if group_by == 'breed':
            ...
            ).join(Animals).filter(Animals.finca_id == finca_id).group_by(Breeds.name).all()
            return [{'label': name, 'count': count} for name, count in stats]
        
        elif group_by == 'sex':
            stats = db.session.query(
                Animals.sex, func.count(Animals.id)
            ).filter(Animals.finca_id == finca_id).group_by(Animals.sex).all()
            return [{'label': s.value if hasattr(s, 'value') else str(s), 'count': count} for s, count in stats]
