...
class PredictionService:
    ...
    def predict_animal_weight(animal_id: int, target_date: date) -> dict:
        ...
        controls = (
            ...
            .order_by(Control.checkup_date.asc())
            .all()
        )
    ...
    @staticmethod
    def estimate_market_readiness(
        animal_id: int, target_weight: int | None = None
    ) -> dict:
    ...
    def monitor_growth_anomalies(finca_id: int) -> dict:
        ...
        for animal in animals:
            ...
            last_control = (
                Control.query.filter_by(animal_id=animal.id)
                .filter(Control.weight != None)
            ...
            if no_weight_days_raw is not None and days_since_weight > int(
                no_weight_days_raw
            ):
                anomalies.append(
                    {
            ...
            pred = PredictionService.predict_animal_weight(
                animal.id, date.today() + timedelta(days=30)
            )
            ...
            if pred.get("avg_daily_gain") is not None:
                ...
                if (
                    ...
                    and market_ready_pct is not None
                    and animal.weight
                    and animal.weight >= (market_target * market_ready_pct)
                ):
                    anomalies.append(
                        {
                            "animal_id": animal.id,
