...
def evaluate_reproduction_rules(animal, finca_id, trig, today, age_months) -> int:
    ...
    last_neg_diag = (
        ReproductiveEvent.query.filter(
            ReproductiveEvent.animal_id == animal.id,
            ReproductiveEvent.event_type == EventType.Diagnostico,
            ReproductiveEvent.diagnosis_result == DiagnosisResult.Negativo,
        )
        .order_by(ReproductiveEvent.event_date.desc())
        .first()
    )
    if last_neg_diag:
        ...
        if days_since_neg > 21:
            trig(
                AlertType.REPRODUCTION,
                f" Diagnóstico negativo hace {days_since_neg} días. Re-inseminar para no perder ciclo reproductivo.",
                AlertPriority.HIGH,
            )
        elif days_since_neg > 14:
            trig(
                AlertType.REPRODUCTION,
                f" Diagnóstico negativo hace {days_since_neg} días. Preparar re-inseminación en próximos días.",
                AlertPriority.MEDIUM,
            )
    ...
    if len(inseminations) >= 3 and len(positive_diags) == 0:
        trig(
            AlertType.REPRODUCTION,
            f" REPEAT BREEDER: {len(inseminations)} inseminaciones sin preñez confirmada. Evaluar fertilidad, toro y técnica.",
            AlertPriority.HIGH,
        )
    elif len(inseminations) >= 5 and len(positive_diags) == 0:
        trig(
            AlertType.REPRODUCTION,
            f" REPEAT BREEDER CRÍTICO: {len(inseminations)} inseminaciones fallidas. Considerar descarte reproductivo.",
    ...
    if last_insem and last_insem.expected_birth_date:
        ...
        if days_overdue > 14:
            trig(
                AlertType.REPRODUCTION,
                f" PARTO VENCIDO: {days_overdue} días past due date ({last_insem.expected_birth_date}). Riesgo de distocia o muerte fetal.",
                AlertPriority.CRITICAL,
            )
        elif days_overdue > 7:
            trig(
                AlertType.REPRODUCTION,
                f" Parto retrasado: {days_overdue} días past due date. Vigilancia intensiva.",
                AlertPriority.HIGH,
            )
        elif days_overdue > 0:
            trig(
                AlertType.REPRODUCTION,
                f" Parto vencido: {days_overdue} días past due date. Monitorear signos de parto.",
                AlertPriority.MEDIUM,
            )
    ...
    last_birth_comp = (
        ReproductiveEvent.query.filter(
            ReproductiveEvent.animal_id == animal.id,
            ReproductiveEvent.event_type == EventType.Parto,
            ReproductiveEvent.complications == True,
        )
        .order_by(ReproductiveEvent.event_date.desc())
        .first()
    )
    if last_birth_comp:
        ...
        if days_since_comp <= 3:
            trig(
                AlertType.HEALTH,
                f" POST-COMPLICACIÓN día {days_since_comp}: Revisión veterinaria obligatoria por complicaciones en parto.",
                AlertPriority.CRITICAL,
            )
        elif days_since_comp <= 7:
            trig(
                AlertType.HEALTH,
                f" POST-COMPLICACIÓN día {days_since_comp}: Segunda revisión post-complicaciones. Verificar recuperación.",
                AlertPriority.HIGH,
            )
    ...
    for birth in recent_births:
        if birth.alive_count and birth.alive_count > 1:
            ...
            if days_since_twin <= 7:
                trig(
                    AlertType.REPRODUCTION,
                    f" PARTO GEMELAR: {birth.alive_count} crías vivas. Manejo especial: doble calostro, monitoreo de hipocalcemia.",
                    AlertPriority.HIGH,
                )

    # ── 12. SECUENCIA INCONSISTENTE ───────────────────────────────────
    births_no_insem = ReproductiveEvent.query.filter(
        ReproductiveEvent.animal_id == animal.id,
        ReproductiveEvent.event_type == EventType.Parto,
    ).all()
    for birth in births_no_insem:
        prior_service = ReproductiveEvent.query.filter(
            ReproductiveEvent.animal_id == animal.id,
            ReproductiveEvent.event_type.in_([EventType.Inseminacion, EventType.Celo]),
            ReproductiveEvent.event_date < birth.event_date,
            ReproductiveEvent.event_date >= birth.event_date - timedelta(days=300),
        ).first()
        if not prior_service:
            trig(
                AlertType.REPRODUCTION,
                f" Secuencia inconsistente: Parto registrado el {birth.event_date} sin inseminación o celo previo. ¿Monta natural no registrada?",
                AlertPriority.MEDIUM,
            )
    ...
    if last_preg:
        ...
        if 0 < days_gest <= 290:
            hitos = [
                ...
                (223, " INICIO DE SECADO obligatorio (60 días pre-parto)."),
