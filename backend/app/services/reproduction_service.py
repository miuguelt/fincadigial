...
class ReproductionService:
    ...
    def get_animal_history(animal_id: int, page: int = 1, limit: int = 50) -> dict:
        query = ReproductiveEvent.query.filter_by(animal_id=animal_id).order_by(
            ReproductiveEvent.event_date.desc()
        )
        pagination = query.paginate(page=page, per_page=limit, error_out=False)
        return {
            "items": [
                e.to_namespace_dict(include_relations=True) for e in pagination.items
            ],
            "page": page,
            "limit": limit,
    ...
    def get_summary(finca_id: int) -> dict:
        ...
        events_30d = ReproductiveEvent.query.filter(
            ReproductiveEvent.event_date >= last_30
        ).count()
        events_365d = ReproductiveEvent.query.filter(
            ReproductiveEvent.event_date >= last_365
        ).count()

        inseminations = ReproductiveEvent.query.filter_by(
        ...
        total_births = ReproductiveEvent.query.filter_by(
        ...
        conception_rate = (
            (positive_diag / inseminations * 100) if inseminations > 0 else 0
        )
    ...
    def get_pending_births(finca_id: int) -> list:
        ...
        events = (
            ...
            .order_by(ReproductiveEvent.expected_birth_date.asc())
            .all()
        )
        ...
        for ev in events:
            animal = ev.animal
            days_to_birth = (
                (ev.expected_birth_date - today).days
            ...
            result.append(
                {
                    ...
                    "expected_birth_date": ev.expected_birth_date.isoformat()
                    if ev.expected_birth_date
                    else None,
                    "days_to_birth": days_to_birth,
                    "status": "overdue"
                    if days_to_birth and days_to_birth < 0
                    else "upcoming",
                }
            )
        return result
    ...
    def get_heat_alerts(finca_id: int) -> list:
        ...
        for f in females:
            ...
            priority = (
                "Alta" if days_since > 60 else ("Media" if days_since > 30 else "Baja")
            )
            alerts.append(
                {
                    "animal_id": f.id,
    ...
    def get_calendar(start_date: str, end_date: str, finca_id: int) -> list:
        events = (
            ...
            .order_by(ReproductiveEvent.event_date.asc())
            .all()
        )
    ...
    def create_batch_events(
        data: dict, finca_id: int, actor_id: int | None = None
    ) -> list:
        animal_ids = data.get("animal_ids")
        if (
            animal_ids is None
            or not isinstance(animal_ids, list)
            or len(animal_ids) == 0
        ):
            from app.models.base_model import ValidationError
