...
class MilkProductionService:
    ...
    def _period_totals(*conditions) -> dict:
        ...
        row = (
            ...
            .filter(*conditions, MilkProduction.is_deleted == False)
            .one()
        )
    ...
    def get_weekly_summary(finca_id: int, start_date: Optional[date] = None) -> dict:
        ...
        session_stats = (
            ...
            .group_by(MilkProduction.milking_session)
            .all()
        )
    ...
    def get_monthly_summary(
        finca_id: int, year: Optional[int] = None, month: Optional[int] = None
    ) -> dict:
        ...
        weekly_stats = (
            ...
            .group_by(func.extract("week", MilkProduction.date))
            .order_by(func.extract("week", MilkProduction.date))
            .all()
        )
    ...
    def get_animal_trend(animal_id: int, finca_id: int, days: int = 30) -> dict:
        ...
        total_liters = (
            sum(row.total_liters for row in daily_stats) if daily_stats else 0
        )
        ...
        return {
            ...
            "session_averages": {
                str(row.milking_session): {
                    "avg_liters": round(float(row.avg_liters), 2)
                    if row.avg_liters
                    else 0,
                    "record_count": row.record_count,
                }
    ...
    def create_batch(date_str: str, finca_id: int, entries: list) -> dict:
            ...
            try:
                if "animal_id" not in entry or "liters" not in entry:
                    errors.append(
                        {
                            "index": i,
                            "error": "Faltan campos requeridos: animal_id, liters",
                        }
                    )
                    continue
                ...
                try:
                    ...
                    errors.append(
                        {
                            "index": i,
                            "error": f"Sesión inválida: {session_str}. Use AM, PM o Extra",
                        }
                    )
                    continue

                animal = Animals.query.filter_by(
                    id=entry["animal_id"], finca_id=finca_id
                ).first()
                if not animal:
                    errors.append(
                        {
                            "index": i,
                            "error": f"Animal {entry['animal_id']} no encontrado en esta finca",
                        }
                    )
                    continue
        ...
        if created:
            ...
            try:
                ...
                logger.warning(
                    f"No se pudo recalcular MilkSummary para finca {finca_id}: {e}"
                )
