...
def _vaccine_due_events(tenant_filter, start_date: date, end_date: date) -> list[dict]:
    ...
    pending = tenant_filter(Vaccinations.query, Vaccinations).filter(
...
def _withdrawal_end_events(tenant_filter, start_date: date, end_date: date) -> list[dict]:
    """Fin del periodo de retiro: hasta esa fecha no se comercializa."""
    ending = tenant_filter(Treatments.query, Treatments).filter(
...
def _task_events(tenant_filter, start_date: date, end_date: date) -> list[dict]:
    ...
    tasks = tenant_filter(Tasks.query, Tasks).filter(
