...
def evaluate_growth_rules(animal, finca_id, trig, today, age_months) -> int:
    ...
    if animal.father and animal.mother:
        if (
            animal.father.idFather == animal.mother.idFather and animal.father.idFather
        ) or (
            animal.father.idMother == animal.mother.idMother and animal.father.idMother
        ):
            trig(
                AlertType.CUSTOM,
        ...
        if (animal.father.id == animal.mother.idFather) or (
            animal.mother.id == animal.father.idMother
        ):
            trig(
                AlertType.CUSTOM,
    ...
    if animal.sex == Sex.Macho and 6 <= age_months <= 9:
        ...
        if not castrado:
            trig(
                AlertType.STATUS,
                f" Castración recomendada: macho de {age_months} meses (rango óptimo 6–9 meses).",
                AlertPriority.MEDIUM,
            )
    ...
    if len(all_controls) >= 1:
        ...
        if days_since_last > 120:
            trig(
                AlertType.GROWTH,
                f" Sin control de peso hace {days_since_last} días. Realizar pesaje para evaluar tendencia.",
                AlertPriority.HIGH,
            )
        elif days_since_last > 90:
            trig(
                AlertType.GROWTH,
                f" Último control de peso hace {days_since_last} días. Programar nuevo pesaje.",
                AlertPriority.MEDIUM,
            )
    ...
    if len(last_controls) >= 2:
        ...
        if w_actual and w_prev and w_prev > 0:
            ...
            if wl_severe is not None and diff_pct >= wl_severe:
                trig(
                    AlertType.GROWTH,
                    f" Pérdida severa de peso: -{diff_pct:.1f}% ({w_prev}→{w_actual} kg). Revisar urgente.",
                    AlertPriority.CRITICAL,
                )
            elif wl_high is not None and diff_pct >= wl_high:
                trig(
                    AlertType.GROWTH,
                    f" Pérdida de peso detectada: -{diff_pct:.1f}% ({w_prev}→{w_actual} kg). Revisar nutrición.",
                    AlertPriority.HIGH,
                )
    ...
    if active_af:
        ...
        try:
            ...
            if m:
                cap = int(m.group(1))
                current = (
                    AnimalFields.query.join(Animals)
                    .filter(
                        AnimalFields.field_id == active_af.field.id,
...
def _check_stale_weight(animal, today, age_months, trig):
    ...
    if not last_ctrl_peso and age_months >= 3:
        trig(
            AlertType.GROWTH,
            " Sin registro de peso en historial clínico. Pesar en próximo control veterinario.",
            AlertPriority.MEDIUM,
        )
    elif last_ctrl_peso:
        ...
        if days_peso > 180:
            trig(
                AlertType.GROWTH,
                f" Último peso registrado hace {days_peso} días. Realizar control de peso cada 60-90 días.",
                AlertPriority.MEDIUM,
            )


def _check_breed_standard(animal, all_controls, age_months, trig):
    expected_w, min_w, expected_adg = BreedGrowthStandard.get_expected_weight(
    ...
    if expected_w and all_controls[-1].weight:
        ...
        if deviation_pct < -25:
            trig(
                AlertType.GROWTH,
                f" Peso CRÍTICO vs raza: {actual_w} kg (esperado {expected_w} kg, -{abs(deviation_pct):.0f}%). Muy por debajo del estándar.",
                AlertPriority.CRITICAL,
            )
        elif deviation_pct < -15:
            trig(
                AlertType.GROWTH,
                f" Peso bajo vs raza: {actual_w} kg (esperado {expected_w} kg, -{abs(deviation_pct):.0f}%). Revisar nutrición y sanidad.",
                AlertPriority.HIGH,
            )
        elif deviation_pct < -10:
            trig(
                AlertType.GROWTH,
                f" Peso ligeramente bajo vs raza: {actual_w} kg (esperado {expected_w} kg, -{abs(deviation_pct):.0f}%). Monitorear.",
                AlertPriority.MEDIUM,
            )
    ...
    if len(all_controls) >= 3:
        ...
        if len(weights) >= 3 and len(dates) >= 3:
            ...
            if total_days > 0:
                ...
                _, _, expected_adg = (
                    BreedGrowthStandard.get_expected_weight(
                        animal.breeds_id, animal.sex.value, animal.age_in_months or 0
                    )
                    if animal.breeds_id
                    else (None, None, None)
                )
                ...
                if actual_adg < 0 and abs(actual_adg) > 0.05:
                    trig(
                        AlertType.GROWTH,
                        f" ADG NEGATIVO: {actual_adg:.3f} kg/día. El animal está perdiendo peso sostenidamente.",
                        AlertPriority.CRITICAL,
                    )
                elif actual_adg < adjusted_expected_adg * 0.4:
                    trig(
                        AlertType.GROWTH,
                        f" ADG muy bajo: {actual_adg:.3f} kg/día (esperado ≥{adjusted_expected_adg:.3f}). Rendimiento <40% del esperado.",
                        AlertPriority.HIGH,
                    )
                elif actual_adg < adjusted_expected_adg * 0.6:
                    trig(
                        AlertType.GROWTH,
                        f" ADG bajo: {actual_adg:.3f} kg/día (esperado ≥{adjusted_expected_adg:.3f}). Rendimiento <60% del esperado.",
                        AlertPriority.MEDIUM,
                    )
        ...
        if len(weights_proj) >= 2:
            ...
            if days_obs > 0:
                ...
                projected_weight_12m = (
                    weights_proj[-1] + current_adg * (12 - current_age) * 30.44
                )
                if current_age < 12 and projected_weight_12m > 0:
                    _, min_12m, _ = (
                        BreedGrowthStandard.get_expected_weight(
                            animal.breeds_id, animal.sex.value, 12
                        )
                        if animal.breeds_id
                        else (None, None, None)
                    )
                    if min_12m and projected_weight_12m < min_12m * 0.8:
                        trig(
                            AlertType.GROWTH,
                            f" Proyección a 12 meses: {projected_weight_12m:.0f} kg (mínimo esperado {min_12m:.0f} kg). Riesgo de bajo rendimiento.",
                            AlertPriority.HIGH,
                        )
...
def _check_bcs_low(animal, trig):
    ...
    if latest_bcs:
        for key, prio in [
            ("bcs_critical", AlertPriority.CRITICAL),
            ("bcs_high", AlertPriority.HIGH),
            ("bcs_medium", AlertPriority.MEDIUM),
            ("bcs_obese", AlertPriority.MEDIUM),
        ]:
            ...
            if val is not None:
                ...
                if key == "bcs_obese":
                    if latest_bcs.score >= threshold:
                        trig(
                            AlertType.GROWTH,
                            f" BCS obeso: {latest_bcs.score}/9 ({latest_bcs.category}). Riesgo de problemas metabólicos y reproductivos.",
                            prio,
                        )
                    ...
                    label = (
                        ...
                        else "por debajo de ideal"
                    )
                    trig(
                        AlertType.GROWTH,
                        f" BCS {label}: {latest_bcs.score}/9 ({latest_bcs.category}). {'Riesgo de muerte por emaciación. Intervención urgente.' if prio == AlertPriority.CRITICAL else 'Incrementar suplementación y revisar sanidad.' if prio == AlertPriority.HIGH else 'Monitorear tendencia.'}",
                        prio,
                    )
    ...
    if BodyConditionScore.get_latest(animal.id):
        ...
        if len(bcs_trend) >= 2:
            ...
            if bcs_drop >= 1.5:
                trig(
                    AlertType.GROWTH,
                    f" BCS en caída: {bcs_trend[-1].score}→{bcs_trend[0].score} en 90 días (-{bcs_drop:.1f} puntos). Revisar nutrición urgente.",
                    AlertPriority.HIGH,
                )
