...
def run_core_initialization():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"No se pudo consultar fincas (posible migración pendiente): {e}")
        ...
        try:
            default_user = User.query.filter(or_(User.email == "instructor@finca.com", User.identification == 11111111)).first()
            if not default_user:
                default_user = User(
            ...
            admin_user = User.query.filter(or_(User.email == "admin@villaluz.com", User.identification == 12345678)).first()
            if not admin_user:
                admin_user = User(
            ...
            op_user = User.query.filter(or_(User.email == "op1@villaluz.com", User.identification == 11112222)).first()
            if not op_user:
                op_user = User(
            ...
            logger.warning(f"Error al inicializar usuarios (posible migración de tabla user pendiente): {e}")
