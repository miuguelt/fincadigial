...
class ICAReportPDF(FPDF):
    def __init__(self, finca_name, report_type):
        super().__init__()
    ...
    def header(self):
        # Gradiente simulado (franjas de verde)
        self.set_fill_color(22, 78, 38) # Verde muy oscuro
        self.rect(0, 0, 210, 40, 'F')
        
        self.set_fill_color(34, 110, 50) # Verde medio
        self.rect(0, 35, 210, 5, 'F')
        
        # Logo placeholder (Textual por ahora)
        self.set_xy(10, 10)
        ...
        self.cell(60, 10, 'REPORTE OFICIAL ICA', align='R', ln=True)
        self.set_xy(140, 22)
        self.set_font('Helvetica', '', 8)
        self.cell(60, 5, f'Generado: {datetime.now().strftime("%d/%m/%Y %H:%M")}', align='R')
    ...
    def footer(self):
        ...
        self.cell(0, 5, f'Finca: {self.finca_name} | Documento verificado por DevBrain AI Core', align='C', ln=True)
    ...
    def draw_styled_table(self, headers, data, col_widths):
        ...
        for row in data:
            ...
            self.set_fill_color(250, 252, 250) if not fill else self.set_fill_color(255, 255, 255)
            row_height = 8
            ...
            for i, header in enumerate(headers):
                ...
                if len(val) > 28:
                    val = val[:25] + '...'
                self.cell(col_widths[i], row_height, val, border='B', fill=True, align='C')
            self.ln()
            fill = not fill
...
def generate_inventory_pdf(finca_name, finca_data, animals_data):
    pdf = ICAReportPDF(finca_name, 'Inventario General de Ganado')
    pdf.alias_nb_pages()
    pdf.add_page()
    pdf.add_finca_section(finca_data)
    
    headers = ['Numero Arete', 'Especie', 'Raza', 'Sexo', 'Edad Meses', 'Peso Kg', 'Estado']
...
def generate_movements_pdf(finca_name, finca_data, movements_data):
    pdf = ICAReportPDF(finca_name, 'Registro de Movimientos (Trazabilidad)')
    pdf.alias_nb_pages()
    pdf.add_page()
    pdf.add_finca_section(finca_data)
    
    headers = ['Fecha', 'Tipo Movimiento', 'Numero Arete', 'Sexo', 'Detalle', 'Destino Origen']
...
def generate_health_pdf(finca_name, finca_data, health_data):
    pdf = ICAReportPDF(finca_name, 'Historial Sanitario y Vacunación')
    pdf.alias_nb_pages()
    pdf.add_page()
    pdf.add_finca_section(finca_data)
    
    headers = ['Fecha', 'Tipo Registro', 'Numero Arete', 'Producto', 'Dosis', 'Veterinario']
...
def generate_animal_cv_pdf(finca_name, animal_data, kpis, reproductive_history):
    ...
    pdf = ICAReportPDF(finca_name, f'Hoja de Vida: {animal_data.get("record", "Animal")}')
    pdf.alias_nb_pages()
    pdf.add_page()
    ...
    fields = [
        ...
        ('Frame Score:', str(kpis.get('frame_score', 'N/A'))),
    ...
    pdf.multi_cell(0, 5, 'Estos indicadores son calculados proactivamente para medir la eficiencia del animal en la finca.')
    pdf.ln(5)
    
    intel_headers = ['Métrica', 'Valor', 'Estado/Alerta']
    intel_data = [
        {'métrica': 'Días Abiertos', 'valor': str(kpis.get('open_days', 0)), 'estado/alerta': 'Normal' if kpis.get('open_days', 0) < 90 else 'Crítico'},
        {'métrica': 'Intervalo Partos', 'valor': str(kpis.get('calving_interval', 0)) + ' días', 'estado/alerta': '-'},
        {'métrica': 'Días en Leche', 'valor': str(kpis.get('days_in_milk', 0)), 'estado/alerta': 'Producción Activa' if kpis.get('days_in_milk', 0) > 0 else '-'},
        {'métrica': 'Tiempo Retiro', 'valor': str(kpis.get('withdrawal_remaining', 0)) + ' días', 'estado/alerta': 'CON RESTRICCIÓN' if kpis.get('is_withdrawing') else 'Apto Consumo'}
    ]
...
def generate_financial_statement_pdf(finca_name, financial_data):
    ...
    pdf.cell(95, 10, f'Total Gastos: $ {financial_data.get("total_expenses", 0):,.2f}', ln=True)
    pdf.ln(5)
