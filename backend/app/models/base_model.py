...
class BaseModel(db.Model):
    ...
    created_at = db.Column(
        db.DateTime,
        default=lambda: datetime.now(timezone.utc),              # client-side default
    ...
    updated_at = db.Column(
        db.DateTime,
        default=lambda: datetime.now(timezone.utc),              # client-side default
        onupdate=lambda: datetime.now(timezone.utc),             # client-side onupdate
        server_default=db.func.now(),         # server-side default
        nullable=False
    )
    ...
    def _validate_and_normalize(cls, data, is_update=False, instance_id=None):
        ...
        if cls.__name__ in TENANT_MODELS and hasattr(cls, 'finca_id'):
            # Si no viene en los datos, intentar recuperarlo del contexto JWT
            if data.get('finca_id') is None:
                f_id = get_current_finca_id()
                if f_id:
                    data['finca_id'] = f_id
                elif not is_update:
                    # En creación es mandatorio tener un contexto de finca
                    # EXCEPCIÓN: Si estamos en modo admin (ej: background tasks globales)
                    import flask
                    is_admin = flask.has_app_context() and getattr(flask.g, 'is_admin', False)
    ...
    def get_namespace_query(cls, filters=None, search=None, search_type='auto', sort_by=None, sort_order='asc',
        ...
        # Esto protege automáticamente todos los namespaces
        from app.utils.tenant_context import TENANT_MODELS, get_current_finca_id
        if cls.__name__ in TENANT_MODELS:
            finca_id = get_current_finca_id()
            if finca_id and hasattr(cls, 'finca_id'):
                query = query.filter(cls.finca_id == finca_id)
                logger.debug(f"Tenant filter applied: {cls.__name__}.finca_id == {finca_id}")
