...
class AnimalGroup(BaseModel):
    ...
    animals = db.relationship(
        "Animals",
        secondary="animal_group_membership",
...
class AnimalGroupMembership(db.Model):
    ...
    group_id = db.Column(
        db.Integer, db.ForeignKey("animal_groups.id"), primary_key=True
    )
...
class Infrastructure(BaseModel):
    ...
    status = db.Column(
        db.String(50), default="Operativo"
    )  # Operativo, Requiere Arreglo, Crítico
    finca_id = db.Column(
        db.Integer, db.ForeignKey("finca.id"), nullable=False, index=True
    )
