...
class LivestockSummary(BaseModel):
    ...
    last_recalculation = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    ...
    def recalculate(self):
        ...
        # Conteos básicos
        self.total_animals = Animals.query.filter_by(finca_id=self.finca_id).count()
        self.active_animals = Animals.query.filter_by(finca_id=self.finca_id, status=AnimalStatus.Vivo).count()
        self.sold_animals = Animals.query.filter_by(finca_id=self.finca_id, status=AnimalStatus.Vendido).count()
        self.dead_animals = Animals.query.filter_by(finca_id=self.finca_id, status=AnimalStatus.Muerto).count()

        self.male_count = Animals.query.filter_by(finca_id=self.finca_id, sex=Sex.Macho).count()
        self.female_count = Animals.query.filter_by(finca_id=self.finca_id, sex=Sex.Hembra).count()

        # Animales enfermos (con tratamientos activos en los últimos 30 días)
        thirty_days_ago = datetime.now() - timedelta(days=30)
        self.sick_animals = db.session.query(db.func.count(db.func.distinct(Treatments.animal_id))).filter(
            Treatments.finca_id == self.finca_id,
            Treatments.treatment_date >= thirty_days_ago
        ).scalar() or 0

        self.last_recalculation = datetime.now(timezone.utc)
        db.session.commit()
