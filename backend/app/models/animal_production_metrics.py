...
class AnimalProductionMetrics(BaseModel):
    ...
    animal = db.relationship(
        "Animals", back_populates="production_metrics", lazy="selectin"
    )
    finca = db.relationship(
        "Finca", backref="production_metrics_records", lazy="selectin"
    )
    ...
    def get_average(cls, finca_id, metric_type, days=30):
        ...
        result = (
            db.session.query(func.avg(cls.value))
            .filter(
