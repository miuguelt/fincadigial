...
def build_alert_dedupe_key(
    *,
    ...
    condition = (
        category.strip().casefold() if category else normalize_alert_condition(message)
    )
    source = "|".join(
        (
            str(finca_id or 0),
...
class AnimalAlertConfig(BaseModel):
    ...
    __table_args__ = (db.Index("ix_animal_alert_configs_finca_id", "finca_id"),)

    id = db.Column(db.Integer, primary_key=True)
    animal_id = db.Column(
        db.Integer, db.ForeignKey("animals.id"), nullable=True
    )  # None = global de finca
    alert_type = db.Column(
        db.Enum(AlertType, values_callable=_enum_values), nullable=False
    )
    dimension = db.Column(db.String(50), nullable=False)
    condition_value = db.Column(db.String(255), nullable=False)
    message = db.Column(db.Text, nullable=False)
    priority = db.Column(
        db.Enum(AlertPriority, values_callable=_enum_values), default=AlertPriority.HIGH
    )
...
class AnimalAlert(BaseModel):
    ...
    field_id = db.Column(
        db.Integer, db.ForeignKey("fields.id"), nullable=True
    )  # Nuevo: Vínculo con potrero
    config_id = db.Column(
        db.Integer, db.ForeignKey("animal_alert_configs.id"), nullable=True
    )
    alert_type = db.Column(
        db.Enum(AlertType, values_callable=_enum_values), nullable=False
    )
    ...
    )  # Campo para sugerencias de IA (offline base)
    priority = db.Column(
        db.Enum(AlertPriority, values_callable=_enum_values),
    ...
    config = db.relationship(
        "AnimalAlertConfig", backref=db.backref("triggered_alerts", lazy="dynamic")
    )
