...
class Animals(BaseModel):
    ...
    _filterable_fields = ['sex', 'status', 'finca_id', 'breeds_id', 'birth_date', 'weight', 'entry_date', 'created_at', 'idFather', 'idMother']
    ...
    def create(cls, commit=True, **kwargs):
        """Sobreescribe create para disparar la actualización incremental de estadísticas."""
    ...
    def current_field_name(self):
        ...
        try:
            ...
            active_assignment = self.animal_fields.filter_by(removal_date=None).first()
    ...
    def health_indicator(self):
        ...
        if self.status == AnimalStatus.Muerto: return 'critical'
        
        # Último control de salud
        last_control = self.controls.first()
        if last_control and last_control.health_status.value == 'Enfermo':
            return 'critical'
            
        # Vacunación vencida (> 6 meses)
        last_vacc = self.vaccinations.order_by(db.desc('vaccination_date')).first()
        if last_vacc and (today - last_vacc.vaccination_date).days > 180:
            return 'critical'
        ...
        if self.is_pregnant and self.last_calving_date:
            # Asumiendo ciclo de ~283 días para bovinos
            gestation_days = 283
    ...
    def to_ai_context(self, include_relations=True, depth=1):
        ...
        try:
            active_af = self.animal_fields.filter_by(removal_date=None).first()
            if active_af:
