...
class Animals(BaseModel):
    ...
    record = db.Column(db.String(255), nullable=False)  # unique=True removido - ahora es compuesto con finca_id
    qr_code = db.Column(db.String(100), unique=True, nullable=True) # Para escaneo en corral / NFC
    ...
    _namespace_fields = [
        'id', 'record', 'qr_code', 'sex', 'birth_date', 'weight', 'status', 'finca_id', 'breeds_id',
        'idFather', 'idMother', 'idFatherFather', 'idFatherMother', 'idMotherFather', 'idMotherMother',
        'is_pregnant', 'is_lactating', 'last_calving_date',
        'entry_date', 'purchase_date', 'sale_date', 'exit_date',
        'exit_reason', 'pending_alerts_count', 'created_at', 'updated_at'
    ]
    ...
    _filterable_fields = ['id', 'sex', 'status', 'breeds_id', 'birth_date', 'weight', 'created_at', 'idFather', 'idMother']
    ...
    alerts = db.relationship('AnimalAlert', back_populates='animal', lazy='dynamic', cascade='all, delete-orphan')
    alert_configs = db.relationship('AnimalAlertConfig', back_populates='animal', lazy='dynamic', cascade='all, delete-orphan')
    health_history = db.relationship('AnimalHealthHistory', back_populates='animal', lazy='dynamic', cascade='all, delete-orphan')
    production_metrics = db.relationship('AnimalProductionMetrics', back_populates='animal', lazy='dynamic', cascade='all, delete-orphan')
    ...
    def create(cls, commit=True, **kwargs):
        ...
        if instance and instance.finca_id:
            ...
            summary.handle_animal_event('create', animal_data={'status': instance.status, 'sex': instance.sex})
            db.session.commit()
        return instance
    ...
    def restore(self, commit=True):
        ...
        if self.finca_id:
            try:
                from app.models.livestock_summary import LivestockSummary
                summary = LivestockSummary.get_for_finca(self.finca_id)
                summary.handle_animal_event('create', animal_data={'status': self.status, 'sex': self.sex})
                if commit:
                    db.session.commit()
            except Exception as e:
                logger.error(f"Error actualizando resumen incremental tras restore: {e}")
        return result
        ...
        for date_field in ['entry_date', 'purchase_date', 'sale_date', 'exit_date']:
            if date_field in data and data[date_field]:
                if data[date_field] > date.today():
                    raise ValidationError(f"La fecha de {date_field.replace('_', ' ')} no puede ser futura")
    ...
    def max_pending_priority(self):
        ...
        if hasattr(self, '_prefetched_alerts'):
            ...
            sorted_alerts = sorted(self._prefetched_alerts, key=get_weight, reverse=True)
    ...
    def current_field_name(self):
        ...
        try:
            if hasattr(self, '_prefetched_active_field'):
                ...
                active_assignment = self.animal_fields.filter_by(removal_date=None, is_deleted=False).first()
    ...
    def health_indicator(self):
        ...
        if self.status == AnimalStatus.Muerto or str(self.status) == 'AnimalStatus.Muerto' or str(self.status) == 'Muerto':
            return 'critical'
        ...
        if last_control:
            hs = getattr(last_control.health_status, 'value', last_control.health_status)
            if hs == 'Enfermo':
                return 'critical'
        ...
        if self.is_pregnant and self.last_calving_date:
            ...
            gestation_days = int(float(gest_entry.content)) if (gest_entry and gest_entry.content) else 283
    ...
    def to_namespace_dict(self, include_relations=False, depth=1, fields=None):
        ...
        def _safe_get(prop_name, default=None):
            try:
                ...
                logger.debug(f"Error accediendo a propiedad {prop_name} en Animal {self.id}: {e}")
                return default
    ...
    def get_namespace_query(cls, filters=None, search=None, search_type='auto', sort_by=None, sort_order='asc',
        ...
        query = super().get_namespace_query(filters, search, search_type, sort_by, sort_order, None, None, include_relations)
        ...
        if 'destetar' in custom_filters and str(custom_filters['destetar']).lower() in ('true', '1', 'yes'):
            from datetime import date, timedelta
    ...
    def to_ai_context(self, include_relations=True, depth=1):
        ...
        recent_diseases = []
        try:
            recent_diseases = [d.disease.name for d in self.diseases.limit(3).all() if d.disease]
        except Exception:
            pass
        ...
        recent_vaccines = []
        try:
            recent_vaccines = [f"{v.vaccine.name} ({v.vaccination_date})" for v in self.vaccinations.order_by(db.desc('vaccination_date')).limit(3).all() if v.vaccine]
        except Exception:
            pass
        ...
        recent_treatments = []
        try:
            recent_treatments = [f"{t.description} ({t.treatment_date})" for t in self.treatments.order_by(db.desc('treatment_date')).limit(3).all()]
        except Exception:
            pass
        ...
        try:
            active_af = self.animal_fields.filter_by(removal_date=None, is_deleted=False).first()
            if active_af:
        ...
        vaccines_text = ", ".join(recent_vaccines) if recent_vaccines else "Sin registros recientes"
        treatments_text = ", ".join(recent_treatments) if recent_treatments else "Sin registros recientes"
...
"""
    ...
    @classmethod
    def batch_weight(cls, animal_ids, weight, checkup_date=None, health_status='Sano', notes=None, finca_id=None):
        ...
        if finca_id:
            owned = cls.query.filter(cls.id.in_(animal_ids), cls.finca_id == finca_id).all()
    ...
    @classmethod
    def batch_vaccinate(cls, animal_ids, vaccine_id, vaccination_date=None, dosis=None, batch_number=None, next_due_date=None, notes=None, finca_id=None, performed_by=None):
