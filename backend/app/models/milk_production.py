...
class MilkProduction(BaseModel):
    ...
    __table_args__ = (
        ...
        db.UniqueConstraint(
            ...
            "date",
            "milking_session",
            name="uq_milk_production_animal_date_session",
        ),
    )
    ...
    milking_session = db.Column(
        db.Enum(MilkSession), nullable=False, default=MilkSession.AM
    )

    # Calidad (opcional)
    fat_percentage = db.Column(db.Float, nullable=True)
    protein_percentage = db.Column(db.Float, nullable=True)
    somatic_cells = db.Column(
        db.Integer, nullable=True
    )  # Células somáticas (indicador de mastitis)
    ...
    _namespace_relations = {"control": {"fields": ["id", "health_status"]}}
    _filterable_fields = [
        "animal_id",
        "finca_id",
        "control_id",
    ...
    def _validate_and_normalize(cls, data, is_update=False, instance_id=None):
        ...
        if "liters" in data:
            ...
            if liters < cls.MIN_LITERS or liters > cls.MAX_LITERS:
                errors.append(
                    f"Litros debe estar entre {cls.MIN_LITERS} y {cls.MAX_LITERS}"
                )

        # Validar células somáticas
        if "somatic_cells" in data and data["somatic_cells"] is not None:
            if (
                data["somatic_cells"] < 0
                or data["somatic_cells"] > cls.MAX_SOMATIC_CELLS
            ):
                errors.append(
                    f"Células somáticas debe estar entre 0 y {cls.MAX_SOMATIC_CELLS}"
                )
