...
class BodyConditionScore(BaseModel):
    ...
    # Relaciones
    animal = db.relationship('Animals', backref='body_condition_scores', lazy='selectin')
    ...
    _namespace_fields = [
        'id', 'animal_id', 'finca_id', 'score_date', 'score',
        'evaluator_id', 'notes', 'created_at', 'updated_at'
    ]
    ...
    def get_latest(cls, animal_id):
        ...
        return cls.query.filter_by(animal_id=animal_id).order_by(
            cls.score_date.desc()
        ).first()
    ...
    def get_trend(cls, animal_id, days=90):
        ...
        return cls.query.filter(
            ...
            cls.score_date >= cutoff
        ).order_by(cls.score_date).all()
    ...
    def get_herd_average(cls, finca_id, days=30):
        ...
        cutoff = date.today() - timedelta(days=days)
        result = db.session.query(func.avg(cls.score)).filter(
