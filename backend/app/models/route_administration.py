...
class RouteAdministration(BaseModel):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "name", "finca_id", name="uq_route_administrations_name_finca"
        ),
    )
    ...
    _filterable_fields = ["status", "finca_id", "created_at"]
    ...
    medications = db.relationship(
        "Medications", back_populates="route_administration_rel", lazy="dynamic"
    )
    vaccines = db.relationship(
        "Vaccines", back_populates="route_administration_rel", lazy="dynamic"
    )
