...
class BreedGrowthStandard(BaseModel):
    ...
    expected_adg_kg = db.Column(
        db.Float, nullable=False
    )  # Ganancia diaria esperada (kg/día)
    ...
    def get_expected_weight(cls, breed_id, sex, age_months):
        ...
        standards = (
            cls.query.filter_by(breed_id=breed_id, sex=sex)
            .order_by(cls.age_months)
            .all()
        )
        ...
        expected = lower.expected_weight_kg + t * (
            upper.expected_weight_kg - lower.expected_weight_kg
        )
        min_w = lower.min_weight_kg + t * (upper.min_weight_kg - lower.min_weight_kg)
        adg = lower.expected_adg_kg + t * (
            upper.expected_adg_kg - lower.expected_adg_kg
        )
