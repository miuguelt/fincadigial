...
class JoinRequest(db.Model):
    ...
    request_type = db.Column(
        db.Enum(JoinRequestType), default=JoinRequestType.REQUEST, nullable=False
    )
    status = db.Column(
        db.Enum(JoinRequestStatus), default=JoinRequestStatus.PENDING, nullable=False
    )
    ...
    def create_invitation(
        ...
        db.session.commit()
        logger.info(
            f"Invitation created: id={req.id} token_hash={req.token_hash[:8]}..."
        )
        return req, token
    ...
    def find_by_token(cls, token: str) -> "JoinRequest | None":
        ...
        return cls.query.filter_by(
            token_hash=token_hash, status=JoinRequestStatus.PENDING
        ).first()
    ...
    def to_dict(self):
        return {
            ...
            "notes": self.notes,
            "invitation_method": self.invitation_method.value
            if self.invitation_method
            else None,
