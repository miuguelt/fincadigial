...
class AnimalMovement(BaseModel):
    ...
    rpp_destino_externo = db.Column(db.String(12), nullable=True)  # ICA RPP is a 12-digit code
    ...
    guia_movilizacion = db.Column(db.String(50), nullable=True)  # GSMI ICA (mandatory for transit)
    ...
    # Relationships
    animal = db.relationship('Animals', foreign_keys=[animal_id], backref=db.backref('movements', cascade='all, delete-orphan'), lazy='selectin')
    finca_origen = db.relationship('Finca', foreign_keys=[finca_origen_id], backref='movements_sent', lazy='selectin')
    finca_destino = db.relationship('Finca', foreign_keys=[finca_destino_id], backref='movements_received', lazy='selectin')

    _namespace_fields = [
        'id', 'animal_id', 'finca_origen_id', 'finca_destino_id',
        'finca_destino_externa', 'rpp_destino_externo', 'tipo_movimiento', 'fecha_movimiento',
        'precio_venta', 'comprador', 'comprador_nit',
        'arete_sinigan', 'guia_movilizacion', 'ruv_vacunacion',
        'placa_vehiculo', 'nombre_conductor', 'cedula_conductor', 'precinto_seguridad',
    ...
    _filterable_fields = ['animal_id', 'finca_origen_id', 'finca_destino_id', 'tipo_movimiento', 'fecha_movimiento']
    _searchable_fields = ['comprador', 'guia_movilizacion', 'placa_vehiculo', 'nombre_conductor', 'notes']
