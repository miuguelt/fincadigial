...
class Medications(BaseModel):
    ...
    contraindications = db.Column(
        db.String(255), nullable=True
    )  # Opcional, nullable for tests
    ...
    treatments = db.relationship(
        "TreatmentMedications", back_populates="medications", lazy="dynamic"
    )
    # OPTIMIZED: Changed from lazy='select' to lazy='selectin' to prevent N+1 queries
    route_administration_rel = db.relationship(
        "RouteAdministration", back_populates="medications", lazy="selectin"
    )
