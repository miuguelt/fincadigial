...
class ProfessionalCredential(BaseModel):
    ...
    status = db.Column(
        db.Enum(CredentialStatus),
        nullable=False,
        default=CredentialStatus.Autodeclarado,
    )
    ...
    def effective_status(self) -> CredentialStatus:
        ...
        if self.status == CredentialStatus.Verificado:
            if (
                ...
                and self.verification_expires_at < date.today()
            ):
    ...
    def mark_verified(
        self, verifier_id: int, source: str, reference: str, notes: str | None = None
    ):
    ...
    def __repr__(self):
        return (
            f"<ProfessionalCredential User={self.user_id} Status={self.status.value}>"
        )
