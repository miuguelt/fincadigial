...
class Vaccines(BaseModel):
    ...
    route_administration_id = db.Column(
        db.Integer, db.ForeignKey("route_administrations.id"), nullable=False
    )
    ...
    target_disease_id = db.Column(
        db.Integer, db.ForeignKey("diseases.id"), nullable=False
    )
    ...
    treatments = db.relationship(
        "TreatmentVaccines", back_populates="vaccines", lazy="dynamic"
    )
    vaccinations = db.relationship(
        "Vaccinations", back_populates="vaccines", lazy="dynamic"
    )
    # OPTIMIZED: Changed from lazy='select' to lazy='selectin' to prevent N+1 queries
    route_administration_rel = db.relationship(
        "RouteAdministration", back_populates="vaccines", lazy="selectin"
    )
