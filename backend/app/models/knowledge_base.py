...
class KBRegla(db.Model):
    ...
    recomendacion_id = db.Column(
        db.Integer, db.ForeignKey("kb_recomendaciones.id"), nullable=False
    )
...
class KBCalendario(db.Model):
    ...
    )  # vacunacion, desparasitacion, revision
    ...
    frecuencia_dias = db.Column(
        db.Integer, nullable=True
    )  # Cada X días (0 = única vez)
    producto_sugerido = db.Column(
