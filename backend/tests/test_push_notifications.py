...
class TestPushSubscriptionModel(unittest.TestCase):
    ...
    def test_duplicate_endpoint_updates_existing(self):
        ...
        with self.app.app_context():
            ...
            sub1 = PushSubscription.create(
                user_id=self.user.id, endpoint=endpoint, p256dh="KEY1", auth="AUTH1"
            )

            # Crear segunda con mismo endpoint
            sub2 = PushSubscription.create(
                user_id=self.user.id, endpoint=endpoint, p256dh="KEY2", auth="AUTH2"
            )
    ...
    def test_deactivate_by_endpoint(self):
        ...
        with self.app.app_context():
            ...
            PushSubscription.create(
                user_id=self.user.id, endpoint=endpoint, p256dh="KEY", auth="AUTH"
            )
    ...
    def test_deactivate_by_user(self):
        ...
        with self.app.app_context():
            ...
            active = PushSubscription.get_user_subscriptions(
                self.user.id, active_only=True
            )
...
class TestPushNotificationEndpoints(unittest.TestCase):
    ...
    def test_subscribe_with_invalid_data(self):
        ...
        response = self.client.post(
            "/api/v1/push/subscribe",
            data=json.dumps(
                {"subscription": {"keys": {"p256dh": "BIPUL...", "auth": "aBCd..."}}}
            ),
            headers={"Authorization": f"Bearer {token}"},
            content_type="application/json",
        )
    ...
    def test_get_subscriptions(self):
        ...
        response = self.client.get(
            "/api/v1/push/subscriptions", headers={"Authorization": f"Bearer {token}"}
        )
