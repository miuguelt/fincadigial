...
def test_ai_insights_general(client, auth_headers):
    ...
    with patch("app.services.cortex_service.CortexService.call_claude") as mock_claude:
        ...
        response = client.get("/api/v1/analytics/ai-insights", headers=auth_headers)
...
def test_weight_prediction(client, auth_headers):
    ...
    with patch(
        "app.services.analytics.prediction_service.PredictionService.predict_animal_weight"
    ) as mock_predict:
        mock_predict.return_value = {"predicted_weight": 460.5}

        response = client.get(
            "/api/v1/analytics/predictions/weight/1", headers=auth_headers
        )
...
def test_run_predictive_analysis(client, auth_headers):
    ...
    with patch(
        "app.tasks.predictive_tasks.run_finca_predictive_analysis.delay"
    ) as mock_task:
        mock_task.return_value = MagicMock(id="test-task-id")

        response = client.post("/api/v1/analytics/predictive/run", headers=auth_headers)
...
def test_intelligence_summary(client, auth_headers):
    ...
    with patch(
        "app.services.predictive_engine_service.PredictiveEngineService.get_finca_insights_summary"
    ) as mock_summary:
        mock_summary.return_value = "Resumen ok"

        response = client.get(
            "/api/v1/analytics/predictive/insights", headers=auth_headers
        )
...
def test_campesino_tips(client, auth_headers):
    """Prueba el endpoint de tips rotativos para el dashboard campesino"""
    response = client.get("/api/v1/intelligence/tips", headers=auth_headers)
    assert response.status_code == 200
    data = response.get_json()
