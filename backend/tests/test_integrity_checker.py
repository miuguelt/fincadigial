...
class TestIntegrityChecker:
    ...
    def test_check_integrity_with_dependencies(self, app, db_session):
        with app.app_context():
            ...
            can_delete, warnings = OptimizedIntegrityChecker.can_delete_safely(Finca, finca.id)
            
            # Debería haber al menos una advertencia para la tabla 'animals'
            assert len(warnings) > 0
            
            # Encontrar la advertencia de 'animals'
            animal_warning = next((w for w in warnings if w.dependent_table == 'animals'), None)
            ...
            warnings_cached = OptimizedIntegrityChecker.check_integrity_fast(Finca, finca.id)
    ...
    def test_get_batch_dependencies(self, app, db_session):
        with app.app_context():
            ...
            batch_results = OptimizedIntegrityChecker.get_batch_dependencies([animal.id], 'animals')
            ...
            control_dep = next((d for d in animal_deps if d['table'] == 'control'), None)
    ...
    def test_get_batch_dependencies_invalid_model(self):
        ...
        res = OptimizedIntegrityChecker.get_batch_dependencies([1], 'non_existent_model')
        assert res == {1: []}
