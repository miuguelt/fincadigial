...
def _finca():
    ...
    if not finca:
        finca = Finca.create(name='Finca Test Acreditación', type=FarmType.Tradicional, is_active=True)
        db.session.commit()
    return finca
...
def vet(app, db_session):
    with app.app_context():
        ...
        return {'user_id': user.id, 'headers': _headers(user, finca), 'finca_id': finca.id}
...
class TestSelfService:
    ...
    def test_rol_no_veterinario_no_puede_acreditarse(self, client, operario):
        response = client.put(f'{BASE}/me', headers=operario['headers'], json=VALID_PAYLOAD)
    ...
    def test_supresion_elimina_la_credencial(self, client, vet):
        ...
        assert client.get(f'{BASE}/me', headers=vet['headers']).get_json()['data'] is None
...
class TestVerification:
    def test_admin_verifica_y_deja_rastro(self, client, app, vet, admin):
        ...
        with app.app_context():
            credential_id = ProfessionalCredential.query.filter_by(
                user_id=vet['user_id']
            ).first().id
    ...
    def test_verificar_sin_referencia_rechaza(self, client, app, vet, admin):
        ...
        response = client.post(f'{BASE}/{credential_id}/verify', headers=admin['headers'], json={})
        assert response.status_code in (400, 422)
    ...
    def test_editar_un_campo_clave_reinicia_la_revision(self, client, app, vet, admin):
        ...
        assert response.status_code == 200
        assert response.get_json()['data']['status'] == CredentialStatus.EnRevision.value

    def test_editar_un_campo_accesorio_conserva_la_verificacion(self, client, app, vet, admin):
        client.put(f'{BASE}/me', headers=vet['headers'], json=VALID_PAYLOAD)
        with app.app_context():
            credential_id = ProfessionalCredential.query.filter_by(
                user_id=vet['user_id']
        ...
        assert response.status_code == 200
        assert response.get_json()['data']['status'] == CredentialStatus.Verificado.value
...
class TestExposicionDeDatos:
    def test_badge_enmascara_la_matricula(self, client, vet, operario):
        ...
        response = client.get(f'{BASE}/user/{vet["user_id"]}/badge', headers=operario['headers'])
        assert response.status_code == 200
    ...
    def test_pending_cerrado_a_roles_sin_permiso(self, client, operario):
        assert client.get(f'{BASE}/pending', headers=operario['headers']).status_code == 403
