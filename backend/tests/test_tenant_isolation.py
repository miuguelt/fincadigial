    ...
    def setUp(self):
        ...
        with self.app.app_context():
            ...
            self.finca_a = Finca.create(
                name="Finca A - El Progreso", type=FarmType.Tradicional, is_active=True
            )

            self.finca_b = Finca.create(
                name="Finca B - La Esperanza", type=FarmType.Educativa, is_active=True
            )
    ...
    def test_Animales_aislados_por_finca(self):
        ...
        with self.app.app_context():
            ...
            Animal_a_en_b = Animal.query.filter_by(
                finca_id=self.finca_b.id, record="A-001"
            ).first()

            self.assertIsNone(Animal_a_en_b, "Animal A-001 no debe existir en finca B")
    ...
    def test_tenant_models_lista_completa(self):
        ...
        for modelo in modelos_esperados:
            with self.subTest(modelo=modelo):
                self.assertIn(
                    modelo, TENANT_MODELS, f"{modelo} debe estar en TENANT_MODELS"
                )
...
class TestFincaModel(unittest.TestCase):
    ...
    def test_finca_relacion_con_Animales(self):
        ...
        with self.app.app_context():
            finca = Finca.create(
                name="Finca con Animales", type=FarmType.Tradicional, is_active=True
            )
            db.session.commit()
