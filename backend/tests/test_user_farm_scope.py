...
def test_farm_catalog_is_global_only_for_master_administrator(app, client, monkeypatch):
    ...
    local_response = client.get('/api/v1/fincas?limit=100&cache_bust=1', headers=farm_admin_headers)
    global_response = client.get('/api/v1/fincas?limit=100&cache_bust=1', headers=master_headers)

    assert local_response.status_code == 200
    assert {item['id'] for item in local_response.get_json()['data']} == {farm_a_id}
    assert global_response.status_code == 200
    assert {farm_a_id, farm_b_id} <= {item['id'] for item in global_response.get_json()['data']}
