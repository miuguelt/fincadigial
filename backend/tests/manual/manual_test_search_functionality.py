...
def test_search_functionality():
    ...
    with app.app_context():
        ...
        for model_name, model_class in models_to_test:
            ...
            year_count = (
                year_query.count()
                if hasattr(year_query, "count")
                else len(year_query.all())
            )
            ...
            month_count = (
                month_query.count()
                if hasattr(month_query, "count")
                else len(month_query.all())
            )
            ...
            today_count = (
                today_query.count()
                if hasattr(today_query, "count")
                else len(today_query.all())
            )
            ...
            text_count = (
                text_query.count()
                if hasattr(text_query, "count")
                else len(text_query.all())
            )
            print(
                f"  🔤 Búsqueda de texto 'a' en todas las columnas: {text_count} resultados"
            )
            ...
            if model_class.query.first():
                ...
                id_count = (
                    id_query.count()
                    if hasattr(id_query, "count")
                    else len(id_query.all())
                )
