...
def test_with_specific_id():
    ...
    try:
        ...
        with app.app_context():
            ...
            # Hijos como padre
            query = text(
                "SELECT COUNT(*) as count FROM animals WHERE idFather = :animal_id"
            )
            ...
            # Hijos como madre
            query = text(
                "SELECT COUNT(*) as count FROM animals WHERE idMother = :animal_id"
            )
            result = db.session.execute(query, {"animal_id": animal_id}).fetchone()
            ...
            # Tratamientos
            query = text(
                "SELECT COUNT(*) as count FROM treatments WHERE animals_id = :animal_id"
            )
            result = db.session.execute(query, {"animal_id": animal_id}).fetchone()
            ...
            # Vacunaciones
            query = text(
                "SELECT COUNT(*) as count FROM vaccinations WHERE animals_id = :animal_id"
            )
            result = db.session.execute(query, {"animal_id": animal_id}).fetchone()
                    ...
                    if warning.dependent_field == "idFather":
                        if warning.dependent_count != hijos_padre:
                            discrepancies.append(
                                f"idFather: IC={warning.dependent_count} vs Real={hijos_padre}"
                            )
                    elif warning.dependent_field == "idMother":
                        if warning.dependent_count != hijos_madre:
                            discrepancies.append(
                                f"idMother: IC={warning.dependent_count} vs Real={hijos_madre}"
                            )
                elif warning.dependent_table == "treatments":
                    if warning.dependent_count != treatments:
                        discrepancies.append(
                            f"treatments: IC={warning.dependent_count} vs Real={treatments}"
                        )
                elif warning.dependent_table == "vaccinations":
                    if warning.dependent_count != vaccinations:
                        discrepancies.append(
                            f"vaccinations: IC={warning.dependent_count} vs Real={vaccinations}"
                        )
