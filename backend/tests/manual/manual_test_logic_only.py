...
def test_logic_only():
    ...
    try:
        ...
        self_refs = [
            rel for rel in relationships if rel["target_table"] == Animals.__tablename__
        ]
        forward_rels = [
            rel
            for rel in relationships
            if not rel.get("reverse") and rel["target_table"] != Animals.__tablename__
        ]
        ...
        reverse_breeds = [
            rel for rel in relationships if rel["name"] == "reverse_breeds"
        ]
        if reverse_breeds:
        ...
        if set(expected_self_refs) == set(actual_self_refs):
            print(
                f"✅ CORRECTO: Auto-referencias detectadas correctamente: {actual_self_refs}"
            )
        else:
            print(
                f"❌ ERROR: Auto-referencias incorrectas. Esperadas: {expected_self_refs}, Detectadas: {actual_self_refs}"
            )
            return False
        ...
        for rel in self_refs_processed:
            field_name = (
                rel["foreign_keys"][0]
                if rel["foreign_keys"]
                else f"{Animals.__tablename__}_id"
            )
