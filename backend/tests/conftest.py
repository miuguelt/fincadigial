...
_cached_test_password = None


def get_test_password() -> str:
    """Obtiene la contraseña de prueba del entorno o genera una dinámica y segura."""
    global _cached_test_password
    if not _cached_test_password:
        _cached_test_password = (
            os.environ.get("TEST_USER_PASSWORD") or f"T3st_{secrets.token_urlsafe(8)}A!"
        )
    return _cached_test_password
...
def token_for(app, db_session):
    ...
    def _token_for(role_name: str, finca_type: str = "Tradicional"):
        with app.app_context():
            ...
            f_type = next(
                (t for t in FarmType if t.value == finca_type), FarmType.Tradicional
            )
