...
def test_campesino_v1_crud_smoke(client, token_for):
    ...
    water = _assert_created(
        client.post(
            f"{BASE}/water-sources",
            json={
                "name": "Nacimiento La Pena",
                "source_type": "stream",
                "is_potable": True,
            },
            headers=headers,
    ...
    for endpoint in [
        ...
        assert resp.status_code == 200, (
            f"{endpoint}: {resp.get_data(as_text=True)[:300]}"
        )
