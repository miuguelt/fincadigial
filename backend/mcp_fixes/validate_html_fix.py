...
def validate_html_fixed(html_content):
    ...
    try:
        ...
        if not soup.find("html"):
            issues.append(
                {"type": "missing_tag", "message": "Tag <html> no encontrado"}
            )

        if not soup.find("head"):
            issues.append(
                {"type": "missing_tag", "message": "Tag <head> no encontrado"}
            )

        if not soup.find("body"):
            issues.append(
                {"type": "missing_tag", "message": "Tag <body> no encontrado"}
            )
        ...
        for img in images_without_alt:
            issues.append(
                {
                    "type": "accessibility",
                    "message": f"Imagen sin alt: {img.get('src', 'unknown')}",
                }
            )
            ...
            if input_id and not soup.find("label", {"for": input_id}):
                ...
                if not inp.get("aria-label") and not inp.get("placeholder"):
                    issues.append(
                        {
                            "type": "accessibility",
                            "message": f"Input sin label: {input_id}",
                        }
                    )

        return {"valid": len(issues) == 0, "issueCount": len(issues), "issues": issues}
