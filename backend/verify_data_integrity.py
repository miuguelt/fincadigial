...
def verify_milk_production():
    ...
    with app.app_context():
        ...
        orphaned = db.session.query(MilkProduction).filter(
            ~MilkProduction.animal_id.in_(
        ...
        males_with_milk = db.session.query(MilkProduction).join(
            Animals, MilkProduction.animal_id == Animals.id
        ).filter(Animals.sex == 'Macho').count()
        
        if males_with_milk > 0:
            issues.append(f"⚠ {males_with_milk} machos con producción de leche")
        
        # 3. Verificar animales no vivos
        dead_with_milk = db.session.query(MilkProduction).join(
            Animals, MilkProduction.animal_id == Animals.id
        ).filter(Animals.status != AnimalStatus.Vivo).count()
        ...
        if not issues:
            ...
            for issue in issues:
                print(f"     {issue}")
            return {'table': 'milk_production', 'status': 'WARNING', 'count': total, 'issues': issues}
...
def verify_reproductive_events():
    ...
    from app.models.reproduction import ReproductiveEvent, EventType, InseminationTechnique, DiagnosisResult
    ...
    with app.app_context():
        ...
        orphaned = db.session.query(ReproductiveEvent).filter(
            ~ReproductiveEvent.animal_id.in_(db.session.query(Animals.id))
        ).count()
        ...
        orphaned_sires = db.session.query(ReproductiveEvent).filter(
        ...
        from app.models.reproduction import InseminationTechnique
        invalid_tech = db.session.query(ReproductiveEvent).filter(
        ...
        pending_diags = db.session.query(ReproductiveEvent).filter(
        ...
        if not issues:
            ...
            for issue in issues:
                print(f"     {issue}")
            return {'table': 'reproductive_events', 'status': 'WARNING', 'count': total, 'issues': issues}
...
def verify_offspring():
    ...
    with app.app_context():
        ...
        orphaned = db.session.query(Offspring).filter(
            ~Offspring.birth_event_id.in_(db.session.query(ReproductiveEvent.id))
        ).count()
        ...
        orphaned_animals = db.session.query(Offspring).filter(
            Offspring.animal_id != None,
            ~Offspring.animal_id.in_(db.session.query(Animals.id))
        ).count()
...
def verify_transactions():
    ...
    with app.app_context():
        ...
        orphaned = db.session.query(Transaction).filter(
            ~Transaction.finca_id.in_(db.session.query(Finca.id))
        ).count()
        ...
        orphaned_animals = db.session.query(Transaction).filter(
...
def verify_inventory_movements():
    ...
    with app.app_context():
        ...
        orphaned = db.session.query(InventoryMovement).filter(
            ~InventoryMovement.lot_id.in_(db.session.query(InventoryLot.id))
        ).count()
        ...
        if not issues:
            ...
            for issue in issues:
                print(f"     {issue}")
            return {'table': 'inventory_movements', 'status': 'WARNING', 'count': total, 'issues': issues}
...
def verify_user_finca():
    ...
    with app.app_context():
        ...
        orphaned_users = db.session.query(UserFinca).filter(
            ~UserFinca.user_id.in_(db.session.query(User.id))
        ).count()
        ...
        orphaned_fincas = db.session.query(UserFinca).filter(
            ~UserFinca.finca_id.in_(db.session.query(Finca.id))
        ).count()
        ...
        duplicates = db.session.query(
            UserFinca.user_id, UserFinca.finca_id
        ).group_by(
            UserFinca.user_id, UserFinca.finca_id
...
def verify_animal_images():
    ...
    with app.app_context():
        ...
        # 1. Verificar animal_id existen
        orphaned = db.session.query(AnimalImages).filter(
            ~AnimalImages.animal_id.in_(db.session.query(Animals.id))
        ).count()
        ...
        from sqlalchemy import func
        primary_counts = db.session.query(
            AnimalImages.animal_id, func.count().label('primary_count')
        ).filter(AnimalImages.is_primary == True).group_by(
            AnimalImages.animal_id
        ).having(func.count() > 1).all()
