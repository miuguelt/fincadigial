...
export const InviteUserDialog: React.FC<InviteUserDialogProps> = ({ open, onOpenChange, fincaId }) => {
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      showToast(error.response?.data?.message || 'No se pudo enviar la invitación', 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Envía una invitación por correo electrónico para que un usuario se una a esta finca con un rol específico.
          </DialogDescription>
        </DialogHeader>
