...
import { BrainCircuit, Activity, ShieldAlert, Cpu, Network, Zap } from 'lucide-react';
...
export const ExecutiveIntelligence: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const res = await livestockService.getSummary();
        setStats(res);
  ...
  const handleRunAnalysis = async () => {
    setIsAnalyzing(true);
    setAiInsight("Iniciando escaneo inteligente del hato... Esto puede tomar unos minutos.");
    try {
      const res = await analyticsService.runPredictiveAnalysis();
      
      // Feedback inmediato del inicio de tarea
      setAiInsight(res.message || "Análisis en segundo plano iniciado. El Cerebro Villa Luz está procesando los datos de los animales...");
      ...
      setAiInsight("Hubo un error al conectar con el motor predictivo. Por favor, intenta de nuevo.");
    } finally {
      setIsAnalyzing(false);
    }
  };

  const healthScore = stats ? Math.floor(((stats.active_animals - stats.sick_animals) / stats.active_animals) * 100) : 0;
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-black tracking-tight">Cerebro Villa Luz</h3>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Inteligencia de Finca Activa</p>
            </div>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 bg-green-500/20 text-green-400 rounded-full text-xs font-black border border-green-500/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white/5 backdrop-blur-sm p-6 rounded-3xl border border-white/5">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] font-black text-slate-500 uppercase">Índice Vital</span>
            </div>
            <div className="text-4xl font-black mb-1">{healthScore}%</div>
            <div className="text-xs text-slate-400">Salud proyectada del hato</div>
          ...
          <div className="bg-white/5 backdrop-blur-sm p-6 rounded-3xl border border-white/5">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] font-black text-slate-500 uppercase">Red de Campo</span>
            </div>
            <div className="text-4xl font-black mb-1">{meshSyncs}</div>
            <div className="text-xs text-slate-400">Intercambios Mesh registrados hoy</div>
            <div className="mt-4 flex gap-1">
              {[...Array(5)].map((_, i) => (
                <div key={i} className={`h-1 flex-1 rounded-full ${i < 4 ? 'bg-green-500' : 'bg-slate-700'}`} />
              ))}
            </div>
          </div>

          {/* Seguridad Geofencing */}
          <div className="bg-white/5 backdrop-blur-sm p-6 rounded-3xl border border-white/5">
            <div className="flex items-center justify-between mb-4">
              <ShieldAlert className="text-amber-400" size={20} />
              <span className="text-[10px] font-black text-slate-500 uppercase">Zonas de Riesgo</span>
            </div>
            <div className="text-4xl font-black mb-1">0</div>
            <div className="text-xs text-slate-400">Invasiones críticas de geocerca</div>
            <div className="mt-4 flex items-center gap-2 text-[10px] font-bold text-green-400">
              <CheckCircle size={12} />
              TODOS LOS OPERARIOS SEGUROS
            </div>
          </div>
        </div>

        {/* Sección de IA Predictive (F7) */}
        <div className="mt-8 p-6 bg-gradient-to-r from-blue-600/20 to-purple-600/20 rounded-[2.5rem] border border-blue-500/20 backdrop-blur-md">
          <div className="flex items-center justify-between mb-4">
            ...
            <button 
              ...
              className={`px-6 py-2 rounded-full text-xs font-black uppercase tracking-tighter transition-all ${
                ...
                : 'bg-white text-slate-900 hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.2)]'
              }`}
            >
          ...
          <div className="bg-slate-950/40 p-5 rounded-2xl border border-white/5 min-h-[80px] flex items-center justify-center text-center">
            {aiInsight ? (
              ...
              <p className="text-xs text-slate-500 font-medium">
                Inicia un escaneo para obtener recomendaciones predictivas basadas en datos reales.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="mt-8 flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[10px] font-bold text-slate-300">
              Última sincronización del Gateway: <span className="text-white">{new Date().toLocaleTimeString()}</span>
            </p>
          </div>
          <button className="text-[10px] font-black text-blue-400 hover:text-blue-300 transition-colors uppercase tracking-widest">
