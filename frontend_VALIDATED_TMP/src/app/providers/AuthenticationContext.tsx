...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
      ...
      if (userFromApi) {
        ...
        if (status === 401) {
          ...
          if (persisted) {
            ...
            setUser(persisted)
            setRole(persisted.role || null)
            setName(persisted.fullname || null)
            setIsAuthenticated(true)
            setLoading(false)
          } else {
            clearAuthState()
  ...
  const hasPermission = useCallback((permission: string) => {
    ...
    const rolePermissions: Record<string, string[]> = {
      ...
      [Role.Capataz]: ['dashboard:read', 'user:read', 'user:write', 'system:read', 'system:write', 'animal:read', 'animal:write', 'treatment:read', 'treatment:write', 'task:read', 'task:write'],
      [Role.Instructor]: ['dashboard:read', 'user:read', 'animal:read', 'animal:write', 'treatment:read', 'treatment:write', 'task:read'],
      [Role.Veterinario]: ['dashboard:read', 'user:read', 'animal:read', 'animal:write', 'treatment:read', 'treatment:write', 'task:read'],
      [Role.Aprendiz]: ['dashboard:read', 'animal:read', 'treatment:read', 'task:read'],
      [Role.Operario]: ['dashboard:read', 'animal:read', 'treatment:read', 'task:read'],
    }
