...
def run_complex_audit():
    ...
    with app.app_context():
        ...
        res_login = client.post(
            "/api/v1/auth/login",
            json={"identification": admin_id, "password": admin_password},
        )
        ...
        if not all([animal, medication, vaccine]):
            print(
                "❌ [VET_AUDIT] Faltan recursos base (Animal/Med/Vac). Ejecuta massive_seed primero."
            )
            return

        print(
            f"🔬 [VET_AUDIT] Animal: {animal.record} | Med: {medication.name} | Vac: {vaccine.name}"
        )
