...
def main():
    ...
    # 2. Indexar Código Python (BackFinca)
    print("\n--- Indexando Código Backend ---")
    py_files = glob.glob(f"{root_dir}/BackFinca/app/**/*.py", recursive=True)
    for f in py_files:
        if "__init__" in f or "venv" in f: continue
        print(f"Procesando {os.path.basename(f)}...")
        with open(f, 'r', encoding='utf-8') as file:
            index_text(service, file.read(), f)
    ...
    tsx_files = glob.glob(f"{root_dir}/VillaLuzFront/src/**/*.tsx", recursive=True)
    for f in tsx_files:
        print(f"Procesando {os.path.basename(f)}...")
        with open(f, 'r', encoding='utf-8') as file:
            index_text(service, file.read(), f)
