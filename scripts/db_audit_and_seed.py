#!/usr/bin/env python3
"""
Script de auditoría de la Base de Datos VillaLuz.

Las rutinas de poblamiento se conservan únicamente para entornos de prueba
controlados. Nunca se ejecutan por defecto porque generan datos que no
representan movimientos reales de la finca.
...
_ALLOW_SIMULATION = os.getenv("ALLOW_SIMULATION_SCRIPTS", "").lower() == "true"


def _require_simulation_permission():
    if not _ALLOW_SIMULATION:
        raise RuntimeError(
            "Poblamiento sintético deshabilitado. "
            "Use ALLOW_SIMULATION_SCRIPTS=true solo en una base de pruebas."
        )
    ...
    with app.app_context():
        ...
        for table in sorted(tables):
            try:
                ...
                table_info = {"name": table, "records": count}
...
def seed_animal_images(app, db):
    ...
    sample_images = [
        {
            "filename": "vaca_001.jpg",
            "filepath": "/uploads/animals/vaca_001.jpg",
            "mime_type": "image/jpeg",
        },
        {
            "filename": "vaca_002.jpg",
            "filepath": "/uploads/animals/vaca_002.jpg",
            "mime_type": "image/jpeg",
        },
        {
            "filename": "toro_001.jpg",
            "filepath": "/uploads/animals/toro_001.jpg",
            "mime_type": "image/jpeg",
        },
        {
            "filename": "ternero_001.jpg",
            "filepath": "/uploads/animals/ternero_001.jpg",
            "mime_type": "image/jpeg",
        },
    ]
    ...
    for i, animal in enumerate(animals):
        ...
        try:
            existing = AnimalImages.query.filter_by(
                animal_id=animal.id, filename=img_data["filename"]
            ).first()
            if not existing:
...
def seed_milk_production(app, db):
    ...
    animals = (
        Animals.query.filter(Animals.sex == "Hembra", Animals.status == AnimalStatus.Vivo)
        .limit(5)
        .all()
    )
            ...
            for session in sessions:
                try:
                    ...
                    existing = MilkProduction.query.filter_by(
                        animal_id=animal.id, date=record_date, milking_session=session
                    ).first()
                    if not existing:
...
def seed_reproductive_events(app, db):
    ...
    # Buscar machos
    males = (
        Animals.query.filter(Animals.sex == "Macho", Animals.status == AnimalStatus.Vivo)
        .limit(3)
        .all()
    )
    ...
    for female in females:
        try:
            ...
            existing = ReproductiveEvent.query.filter_by(
                animal_id=female.id, event_type=EventType.Celo, event_date=ev_date
            ).first()
            if not existing:
            ...
            existing = ReproductiveEvent.query.filter_by(
                animal_id=female.id, event_type=EventType.Inseminacion, event_date=ev_date_ins
            ).first()
            if not existing:
            ...
            existing = ReproductiveEvent.query.filter_by(
                animal_id=female.id, event_type=EventType.Diagnostico, event_date=ev_date_diag
            ).first()
            if not existing:
                ReproductiveEvent.create(
                    animal_id=female.id,
    ...
    transactions_data = [
        {
            ...
            "category": TransactionCategory.Milk,
            "amount": 2500000,
            "desc": "Venta mensual de leche",
        },
        {
            "type": TransactionType.Income,
            "category": TransactionCategory.Animal,
            "amount": 3500000,
            "desc": "Venta de novilla",
        },
        {
            "type": TransactionType.Expense,
            "category": TransactionCategory.Food,
            "amount": 800000,
            "desc": "Compra de concentrado",
        },
        {
            "type": TransactionType.Expense,
            "category": TransactionCategory.Medication,
            "amount": 450000,
            "desc": "Vacunación general",
        },
        {
            "type": TransactionType.Expense,
            "category": TransactionCategory.Service,
            "amount": 600000,
            "desc": "Servicio veterinario",
        },
        {
            "type": TransactionType.Expense,
            "category": TransactionCategory.Other,
            "amount": 200000,
            "desc": "Mantenimiento de potreros",
        },
    ]
    ...
    for i, trans in enumerate(transactions_data):
        try:
            existing = Transaction.query.filter_by(
                finca_id=finca_id, description=trans["desc"]
            ).first()
            if not existing:
                Transaction.create(
                    finca_id=finca_id,
    ...
    for lot in lots:
        try:
            existing = InventoryMovement.query.filter_by(
                lot_id=lot.id, finca_id=lot.finca_id
            ).first()
            if not existing:
    ...
    for i, treatment in enumerate(treatments):
        ...
        try:
            ...
            existing = TreatmentMedications.query.filter_by(
                treatment_id=treatment.id, medication_id=med.id
            ).first()

            if not existing:
                TreatmentMedications.create(treatment_id=treatment.id, medication_id=med.id)
                created += 1
        except Exception as e:
            print(f"  ⚠ Error: {e}")
    ...
    for i, treatment in enumerate(treatments):
        ...
        try:
            ...
            existing = TreatmentVaccines.query.filter_by(
                treatment_id=treatment.id, vaccine_id=vaccine.id
            ).first()

            if not existing:
                TreatmentVaccines.create(treatment_id=treatment.id, vaccine_id=vaccine.id)
                created += 1
        except Exception as e:
            print(f"  ⚠ Error: {e}")
...
def seed_animal_fields(app, db):
    ...
    for animal in animals:
        try:
            ...
            existing = AnimalFields.query.filter_by(animal_id=animal.id, removal_date=None).first()
...
def seed_all_tables():
    ...
    _require_simulation_permission()
    from app import create_app
    from app.extensions import db
...
def seed_membership_requests(app, db):
    ...
    for user in users:
        ...
        try:
            # Verificar si ya existe
            existing = MembershipRequest.query.filter_by(user_id=user.id, finca_id=finca.id).first()
...
def main():
    ...
    if args.table:
        ...
        if initial_status["empty_tables"]:
            print(
                "\n⚠️ Hay tablas vacías, pero no se poblarán automáticamente: "
                "los datos deben registrarse desde los flujos reales del sistema."
            )
        else:
