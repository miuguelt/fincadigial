...
def test_alert_engine_v3():
    ...
    with app.app_context():
        ...
        if old_ids:
            MilkProduction.query.filter(MilkProduction.animal_id.in_(old_ids)).delete(
                synchronize_session=False
            )
            ReproductiveEvent.query.filter(ReproductiveEvent.animal_id.in_(old_ids)).delete(
                synchronize_session=False
            )
