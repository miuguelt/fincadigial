...
class BackupSystem:
    ...
    def create_backup(self, backup_type="snapshot", description="", tags=None):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            timestamp, backup_type, str(backup_path), 
            total_size, compressed_size, description, json.dumps(tags or [])
        ))
        conn.commit()
        conn.close()
        ...
        if compressed_size != total_size:
            print(f"   Comprimido: {self.format_size(compressed_size)} ({metadata['compression_ratio']:.1f}%)")
    ...
    def restore_backup(self, timestamp, target_path=None):
        ...
        target_path = Path(target_path) if target_path else self.project_root / f"restored_{timestamp}"
...
def main():
    ...
    parser.add_argument("--type", choices=["daily", "weekly", "snapshot"], default="snapshot", 
                       help="Tipo de backup")
