...
class RestoreSystem:
    ...
    def display_backups(self, backups):
        ...
        print("-" * 100)
        print(
            f"{'#':<3} {'Timestamp':<17} {'Type':<10} {'Size':<10} {'Compressed':<12} {'Description':<30} {'Tags'}"
        )
        print("-" * 100)
        ...
        for i, (timestamp, btype, size, comp_size, desc, tags) in enumerate(backups, 1):
            ...
            print(
                f"{i:<3} {timestamp:<17} {btype:<10} {size_str:<10} {comp_str:<12} {desc_str:<30} {tags_str}"
            )
    ...
    def restore_backup(self, timestamp, target_path=None, dry_run=False):
        ...
        if target_path:
            ...
            target = self.backup_root / "restores" / f"restored_{timestamp_clean}"
    ...
    def interactive_restore(self):
        ...
        while True:
            try:
                choice = input(
                    f"\n🔍 Seleccione backup (1-{len(backups)}) o 'q' para salir: "
                ).strip()
