...
def setup_functional_test():
    ...
    with app.app_context():
        ...
        if not boyaca_t:
            boyaca_t = Territory(name="Vereda Higueras", municipality="Duitama", department="Boyacá", vereda="Higueras")
        ...
        # 2. Crear Fincas
        fincas = [
            {"name": "SENA Regional Boyacá - CEDEAGRO", "type": FarmType.Educativa, "t_id": boyaca_t.id, "mun": "Duitama", "dep": "Boyacá"},
            {"name": "Finca El Recuerdo (Ganadera)", "type": FarmType.Tradicional, "t_id": antioquia_t.id if antioquia_t else None, "mun": "Rionegro", "dep": "Antioquia"}
        ]
        ...
        # 3. Crear Usuarios
        users_data = [
            {"id": 3001, "name": "Instructor Boyacá", "email": "inst_boyaca@sena.edu.co", "phone": "3110000001", "role": Role.Instructor, "finca": "SENA Regional Boyacá - CEDEAGRO", "ptype": ProducerType.Institucional},
            {"id": 4001, "name": "Don Pedro", "email": "pedro_recuerdo@gmail.com", "phone": "3110000002", "role": Role.Propietario, "finca": "Finca El Recuerdo (Ganadera)", "ptype": ProducerType.Comercial_Pequeno}
        ]
        ...
        for ud in users_data:
            ...
            if not u:
                u = User.create(
                    ...
                    password="password123", phone=ud["phone"], role=ud["role"], 
            ...
            if not ProducerProfile.query.filter_by(user_id=u.id).first():
                db.session.add(ProducerProfile(user_id=u.id, producer_type=ud["ptype"], land_tenure="Propia", notes="Prueba Funcional"))

        # 4. Registrar Animal para Don Pedro
        finca_pedro = fincas_map["Finca El Recuerdo (Ganadera)"]
        pedro = User.query.filter_by(identification=4001).first()
