...
def check_frontend_routes(app_routes_path: str) -> tuple[list[str], list[str]]:
    ...
    with open(app_routes_path, encoding="utf-8") as f:
        content = f.read()
...
def main():
    ...
    if app_routes.exists():
        ...
        if missing_routes:
            print("   ❌ Rutas faltantes en AppRoutes.tsx:")
    ...
    if missing_backend:
        print("   ❌ Endpoints faltantes en backend:")
    ...
    if admin_pages.exists():
        ...
        if missing_pages:
            print("   ⚠️  Páginas potencialmente faltantes (directorios no encontrados):")
