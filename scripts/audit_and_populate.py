...
def run_operations():
    ...
    with app.app_context():
        ...
        if not father:
            father = Animals.create(record="FATHER-001", sex=Sex.Macho, breeds_id=breed.id, 
                                   birth_date=date(2018, 1, 1), weight=600, status=AnimalStatus.Vivo)
            print("✅ [SEED] Padre creado.")
        
        mother = Animals.query.filter_by(record="MOTHER-001").first()
        if not mother:
            mother = Animals.create(record="MOTHER-001", sex=Sex.Hembra, breeds_id=breed.id, 
                                   birth_date=date(2018, 2, 1), weight=500, status=AnimalStatus.Vivo)
            print("✅ [SEED] Madre creado.")
            
        son = Animals.query.filter_by(record="SON-001").first()
        if not son:
            son = Animals.create(record="SON-001", sex=Sex.Macho, breeds_id=breed.id, 
                               birth_date=date(2024, 1, 1), weight=80, status=AnimalStatus.Vivo,
                               idFather=father.id, idMother=mother.id)
            print(f"✅ [SEED] Hijo creado con genealogía.")
        else:
        ...
        # Login como Administrador (99999999 / password123)
        login_data = {"identification": 99999999, "password": "password123"}
        ...
        if response.status_code == 200:
            ...
            if son:
                ...
                res = client.get(f'/api/v1/animals/tree/ancestors?animal_id={son.id}', headers=headers)
            ...
            new_animal = {"record": "CRUD-VERIFIED-001", "sex": "Macho", "birth_date": "2023-01-01", 
                          "weight": 100, "breeds_id": breed.id, "status": "Vivo"}
            ...
            print(f"✅ [CRUD] Update OK (Peso actualizado a 135)")
...
if __name__ == "__main__":
    ...
    if missing:
        sql = "BEGIN TRANSACTION;\n" + "".join([f"ALTER TABLE animals ADD COLUMN {c} INTEGER REFERENCES animals(id);\n" for c in missing]) + "COMMIT;"
        db_path = 'BackFinca/instance/finca.db' if os.path.exists('BackFinca/instance/finca.db') else 'BackFinca/finca.db'
        conn = sqlite3.connect(db_path)
    ...
    try:
        import subprocess
        subprocess.run([sys.executable, 'BackFinca/seed_data.py'], capture_output=True)
        run_operations()
