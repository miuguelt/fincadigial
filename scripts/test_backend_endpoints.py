...
def test_endpoint(
    method: str, endpoint: str, description: str, token: str = None
) -> tuple[bool, str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if token:
                return False, "⚠️  Unauthorized (401) - Token inválido?"
            return True, "🔒 Protegido (401) - Requiere login"
        elif response.status_code == 404:
            return False, "❌ Not Found (404) - Endpoint no existe"
        ...
        return False, "❌ Connection Error - Backend no responde"
...
def main():
    ...
    try:
        admin_id, admin_password = get_role_credentials("Administrador")
        login_data = {
