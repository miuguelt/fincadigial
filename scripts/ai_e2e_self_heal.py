...
if not os.path.isdir(BACKEND_DIR):
    BACKEND_DIR = os.path.join(PROJECT_DIR, "BackFinca")
...
try:
    ...
    with app.app_context():
        ...
        print(f"   Conectado a PostgreSQL: {db.engine.url.database} en {db.engine.url.host}:{db.engine.url.port}")
        report["database"]["status"] = "CONNECTED"
        
        for table_name, table in metadata_tables.items():
            if table_name not in db_tables:
                print(f"   ⚠️ Tabla faltante en DB física: '{table_name}'. Creando tabla automáticamente...")
                try:
                    ...
                    report["database"]["errors_found"].append(f"Error creando tabla {table_name}: {str(ex)}")
                continue
            ...
            for col_name, column in table.columns.items():
                if col_name not in db_cols:
                    ...
                    report["database"]["errors_found"].append(f"Columna faltante: {table_name}.{col_name}")
                    ...
                    try:
                        ...
                        db.session.commit()
                        print(f"   ⚡ AUTORREPARADO: Columna '{col_name}' agregada con éxito a la tabla '{table_name}'.")
                        report["database"]["repaired_columns"].append(f"{table_name}.{col_name} ({ddl_type})")
                    except Exception as alter_ex:
                        db.session.rollback()
                        print(f"   ❌ Fallo al aplicar ALTER TABLE: {alter_ex}")
                        report["database"]["errors_found"].append(f"Fallo ALTER {table_name}.{col_name}: {str(alter_ex)}")
        
        if not report["database"]["errors_found"]:
            print("   ✅ Base de datos 100% sincronizada y saludable. Cero discrepancias encontradas.")
            report["database"]["status"] = "SYNCHRONIZED"
        else:
            if report["database"]["repaired_columns"]:
                print(f"   🛠️ Reparaciones realizadas: {len(report['database']['repaired_columns'])} campos corregidos.")
                report["database"]["status"] = "REPAIRED"
            else:
                print("   ❌ Se encontraron discrepancias que no pudieron autorrepararse de forma automática.")
...
# Cargar identidad SSOT
identity_path = os.path.abspath(os.path.join(PROJECT_DIR, "..", "devbrain", "config", "IDENTITY.json"))
admin_id = 1098
admin_password = "Villaluz2024!" # Fallback por defecto
...
try:
    ...
    for user in identity.get("test_users", []):
        if user.get("role") == "Administrador":
            ...
            print(f"   🔑 Cargadas credenciales SSOT desde devbrain: Admin {admin_id}")
            break
except Exception as e:
    print(f"   ⚠️ Advertencia SSOT: No se pudo leer {identity_path}. Usando fallback. Error: {e}")

# Iniciar pruebas HTTP
token = None
try:
    login_data = {
        "identification": str(admin_id),
...
if token:
    ...
    if smoke_success:
        ...
        print("   ⚠️ Algunos endpoints retornaron códigos de error. Diagnostica los logs del backend.\n")
else:
...
if os.path.isfile(vite_config_path):
    try:
        ...
        if has_optimize:
            print("   ✅ Vite Config: optimizeDeps.include detectado. Las dependencias masivas están pre-empaquetadas.")
            report["vite_hmr"]["status"] = "OPTIMIZED"
            report["vite_hmr"]["details"] = "Pre-bundling configured correctly."
        else:
            print("   ⚠️ Vite Config: Falta sección optimizeDeps.include. Podrías experimentar fallas 504 HMR en el primer arranque.")
...
print(f"2. Base de Datos: {'🟢 SINCRONIZADA' if report['database']['status'] == 'SYNCHRONIZED' else '🔵 AUTORREPARADA' if report['database']['status'] == 'REPAIRED' else '🔴 ERROR/DESINCRONIZADA'}")
print(f"3. Pruebas de Humo API: {'🟢 EXITOSO (ALL PASS)' if report['api_smoke_test']['status'] == 'ALL_PASS' else '🔴 CON ERRORES'}")
print(f"4. Optimización HMR: {'🟢 CORRECTA' if report['vite_hmr']['status'] == 'OPTIMIZED' else '🟡 SIN OPTIMIZAR'}")
print(f"\nReporte JSON completo guardado en: [self_heal_run.json](file:///{LOG_FILE})")
print("=" * 80)

# Código de salida basado en gravedad
if report["database"]["status"] in ["SYNCHRONIZED", "REPAIRED"] and report["api_smoke_test"]["status"] == "ALL_PASS":
    sys.exit(0)
else:
    sys.exit(1)
