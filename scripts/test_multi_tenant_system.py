...
class MultiTenantTester:
    ...
    def login(self, identifier: str, password: str, role_name: str) -> Optional[str]:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if data.get("success"):
                    ...
                    role = user.get("role")
                    print_success(f"Login {role_name}: {user.get('fullname')} (finca_id={finca_id}, type={finca_type}, role={role})")
    ...
    def test_login_all_roles(self) -> bool:
        ...
        credentials = [
            ("1098", "Admin1234!", "Administrador"),
            ("11111111", "Instructor1234!", "Instructor"),
            ("22222222", "Aprendiz1234!", "Aprendiz"),
        ]
        ...
        if success_count == len(credentials):
            print_success(f"Todos los roles autenticaron correctamente ({success_count}/{len(credentials)})")
            return True
        else:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for animal in animals[:3]:  # Verificar primeros 3
                    ...
                    if finca_id != 1:
                        print_error(f"Animal {animal.get('id')} tiene finca_id={finca_id}, esperado=1")
                        return False
    ...
    def test_rbac_restriction(self) -> bool:
        ...
        if aprendiz_token:
            ...
            resp = requests.post(
                f"{BASE_URL}/animals",
                headers=headers,
                json={"name": "Test Animal"}
            )
            if resp.status_code in [403, 401]:
                print_success("Aprendiz no puede crear animales (403)")
                results.append(True)
            else:
                print_error(f"Aprendiz debería tener 403 para POST animals, obtuvo {resp.status_code}")
        ...
        if instructor_token:
            ...
            if resp.status_code in [403, 401]:
                ...
                print_error(f"Instructor debería tener 403 para GET /users, obtuvo {resp.status_code}")
                results.append(False)
        ...
        try:
            ...
            if resp.status_code == 201:
                ...
                if data.get("success"):
                    ...
                    print_success(f"Usuario creado: {user.get('fullname')} (Rol: {user.get('role')})")
