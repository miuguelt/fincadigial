...
def run_massive_seed_and_audit():
    ...
    with app.app_context():
        ...
        if not admin:
            admin = User(
                identification=99999999, fullname="Super Admin", email="admin99@finca.com",
                phone="9999999999", role=Role.Administrador if hasattr(Role, 'Administrador') else 'Administrador',
                password=generate_password_hash("password123"), status=True,
        ...
        for i in range(20):
            ...
            if not User.query.filter((User.identification == uid) | (User.email == u_email) | (User.phone == u_phone)).first():
                db.session.add(User(
                    identification=uid, fullname=f"Usuario Prueba {i}",
                    email=u_email, phone=u_phone,
                    role=random.choice(list(Role)), password=generate_password_hash("Pass123!"), status=True,
                    finca_id=default_finca.id
                ))
        ...
        for i in range(20):
            ...
            if not FoodTypes.query.filter_by(food_type=ft_name, finca_id=default_finca.id).first():
                db.session.add(FoodTypes(
                    food_type=ft_name, sowing_date=date.today() - timedelta(days=90),
                    area=50, handlings="Manejo estándar", gauges="Medidor 1",
                    finca_id=default_finca.id
                ))
        db.session.commit()
        ...
        for i in range(20):
            ...
            if not Fields.query.filter_by(name=fname, finca_id=default_finca.id).first():
                ...
                db.session.add(Fields(
                    name=fname, ubication=f"Bloque {i%5}", capacity="100",
                    state=random.choice(list(LandStatus)), handlings="Rotacional",
                    gauges="1x1", area="200", food_type_id=ft.id,
                    finca_id=default_finca.id
                ))
        db.session.commit()
        ...
        for i in range(20):
            ...
            if not Animals.query.filter_by(record=rec, finca_id=default_finca.id).first():
                ...
                if len(existing) > 2 and random.random() > 0.3:
                    males = [a for a in existing if str(a.sex) == 'Macho' or getattr(a.sex, 'value', '') == 'Macho']
                    females = [a for a in existing if str(a.sex) == 'Hembra' or getattr(a.sex, 'value', '') == 'Hembra']
                    if males: f_id = random.choice(males).id
                    if females: m_id = random.choice(females).id
                
                db.session.add(Animals(
                    record=rec, sex=random.choice(list(Sex)), breeds_id=breed.id,
                    birth_date=date.today() - timedelta(days=random.randint(100, 2000)),
                    weight=random.uniform(40, 600), status=random.choice(list(AnimalStatus)),
        ...
        for i in range(20):
            ...
            if not Medications.query.filter_by(name=m_name).first():
                db.session.add(Medications(
                    name=m_name, description="Antiinflamatorio", dosis="5ml",
                    route_administration_id=route.id, availability=True
                ))
            ...
            if not Vaccines.query.filter_by(name=v_name).first():
                ...
                if t_dis:
                    db.session.add(Vaccines(
                        name=v_name, dosis="2ml", route_administration_id=route.id,
                        vaccination_interval="Anual", type=random.choice(list(VaccineType)),
                        national_plan="Si", target_disease_id=t_dis.id
                    ))
        db.session.commit()
        ...
        for i in range(20):
            ...
            if meds:
                db.session.add(TreatmentMedications(
            ...
            # N:M con Vacunas
            if vacs:
                db.session.add(TreatmentVaccines(
                    treatment_id=treatment.id,
                    vaccine_id=random.choice(vacs).id
                ))
        db.session.commit()
        ...
        res_login = client.post('/api/v1/auth/login', json={"identification": 99999999, "password": "password123"})
        ...
        for name, url in endpoints_to_audit:
            ...
            if status == 200:
                ...
                report.append(f"✅ {name:15} | GET List | {status} | {ms:.1f}ms | Total: {total} | Format: {'OK' if success_flag else 'WARN'}")
            else:
            ...
            if status == 200 and data.get('data'):
                ...
                if res_dep.status_code == 200:
                    ...
                    report.append(f"✅ {name:15} | GET Dep. | {res_dep.status_code} | {ms:.1f}ms | Bloqueos: {dep_data.get('totalDependencies', 0)}")
                else:
                    report.append(f"⚠️ {name:15} | GET Dep. | {res_dep.status_code} | Endpoint no implementado o error")
