...
const BASE_RETRY_DELAY_MS = 20000; // 20 seconds base delay to clear backend's 15s cooldown
const MAX_RETRY_DELAY_MS = 300000; // 5 minutes max
const RATE_LIMIT_BACKOFF_MS = 25000; // 25 seconds base backoff on 429
...
const loadState = () => {
  ...
  try {
    ...
    if (!isRateLimited) {
      // Only restore counters if we are NOT under an active rate limit
...
/**
 * Limpia el estado de rate-limit persistido en localStorage y en memoria.
 * Útil para recuperarse manualmente después de una caída del servidor.
 * También puede llamarse desde la consola: window.__sseReset()
 */
export function resetRateLimit() {
  ...
  saveState();
  console.log('[SSE] Rate limit state cleared. Reconnecting...');
  connectSSE();
}

// Exponer en window para diagnóstico desde consola del navegador
if (typeof window !== 'undefined') {
  (window as any).__sseReset = resetRateLimit;
  (window as any).__sseStatus = getSSEStatus;
}
