...
>(({ className, ...props }, ref) => (
  <div className="relative w-full flex-1 min-h-0 overflow-auto rounded-xl border border-border/70 bg-surface shadow-sm">
    <table
      ref={ref}
      className={cn(
        "w-full caption-bottom text-sm text-left [&_td:last-child]:text-right [&_th:last-child]:text-right text-text-primary",
        className
      )}
      {...props}
    />
