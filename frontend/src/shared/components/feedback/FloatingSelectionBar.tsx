  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed bottom-10 left-1/2 z-[100] w-max max-w-[95vw]"
      >
        <div className={cn(
          "flex items-center gap-2 sm:gap-6 px-4 py-3 sm:px-8 sm:py-4 rounded-[2.5rem]",
          "bg-slate-900/80 dark:bg-slate-900/90 backdrop-blur-3xl border border-white/10",
          "shadow-[0_20px_50px_rgba(0,0,0,0.3)] text-white"
        )}>
          {/* Contador y Estado */}
          <div className="flex items-center gap-3 pr-2 sm:pr-6 border-r border-white/10">
            <div className="h-10 w-10 rounded-[var(--radius-lg)] bg-emerald-500 flex items-center justify-center shadow-[0_0_20px_rgba(16,185,129,0.4)]">
          ...
          <div className="flex items-center gap-1 sm:gap-3">
            {onBulkEdit && (
              <Button
                ...
                size="sm"
                onClick={onBulkEdit}
                className="h-10 px-4 rounded-xl hover:bg-white/10 text-white gap-2 font-bold text-xs uppercase tracking-widest"
              >
                <IconEdit size="sm" />
            ...
            {onDelete && (
              <Button
                ...
                size="sm"
                onClick={onDelete}
                className="h-10 px-4 rounded-xl hover:bg-rose-500 text-white hover:text-white gap-2 font-bold text-xs uppercase tracking-widest bg-rose-500/10"
              >
                <IconTrash size="sm" />
          ...
          <div className="pl-2 sm:pl-6 border-l border-white/10">
            <button
              onClick={onClear}
              className="h-10 w-10 rounded-[var(--radius-full)] hover:bg-white/10 flex items-center justify-center transition-all group"
