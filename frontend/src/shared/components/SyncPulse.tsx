...
import { IconRefresh, IconWifi, IconRadio, IconBolt, IconBattery, IconShieldCheck } from '@/shared/ui/icons';
...
export const SyncPulse: React.FC = () => {
  ...
  useEffect(() => {
    if (!initializedRef.current) {
      ...
      proximitySync.initialize().catch(err => {
  ...
  useEffect(() => {
    ...
    const unsubSync = proximitySync.onSyncStateChange((state) => {
      setSyncState(state);
      if (state.isSyncing) {
        setIsVisible(true);
    ...
    const unsubStatus = proximitySync.onStatusUpdate((msg, type) => {
  ...
  const getStatusColor = () => {
    ...
    switch (statusType) {
      ...
      case 'error': return 'bg-rose-500/90 text-white border-rose-400/50';
      default: return 'bg-slate-900/80 text-white border-slate-700/50';
    }
  };
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={cn(
            "fixed bottom-6 left-6 z-[100001] flex items-center gap-3 px-5 py-3 rounded-[var(--radius-lg)] shadow-[0_20px_50px_rgba(0,0,0,0.3)] border backdrop-blur-2xl transition-all duration-500 overflow-hidden group",
            getStatusColor()
          )}
        >
          ...
          <div className="relative flex items-center justify-center w-8 h-8 rounded-xl bg-white/20 shadow-inner">
            {getStatusIcon()}
          </div>
          ...
          {syncState.isSyncing && (
            <div className="relative flex items-end gap-1 h-3 ml-1">
              <motion.div 
                ...
                className="w-1 bg-white/50 rounded-[var(--radius-full)]" 
              />
              <motion.div 
                animate={{ height: ['60%', '100%', '60%'] }} 
                transition={{ repeat: Infinity, duration: 0.6, delay: 0.2 }}
                className="w-1 bg-white rounded-[var(--radius-full)] shadow-[0_0_8px_rgba(255,255,255,0.5)]" 
              />
              <motion.div 
                animate={{ height: ['40%', '100%', '40%'] }} 
