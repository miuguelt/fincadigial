...
export function useResource<T extends { id?: number | string }, P extends Record<string, any> = Record<string, any>>(
  ...
  const buildEffectiveParams = useCallback((): Record<string, any> | undefined => {
    // Combinar en orden de prioridad: query params URL -> params pasados a refetch -> initialParams -> options.filters
    const base = { ...(initialParams as any), ...(options.filters || {}) };
  ...
  const refetch = useCallback(async (params?: P): Promise<T[]> => {
    ...
    // Reducir throttle para búsquedas (500ms) vs otras operaciones (1500ms)
    const throttleMs = searchQP ? 500 : 1500;
    if (nowTs - lastTs < throttleMs) {
      return data;
    }
        ...
        if (cached && cached.timestamp) {
          ...
          if (isOnline && !__resourceInflight.has(cacheKey)) {
            ...
            const bgPromise = safeExecute(async () => {
              ...
              if (hasPaging && typeof (service as any).getPaginated === 'function') {
  ...
  useEffect(() => {
    if (autoFetch) {
      void refetch(initialParams as P | undefined).catch(() => {});
    }
    return () => {
  ...
  useEffect(() => {
    const unregister = registerResourceRefetch(() => {
      return refetch(undefined as any).catch(() => {});
    });
    return unregister;
  }, [refetch]);
