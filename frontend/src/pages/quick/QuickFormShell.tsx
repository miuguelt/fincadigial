...
export function QuickFormShell({
  ...
  return (
    <div className="min-h-screen bg-background font-sans">
      {/* ── CABECERA COLOREADA ───────────────────────────────────── */}
      <header className={`${colorHeader} px-4 pb-10 pt-6 text-white rounded-b-[var(--radius-xl)] shadow-[var(--shadow-token-lg)] relative overflow-hidden`}>
        <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-[var(--radius-full)] -mr-16 -mt-16 blur-2xl" />
        
        <button
          onClick={() => navigate(volver)}
          className="mb-6 flex items-center gap-2 rounded-[var(--radius-lg)] bg-white/20 px-4 py-2.5 text-[10px] font-black uppercase tracking-widest transition hover:bg-white/30 backdrop-blur-md border border-white/10"
        >
        ...
        <div className="flex items-center gap-4 max-w-lg mx-auto">
          {Icon && (
            <div className="p-4 rounded-[var(--radius-lg)] bg-white/20 backdrop-blur-md border border-white/20 shadow-[var(--shadow-token-md)]">
          ...
          <div>
            ...
            {/* Estado de red */}
            <span
              className={`mt-2 inline-flex items-center gap-1.5 rounded-[var(--radius-full)] px-4 py-1 text-[8px] font-black uppercase tracking-widest ${
                isOnline
                  ? 'bg-white/20 text-white border border-white/10'
                  : 'bg-amber-400 text-amber-900 shadow-[var(--shadow-token-md)]'
              }`}
            >
...
export function QInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`w-full rounded-[var(--radius-lg)] border-2 border-border bg-card px-6 py-5 text-xl font-black text-foreground outline-none transition
    ...
    <div className="grid grid-cols-2 gap-3">
      {options.map((opt) => (
        <button
          ...
          className={`rounded-[var(--radius-lg)] border-2 py-4 px-4 text-xs font-black uppercase tracking-tight transition-all active:scale-95 ${
            value === opt.value
              ? `border-primary bg-primary text-primary-foreground shadow-[var(--shadow-token-md)] shadow-primary/20`
  ...
  return (
    <div className="relative">
      <select
        ...
        className="w-full rounded-[var(--radius-lg)] border-2 border-border bg-card px-6 py-5 text-lg font-black text-foreground outline-none transition focus:border-primary disabled:opacity-50 appearance-none shadow-[var(--shadow-token-sm)]"
      >
        {placeholder && (
...
export function QSubmitButton({
  ...
  return (
    <button
      type="submit"
      disabled={loading}
      className={`w-full rounded-[var(--radius-xl)] ${color} py-6 text-base font-black uppercase tracking-[0.2em] text-white shadow-[var(--shadow-token-lg)] transition-all
...
export function QCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-[var(--radius-xl)] bg-card p-6 shadow-[var(--shadow-token-md)] border border-border">
      {children}
    </div>
  );
}
