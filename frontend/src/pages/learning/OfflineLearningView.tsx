...
import { IconBook, IconFileText, IconDownload, IconPlus, IconSearch, IconFilter, IconPlayerPlay, IconCircleCheck } from '@/shared/ui/icons';
...
export default function OfflineLearningView() {
  ...
  useEffect(() => {
    ...
    if (userRole === 'Instructor' || userRole === 'Administrador' || userRole === 'Propietario') {
      setIsAdmin(true);
    }
  }, [toast]);
  ...
  const getIcon = (type: string) => {
    if (type === 'PDF') return <IconFileText className="w-8 h-8 text-red-500" />;
    if (type === 'VIDEO') return <IconPlayerPlay className="w-8 h-8 text-blue-500" />;
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Header Section */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-emerald-900 to-emerald-700 p-8 rounded-[var(--radius-lg)] shadow-[var(--shadow-token-lg)] text-white">
        <div>
        ...
        <div className="flex gap-2">
          {isAdmin && (
            <Button 
              variant="secondary" 
              className="bg-white text-emerald-900 hover:bg-emerald-50 font-semibold shadow-[var(--shadow-token-md)]"
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="relative w-full md:w-96">
          <IconSearch size="md" className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" />
          <Input 
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 w-full md:w-auto">
          <Badge variant="outline" className="cursor-pointer hover:bg-emerald-50 border-emerald-500 text-emerald-700 text-sm px-4 py-1">Todos</Badge>
          <Badge variant="outline" className="cursor-pointer hover:bg-emerald-50 text-sm px-4 py-1">Sanidad Animal</Badge>
          <Badge variant="outline" className="cursor-pointer hover:bg-emerald-50 text-sm px-4 py-1">Agricultura</Badge>
          <Badge variant="outline" className="cursor-pointer hover:bg-emerald-50 text-sm px-4 py-1">PDFs</Badge>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map(i => (
            <Card key={i} className="animate-pulse border-emerald-100 h-64 bg-gray-50/50" />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredMaterials.map(mat => (
            <Card key={mat.id} className="group hover:shadow-[var(--shadow-token-lg)] transition-all duration-300 border-emerald-100 flex flex-col relative overflow-hidden">
              {cachedMap[mat.id] && (
                <div className="absolute top-0 right-0 p-1 bg-emerald-500 text-white rounded-bl-xl shadow-[var(--shadow-token-md)] z-10 animate-in fade-in slide-in-from-top-1">
              ...
              <CardContent className="flex-1">
                ...
                <div className="flex items-center gap-4 text-xs text-gray-500 font-medium">
                  <span className="flex items-center gap-1 bg-gray-100 px-2 py-1 rounded-md">Nivel: {mat.reading_level || 'General'}</span>
                  <span className="flex items-center gap-1 bg-gray-100 px-2 py-1 rounded-md">{mat.content_type}</span>
                </div>
              </CardContent>
              <CardFooter className="pt-4 border-t bg-gray-50/50 rounded-b-xl flex justify-between items-center gap-2">
                {cachedMap[mat.id] ? (
                  <Button 
                    variant="outline" 
                  ...
                  <Button 
                    variant="default" 
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white shadow-[var(--shadow-token-md)]"
      ...
      {!isLoading && filteredMaterials.length === 0 && (
        <div className="text-center py-20 bg-emerald-50/50 rounded-[var(--radius-lg)] border-2 border-dashed border-emerald-200">
