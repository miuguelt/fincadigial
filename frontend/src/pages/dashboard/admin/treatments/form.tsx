...
/**
 * Redirecciona al flujo modal de AdminTreatmentsPage conforme al estándar de UI y Protocolo GEMINI.md:
...
export default function TreatmentFormPage() {
  ...
  useEffect(() => {
    const basePath = rolePath('/admin/treatments');
    if (id) {
      navigate(`${basePath}?edit=${id}`, { replace: true });
    } else {
      navigate(`${basePath}?create=true`, { replace: true });
    }
  ...
  return (
    <div className="flex items-center justify-center min-h-[50vh]">
      <div className="text-center space-y-2 animate-pulse">
        ...
        <p className="text-muted-foreground text-xs font-semibold">Abriendo formulario de tratamiento...</p>
      </div>
    </div>
  );
}
