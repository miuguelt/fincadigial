...
interface FertilityDashboardProps { isEmbedded?: boolean }
...
function FemaleRankingList({ title, description, items, tone, onAnimalClick }: {
  title: string;
  description: string;
  items: FemaleRanking[];
  tone: 'success' | 'danger';
      ...
      <CardContent className="space-y-2 p-3 sm:p-4">
        {items.length === 0 ? <p className="py-4 text-center text-xs text-muted-foreground">No hay datos suficientes en este período.</p> : items.map((female, index) => (
          <button
            ...
            className="flex min-h-14 w-full items-center gap-3 rounded-xl border border-border/50 bg-muted/20 p-3 text-left transition-colors hover:border-primary/40 hover:bg-primary/[0.04] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          ...
          >
            ...
            <span className={`${tone === 'success' ? 'text-emerald-600' : 'text-rose-600'} shrink-0 text-sm font-black`}>{female.rate}%</span>
...
function AttentionCard({ definition, entries, onAnimalClick }: {
      ...
      <CardContent className="space-y-2 p-3 pt-1">
        {entries.slice(0, 4).map((entry) => (
          <button
            ...
            className="flex min-h-12 w-full items-center gap-2 rounded-xl bg-background/70 px-3 py-2 text-left transition-colors hover:bg-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
            aria-label={`Abrir ficha de ${entry.record}`}
          >
            <span className="min-w-0 flex-1"><span className="fit-clamp block text-sm font-bold text-primary">{entry.record}</span><span className="fit-clamp block text-[11px] text-muted-foreground">{formatAttentionDetail(definition.key, entry)}</span></span>
            <ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground" />
          </button>
        ))}
    ...
    <div className={isEmbedded ? 'space-y-4' : 'min-h-full space-y-5 overflow-x-hidden p-4 sm:p-6 lg:p-8'}>
      {!isEmbedded ? (
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0"><h3 className="flex items-center gap-2 text-base font-bold"><Heart className="h-4 w-4 text-pink-600" /> Fertilidad y decisiones del día</h3><p className="mt-1 text-xs text-muted-foreground">El semáforo y las listas de atención vienen de la unidad de servicio reproductiva.</p></div>
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <KpiMetricCard label={pregnancyLabel} value={pregnancyRate} unit="%" target={kpiData?.targets.pregnancy_rate_pct?.target ?? null} status={kpiData ? kpiData.status.pregnancy_rate_pct : conceptionStatus} sample={efficiency?.resolved_services} hint="Detección de celo × concepción." icon={pregnancyRate !== null && pregnancyRate >= 50 ? <TrendingUp className="h-4 w-4 text-emerald-600" /> : <TrendingDown className="h-4 w-4 text-amber-600" />} />
      ...
      {attentionLists.length > 0 ? <section className="space-y-3"><div className="flex items-end justify-between gap-3"><div><h2 className="text-base font-black sm:text-lg">Qué revisar hoy</h2><p className="text-xs text-muted-foreground">Toca un animal para abrir su ficha completa.</p></div><Badge variant="outline" className="hidden shrink-0 sm:inline-flex">{attentionCount} pendientes</Badge></div><div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">{attentionLists.map(({ definition, entries }) => <AttentionCard key={definition.key} definition={definition} entries={entries} onAnimalClick={openAnimal} />)}</div></section> : <Card className="rounded-2xl border-emerald-500/25 bg-emerald-500/[0.04]"><CardContent className="flex items-start gap-3 p-4"><div className="rounded-xl bg-emerald-500/15 p-2 text-emerald-600"><ClipboardCheck className="h-5 w-5" /></div><div><p className="text-sm font-bold">Sin seguimientos reproductivos prioritarios</p><p className="mt-1 text-xs text-muted-foreground">El sistema no encontró partos vencidos, servicios atrasados ni hembras repetidoras en este corte.</p></div></CardContent></Card>}
      ...
      <section className="grid grid-cols-1 gap-3 xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <Card className="rounded-2xl"><CardHeader className="p-4 pb-2 sm:p-5 sm:pb-2"><CardTitle className="flex items-center gap-2 text-base"><Sparkles className="h-4 w-4 text-blue-600" /> Resultado por técnica</CardTitle><CardDescription className="text-xs">Ayuda a decidir entre monta natural e inseminación artificial</CardDescription></CardHeader><CardContent className="space-y-5 p-4 sm:p-5"><TechniqueBar label="Monta natural" value={data?.conception_by_technique.natural ?? 0} color="#3b82f6" /><TechniqueBar label="Inseminación artificial" value={data?.conception_by_technique.artificial ?? 0} color="#10b981" /><p className="rounded-xl bg-muted/35 p-3 text-[11px] leading-relaxed text-muted-foreground">La tasa se interpreta sobre servicios con desenlace. Un servicio pendiente no se cuenta como fracaso hasta que vence su ventana biológica.</p></CardContent></Card>
      </section>

      <section className="grid grid-cols-1 gap-3 lg:grid-cols-2"><FemaleRankingList title="Hembras con mejor respuesta" description="Útiles para identificar animales y manejos que están funcionando" items={data?.top_females ?? []} tone="success" onAnimalClick={openAnimal} /><FemaleRankingList title="Hembras para revisar" description="No es descarte automático: abre la ficha y cruza sanidad, peso y servicios" items={data?.bottom_females ?? []} tone="danger" onAnimalClick={openAnimal} /></section>
