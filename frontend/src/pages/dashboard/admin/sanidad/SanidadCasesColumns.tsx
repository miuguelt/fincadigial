...
function severityBadge(v: string) {
  if (!v) return '-';
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[11px] font-bold border ${palette[String(v)] || 'bg-muted/10 text-muted-foreground border-border/30'}`}>
      {String(v)}
    </span>
  );
}
