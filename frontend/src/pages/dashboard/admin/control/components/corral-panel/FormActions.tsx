...
export function FormActions({ form, onClose }: FormActionsProps) {
	return (
		<div className="sticky bottom-0 z-20 -mx-4 flex flex-col gap-3 border-t border-gray-200 bg-white/95 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-[0_-8px_20px_-12px_rgba(15,23,42,0.45)] backdrop-blur sm:static sm:mx-0 sm:flex-row sm:bg-transparent sm:p-0 sm:pt-4 sm:shadow-none">
			{onClose && (
				<Button
					type="button"
					variant="outline"
					onClick={onClose}
					className="h-14 w-full border-2 border-gray-300 text-base text-gray-700 sm:w-1/3"
				>
					Cerrar sin guardar
				</Button>
			)}
