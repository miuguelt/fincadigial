...
import { 
  ...
  IconShoppingBag, IconCash, IconSwitchHorizontal,
  IconCalendar, IconShoppingCart, IconMeat, IconDroplet, 
  IconActivity, IconInfoCircle, IconFileText
...
const FarmWallet: React.FC = () => {
  ...
  const getIcon = (category: string) => {
    switch (category) {
      case 'Producción': return <IconDroplet size="lg" className="text-blue-500" />;
      case 'Insumos': return <IconShoppingCart size="lg" className="text-amber-500" />;
      case 'Salud': return <IconActivity size="lg" className="text-red-500" />;
      case 'Ganado': return <IconMeat size="lg" className="text-emerald-500" />;
      default: return <IconCash size="lg" className="text-slate-500" />;
    }
  };
    ...
    <div className="p-4 md:p-8 bg-slate-50 min-h-screen">
      <header className="mb-10 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex-1 text-center md:text-left">
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">Mi Monedero</h1>
          <p className="text-slate-500 font-bold mt-1 uppercase tracking-widest text-xs">Finca Villa Luz</p>
        </div>
        <button 
          onClick={() => window.open('/api/analytics/reports_pro/financial-statement', '_blank')}
          className="flex items-center gap-2 bg-slate-900 text-white px-6 py-3 rounded-[var(--radius-lg)] font-black text-sm shadow-[var(--shadow-token-lg)] hover:bg-black transition-all active:scale-95"
        >
      ...
      <div className="max-w-2xl mx-auto mb-12">
        <motion.div 
          ...
          className="bg-slate-900 p-10 rounded-[3rem] text-white shadow-[var(--shadow-token-lg)] shadow-slate-200 relative overflow-hidden"
        >
          <div className="relative z-10">
            <p className="text-slate-400 font-black uppercase tracking-[0.2em] text-xs mb-4">Saldo Disponible</p>
            <h2 className="text-6xl font-black tracking-tight">$ 1.400.000</h2>
            <div className="flex gap-4 mt-8">
               <div className="bg-success-500/20 px-4 py-2 rounded-[var(--radius-lg)] flex items-center gap-2 border border-success-500/30">
                  <IconTrendingUp size="sm" className="text-success-400" />
                  <span className="text-xs font-black text-success-400">+$ 1.6M</span>
               </div>
               <div className="bg-danger-500/20 px-4 py-2 rounded-[var(--radius-lg)] flex items-center gap-2 border border-danger-500/30">
      ...
      <div className="grid grid-cols-2 gap-4 max-w-2xl mx-auto mb-12">
        <button className="bg-card p-6 rounded-[2.5rem] border-2 border-success-500/20 flex flex-col items-center gap-2 shadow-sm hover:bg-success-500/5 transition-all active:scale-95 group">
          <div className="p-4 bg-success-500 rounded-[var(--radius-lg)] text-white shadow-[var(--shadow-token-md)] group-hover:scale-110 transition-transform">
        ...
        <button className="bg-card p-6 rounded-[2.5rem] border-2 border-danger-500/20 flex flex-col items-center gap-2 shadow-sm hover:bg-danger-500/5 transition-all active:scale-95 group">
          <div className="p-4 bg-danger-500 rounded-[var(--radius-lg)] text-white shadow-[var(--shadow-token-md)] group-hover:scale-110 transition-transform">
      ...
      {/* Transaction List */}
      <div className="max-w-2xl mx-auto bg-white rounded-[3rem] p-8 shadow-sm border border-slate-100">
        <div className="flex justify-between items-center mb-8">
          <h3 className="text-xl font-black text-slate-800">Movimientos</h3>
          <div className="flex bg-slate-100 p-1 rounded-[var(--radius-full)]">
        ...
        <div className="space-y-4">
          {transactions.filter(t => activeTab === 'all' || activeTab === t.type).map((t) => (
            <div key={t.id} className="flex items-center justify-between p-4 rounded-[var(--radius-xl)] hover:bg-slate-50 transition-colors group">
              <div className="flex items-center gap-4">
                <div className="p-4 bg-slate-50 rounded-[var(--radius-lg)] group-hover:bg-white transition-colors">
                  {getIcon(t.category)}
                </div>
                <div>
        ...
        <button className="w-full mt-8 py-4 text-slate-400 font-bold text-sm hover:text-slate-600 flex items-center justify-center gap-2">
