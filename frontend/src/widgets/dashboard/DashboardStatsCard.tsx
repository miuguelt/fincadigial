  ...
  const cardClassName = useMemo(
    () =>
      `hover:shadow-lg transition-shadow duration-200 ${
        onClick
          ? "cursor-pointer hover:brightness-[0.98] will-change-[filter]"
          : ""
      } ${className}`,
    [onClick, className],
  );
...
export const DashboardStatsCard = memo(
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      prevProps.stat?.tendencia?.periodo_actual ===
        nextProps.stat?.tendencia?.periodo_actual &&
      prevProps.stat?.tendencia?.periodo_anterior ===
        nextProps.stat?.tendencia?.periodo_anterior &&
