...
export const FieldActionsMenu: React.FC<FieldActionsMenuProps> = ({
  field,
}) => {
  ...
  const [targetFieldId, setTargetFieldId] = useState<number | undefined>(
    undefined,
  );
  ...
  const loadOptions = useCallback(async () => {
    try {
      const animals = await animalsService
        .getAnimals({ page: 1, limit: 1000 })
        .catch(() => []);
      const animalsData = animals || [];
      setAnimalOptions(
        animalsData.map((a: any) => ({
          value: a.id,
      ...
      const fields = await fieldService
  ...
  const renderFormContent = () => {
    const inputClass =
      "w-full px-3 py-2 text-sm rounded-md border border-input bg-background ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-colors";
    const labelClass = "block text-sm font-medium mb-1.5 text-foreground";
    const selectClass =
      "w-full px-3 py-2 text-sm rounded-md border border-input bg-background ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-colors";
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setFormData({ ...formData, animal_id: parseInt(e.target.value) })
            }
    ...
    <>
        ...
        <div className="space-y-4">
          <div className="p-4 bg-muted/50 rounded-lg border border-border">
            <p className="text-sm text-muted-foreground">
              Esta acción marcará el animal como retirado del potrero con la
              fecha de hoy. El animal ya no contará en la ocupación del potrero.
            </p>
          </div>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="p-4 bg-muted/50 rounded-lg border border-border">
            <p className="text-sm text-muted-foreground">
              Esta acción retirará el animal del potrero actual y lo asignará
              automáticamente al potrero seleccionado con la fecha de hoy.
            </p>
          </div>
