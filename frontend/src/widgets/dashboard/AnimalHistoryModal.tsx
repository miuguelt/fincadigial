...
import { Clock, Activity, MapPin, Stethoscope, Syringe, Calendar, Shield, HeartPulse, TreePine, AlertTriangle, CheckCircle2, Leaf } from "lucide-react";
import {
  Modal,
  ModalContent,
  ModalHeader,
...
export const AnimalHistoryModal = ({
  ...
  const { data: animalDiseases, refetch: refetchDiseases } =
    useAnimalDiseases();
  const { data: animalFields, fetchAnimalFields: refetchFields } = useAnimalFields();
  ...
  // --- Calcular datos rápidos para el banner de resumen ---
  const currentField = fieldRows.find((r) => r.status === "Asignado" || r.is_active);
  const activeDiseases = diseaseRows.filter(
    (r) => r.status?.toLowerCase().includes("activ") || r.status?.toLowerCase().includes("pendiente")
  );
  const lastControl = controlRows[0];
  const animalAge = (() => {
        ...
        <ModalContent className="flex flex-col overflow-hidden">
          <ModalBody className="flex-1 overflow-hidden p-0">
            <div className="flex flex-col h-full">
              ...
              {/* ─── BANNER DE RESUMEN DEL ANIMAL ─── */}
              <div className="shrink-0 px-4 py-3 bg-gradient-to-r from-primary/5 via-primary/3 to-transparent border-b border-border/50">
                <div className="flex flex-wrap items-center gap-3 md:gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-bold text-foreground">{animal.record || `#${animal.idAnimal}`}</span>
                      {animal.breed?.name && (
                      ...
                      {activeDiseases.length > 0 && (
                        <span className="inline-flex items-center gap-1 text-xs font-medium bg-destructive/10 text-destructive px-2 py-0.5 rounded-full">
                          ...
                          {activeDiseases.length} enfermedad{activeDiseases.length > 1 ? "es" : ""} activa{activeDiseases.length > 1 ? "s" : ""}
                        </span>
                      )}
                      {activeDiseases.length === 0 && (
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                    ...
                    {lastControl?.weight && (
                      <div className="flex items-center gap-1">
                        ...
                        <span className="font-semibold text-foreground">{lastControl.weight} kg</span>
                      </div>
                    )}
                    {currentField ? (
                      <div className="flex items-center gap-1">
                        <Leaf className="w-3.5 h-3.5 text-emerald-500" />
                        <span className="text-emerald-600 dark:text-emerald-400 font-medium">{currentField.field}</span>
                      </div>
                    ) : (
              ...
              {historyLoading ? (
                ...
                <div className="flex flex-col justify-center items-center py-8 gap-2 text-destructive">
                  <AlertTriangle className="w-8 h-8" />
                  <span>Error al cargar: {historyError}</span>
                </div>
              ) : (
                // --- Tabs: Tratamientos, Potreros, Enfermedades, Controles, Reproducción ---
                <div className="flex flex-col h-full">
                  <Tabs
                  ...
                  >
                    ...
                    >
                      {timelineEvents.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
                          ...
                          <div>
                            <p className="font-semibold text-foreground mb-1">Sin eventos registrados</p>
                            <p className="text-sm text-muted-foreground">Aún no hay actividad en la línea de tiempo de este animal.</p>
                          </div>
                        </div>
                      ) : (
                    ...
                    >
                      {treatmentRows.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
                          ...
                          <div>
                            <p className="font-semibold text-foreground mb-1">Sin tratamientos registrados</p>
                            <p className="text-sm text-muted-foreground">No se han registrado tratamientos ni vacunaciones para este animal.</p>
                          </div>
                        </div>
                      ) : (
                        <div className="space-y-3">
                                    ...
                                    <span
                                      className={`inline-flex px-2 py-0.5 rounded-md text-xs font-medium ${
                                        r.status
                                          ?.toLowerCase()
                                          .includes("complet")
                                          ? "bg-success/10 text-success dark:bg-green-900/30 dark:text-green-100"
                                          : r.status
                                                ?.toLowerCase()
                                                .includes("activ") ||
                                              r.status
                                                ?.toLowerCase()
                                                .includes("progress")
                                            ? "bg-info/10 text-info dark:bg-blue-900/30 dark:text-blue-100"
                                            : "bg-muted text-foreground/80 dark:bg-card dark:text-foreground"
                                      }`}
                                    >
                                      {r.status}
                                    </span>
                      ...
                      {fieldRows.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
                          <div className="w-16 h-16 rounded-full bg-amber-50 dark:bg-amber-950/30 flex items-center justify-center">
                            <TreePine className="w-8 h-8 text-amber-300" />
                          </div>
                          <div>
                            <p className="font-semibold text-foreground mb-1">Sin movimientos de campo</p>
                            <p className="text-sm text-muted-foreground max-w-xs">
                              Este animal no ha sido asignado a ningún potrero. Usa el menú de acciones para asignarlo.
                            </p>
                          </div>
                        </div>
                      ) : (
                        // Línea de tiempo visual de potreros
                        <div className="relative">
                          {/* Línea vertical conectora */}
                          <div className="absolute left-[1.85rem] top-2 bottom-2 w-0.5 bg-border/60 z-0" />
                          <div className="space-y-2">
                            {fieldRows.map((r, idx) => {
                              const isActive = !r.removal;
                              return (
                                <div
                                  key={`fi-${idx}`}
                                ...
                                >
                                  {/* Nodo en la línea */}
                                  <div className={`z-10 shrink-0 w-[3.7rem] flex justify-center pt-3`}>
                                    <div
                                      className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-transform group-hover:scale-110 ${
                                        isActive
                                          ? "bg-emerald-500 border-emerald-400 shadow-[0_0_8px_rgba(16,185,129,0.4)]"
                                          : "bg-muted border-border"
                                      }`}
                                    >
                                      <Leaf className={`w-3.5 h-3.5 ${ isActive ? "text-white" : "text-muted-foreground"}`} />
                                    </div>
                                  </div>
                                  {/* Tarjeta */}
                                  <div
                                    className={`flex-1 mb-2 rounded-xl border transition-shadow group-hover:shadow-md ${
                                      isActive
                                  ...
                                  >
                                    <div className="p-3">
                                      <div className="flex items-start justify-between gap-2 mb-2">
                                        <div className="flex items-center gap-2">
                                          <MapPin className={`h-4 w-4 shrink-0 ${ isActive ? "text-emerald-500" : "text-muted-foreground"}`} />
                                          <h3 className="font-semibold text-foreground leading-tight">{r.field}</h3>
                                        </div>
                                        <span
                                          className={`inline-flex shrink-0 items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${
                                            isActive
                                      ...
                                      <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                                        <div className="flex items-center gap-1">
                                          ...
                                          <span>Ingreso: <span className="text-foreground font-medium">{r.assignment ? formatDate(r.assignment) : "—"}</span></span>
                                        </div>
                                        {r.removal && (
                                          <div className="flex items-center gap-1">
                                            <Calendar className="w-3 h-3" />
                                            <span>Retiro: <span className="text-foreground font-medium">{formatDate(r.removal)}</span></span>
                                          </div>
                                        )}
                                        {r.duration_days !== "" && (
                                          <div className="flex items-center gap-1">
                                            <Clock className="w-3 h-3" />
                                            <span><span className="text-foreground font-medium">{r.duration_days}</span> días</span>
                                          </div>
                                        )}
                                      </div>
                                      {r.notes && r.notes !== "—" && (
                    ...
                    >
                      {diseaseRows.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
                          ...
                          <div>
                            <p className="font-semibold text-foreground mb-1">¡Sin enfermedades registradas!</p>
                            <p className="text-sm text-muted-foreground">Este animal tiene un historial sanitario limpio.</p>
                          </div>
                        </div>
                      ) : (
                        <div className="space-y-3">
                          {diseaseRows.map((r, idx) => {
                            const isActive = r.status?.toLowerCase().includes("activ") || r.status?.toLowerCase().includes("pendiente");
                            const isResolved = r.status?.toLowerCase().includes("curado") || r.status?.toLowerCase().includes("recovered") || r.status?.toLowerCase().includes("resuelto");
                            return (
                            <div
                              key={`di-${idx}`}
                              className={`p-4 rounded-lg border-l-4 bg-card hover:shadow-md transition-shadow cursor-pointer ${
                                isActive ? "border-l-destructive" : isResolved ? "border-l-success" : "border-l-muted-foreground/30"
                              }`}
                              onClick={() => openPreview("disease", r)}
                            >
                              <div className="flex items-start justify-between mb-3">
                                <div className="flex items-center gap-2">
                                  <Stethoscope className={`h-5 w-5 ${ isActive ? "text-destructive" : isResolved ? "text-success" : "text-muted-foreground"}`} />
                                    ...
                                    <span
                                      className={`inline-flex px-2 py-0.5 rounded-md text-xs font-medium ${
                                        ...
                                        r.severity
                                          ?.toLowerCase()
                                          .includes("sever")
                                          ? "bg-destructive/10 text-destructive dark:bg-red-900/30 dark:text-red-100"
                                          : r.severity
                                                ?.toLowerCase()
                                                .includes("moderado")
                                            ? "bg-warning/10 text-warning dark:bg-amber-900/30 dark:text-amber-100"
                                            : "bg-muted text-foreground/80 dark:bg-card dark:text-foreground"
                                      }`}
                                    >
                                      {r.severity}
                    ...
                    >
                      {controlRows.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
                          <div className="w-16 h-16 rounded-full bg-teal-50 dark:bg-teal-950/30 flex items-center justify-center">
                            <Activity className="w-8 h-8 text-teal-300" />
                          </div>
                          <div>
                            <p className="font-semibold text-foreground mb-1">Sin controles registrados</p>
                            <p className="text-sm text-muted-foreground">No hay controles de peso ni salud registrados para este animal.</p>
                          </div>
                        </div>
                      ) : (
                        <div className="space-y-3">
