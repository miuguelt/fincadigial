...
export const AnimalFieldForm: React.FC<Props> = ({ formData, setFormData, idPrefix, fieldOptions }) => (
  <div className="space-y-4">
    <div>
      <label htmlFor={`${idPrefix}-field`} className={labelClass}>Potrero de Destino *</label>
    ...
    <div className="grid grid-cols-2 gap-4">
      <div>
        <label htmlFor={`${idPrefix}-assigndate`} className={labelClass}>Fecha de Asignación / Traslado *</label>
      ...
      <div>
        <label htmlFor={`${idPrefix}-removenate`} className={labelClass}>Fecha de Retiro (Opcional)</label>
    ...
    <div>
      <label htmlFor={`${idPrefix}-notes`} className={labelClass}>Notas / Observaciones del Traslado</label>
      <textarea id={`${idPrefix}-notes`} placeholder="Observaciones sobre el potrero o condición del pastoreo..." value={formData.notes || ""} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} rows={2} className={inputClass} />
    </div>
  </div>
);
