...
export function LivestockTagWidget({ animal }: LivestockTagWidgetProps) {
  ...
  return (
    <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-950 rounded-[2rem] border border-border/50 shadow-xl overflow-hidden p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-black uppercase tracking-widest text-foreground">Identificación Animal</h3>
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Digital Tagging v2.5</p>
          </div>
        </div>

        <div className="flex bg-slate-100 dark:bg-white/5 p-1 rounded-xl border border-border/50">
          <button
            onClick={() => setMode('qr')}
            className={cn(
              "px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all",
              mode === 'qr' ? "bg-white dark:bg-slate-800 text-primary shadow-sm" : "text-muted-foreground hover:text-foreground"
            )}
          >
            QR
          </button>
          <button
            ...
            className={cn(
              "px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all",
              mode === 'nfc' ? "bg-white dark:bg-slate-800 text-primary shadow-sm" : "text-muted-foreground hover:text-foreground"
            )}
          >
            NFC
          </button>
      ...
      <div className="flex flex-col items-center justify-center min-h-[220px]">
        <AnimatePresence mode="wait">
          {mode === 'qr' ? (
            ...
            >
              <div className="p-4 bg-white rounded-3xl shadow-inner border-8 border-slate-100 dark:border-white/5">
              ...
              <Button 
                variant="outline" 
                size="sm" 
                className="rounded-xl font-black uppercase tracking-widest text-[10px] h-10 px-6 gap-2 border-primary/20 text-primary hover:bg-primary/10"
                onClick={downloadQR}
              >
