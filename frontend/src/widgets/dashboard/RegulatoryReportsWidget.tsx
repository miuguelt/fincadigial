...
const RegulatoryReportsWidget: React.FC = () => {
  ...
  const downloadReport = async (
    type: "inventory" | "movements" | "health",
    format: "pdf" | "csv",
  ) => {
    ...
    <Card className="border-none shadow-2xl shadow-primary/5 bg-card/40 backdrop-blur-xl rounded-[2.5rem] overflow-hidden">
      <CardHeader className="p-8 border-b border-white/5 bg-gradient-to-r from-primary/5 to-transparent">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 font-semibold text-sm h-8 px-4">
      ...
      <CardContent className="p-8 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {reports.map((report) => (
            <motion.div
              ...
              className={cn(
                "group relative p-6 rounded-xl border border-white/5 bg-gradient-to-br transition-all duration-300",
                report.gradient,
              )}
            >
              <div className="flex flex-col gap-4">
                <div
                  className={cn(
                    "h-12 w-12 rounded-lg flex items-center justify-center bg-card shadow-sm group-hover:scale-110 transition-transform",
                    report.iconColor,
                  )}
                >
                  {report.icon}
              ...
              <div className="flex gap-2 mt-6">
                {report.formats.map((format) => (
                  <Button
                    key={format}
                    variant="secondary"
                    className="h-10 flex-1 rounded-xl font-semibold text-sm gap-2 border border-white/5 hover:bg-primary hover:text-white transition-all"
                    disabled={!!loading}
        ...
        <div className="relative p-6 rounded-xl bg-warning/5 border border-warning/10 overflow-hidden">
          <div className="flex items-start gap-4 relative z-10">
            ...
            <div>
              ...
              <p className="text-xs text-warning/80 leading-relaxed font-medium">
                Estos reportes están diseñados para cumplir con los
                requerimientos técnicos del ICA. Asegúrese de validar la{" "}
                <strong>trazabilidad completa</strong> y los{" "}
                <strong>registros de vacunación</strong> antes de proceder con
                el reporte final.
              </p>
            </div>
          </div>
          {/* Decorative blur */}
          <div className="absolute -bottom-10 -right-10 h-32 w-32 bg-warning/10 rounded-full blur-3xl" />
        </div>
      </CardContent>
    </Card>
  );
