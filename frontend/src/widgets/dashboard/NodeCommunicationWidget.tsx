...
export const NodeCommunicationWidget: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubMsg = proximitySync.onMessageReceived((msg) => {
  ...
  const getStatusColor = (peer: DiscoveredPeer) => {
    if (peer.isConnected) return "bg-green-500";
    return "bg-blue-500";
  };

  return (
    <div
      className={`rounded-[var(--radius-xl)] p-6 border shadow-[var(--shadow-token-lg)] overflow-hidden relative ${isFieldMode ? "bg-black border-white text-white" : "bg-card border-border"}`}
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div
            className={`p-3 rounded-[var(--radius-lg)] ${isFieldMode ? "bg-zinc-800" : "bg-indigo-50 text-indigo-600"}`}
          >
            <IconRadio
              size="lg"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-[400px]">
        ...
        <div className="flex flex-col h-full">
          <h4 className="text-[10px] font-black uppercase tracking-widest mb-3 opacity-50">
            Compañeros Cercanos
          </h4>
          <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
            {nodes.length === 0 ? (
              <div
                className={`h-full flex flex-col items-center justify-center rounded-[var(--radius-lg)] border-2 border-dashed ${isFieldMode ? "bg-zinc-900 border-zinc-800 text-zinc-600" : "bg-muted border-border text-muted-foreground"}`}
              >
                <IconWaveSine
                  size="lg"
                  strokeWidth={1}
              ...
              nodes.map((node) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-4 rounded-[var(--radius-lg)] transition-all border-2 ${selectedNode === node.id ? (isFieldMode ? "bg-card border-white text-black" : "bg-indigo-600 border-indigo-600 text-white shadow-[var(--shadow-token-md)] scale-[1.02]") : isFieldMode ? "bg-zinc-900 border-zinc-800 hover:border-zinc-700" : "bg-muted border-slate-50 hover:border-border"}`}
                >
                  <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col h-full">
          ...
          <div
            className={`flex-1 rounded-[var(--radius-lg)] p-4 mb-4 flex flex-col ${isFieldMode ? "bg-zinc-900" : "bg-muted"}`}
          >
              ...
              {messages.length === 0 ? (
                ...
                messages.map((msg) => (
                  <motion.div
                    ...
                    className={`p-3 rounded-xl ${msg.type === "alert" ? (isFieldMode ? "bg-red-600 text-white" : "bg-red-50 text-red-700 border border-red-100") : isFieldMode ? "bg-zinc-800 text-white" : "bg-card text-foreground border border-border shadow-sm"}`}
                  >
          ...
          <div className="space-y-2">
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-3 rounded-xl flex items-center justify-center gap-2 transition-all font-black text-xs uppercase tracking-tighter ${selectedNode ? "bg-red-600 text-white hover:bg-red-700 shadow-[var(--shadow-token-md)] shadow-red-600/20 active:scale-95" : "bg-zinc-800 text-zinc-600 cursor-not-allowed opacity-50"}`}
              >
      ...
      <AnimatePresence>
        {showNotification && (
          <motion.div
            ...
            className={`absolute bottom-6 left-6 right-6 p-4 rounded-[var(--radius-lg)] shadow-[var(--shadow-token-lg)] z-50 border-2 flex items-center gap-4 ${showNotification.type === "alert" ? "bg-red-600 border-white text-white" : isFieldMode ? "bg-zinc-900 border-indigo-500 text-white" : "bg-card border-indigo-100 text-foreground"}`}
          >
            <div
