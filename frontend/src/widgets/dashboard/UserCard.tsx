...
export const UserCard: React.FC<UserCardProps> = ({
  ...
  return (
    <Card
      className={`h-full group hover:shadow-lg transition-all duration-300 border-l-4 ${user.isActive ? "border-l-green-500" : "border-l-gray-300"} ${className}`}
    >
      <CardContent className="p-5 h-full flex flex-col">
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3
                className="font-bold text-lg leading-tight text-foreground fit-clamp max-w-[150px]"
                title={user.username}
              >
                {user.username}
              </h3>
        ...
        <div className="space-y-2.5 text-sm text-muted-foreground flex-1">
          <div className="flex items-center gap-2.5 bg-muted/50/50 p-1.5 rounded-md hover:bg-muted/50 transition-colors">
            <Mail className="h-4 w-4 text-muted-foreground shrink-0" />
            <span className="fit-clamp flex-1" title={user.email}>
        ...
        <div className="mt-auto pt-3 border-t border-border/50 flex justify-end">
          <Button
            variant="outline"
            size="sm"
            className="w-full text-xs font-medium h-8 bg-transparent hover:bg-muted/50 border-border"
            onClick={() => onView?.(user)}
          >
            Ver Perfil
          </Button>
