...
import { Calendar as CalendarIcon, ChevronLeft, ChevronRight, AlertCircle, Syringe, Pill, HeartPulse, Milk } from 'lucide-react';
      ...
      <CardHeader className="bg-gray-50/50 pb-4 border-b">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="text-xs font-medium text-gray-500 mt-1 uppercase tracking-wider">
      ...
      <CardContent className="p-0">
        {sortedDates.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground text-sm flex flex-col items-center">
            ...
            <p className="font-medium text-gray-600">No hay eventos programados en este período</p>
          </div>
        ) : (
          <div className="max-h-[500px] overflow-y-auto bg-gray-50/30">
            {sortedDates.map((date) => {
              ...
              return (
                <div key={date} className={`border-b last:border-0 ${isToday ? 'bg-blue-50/30' : ''}`}>
                  <div className="flex">
                    <div className={`w-24 flex-shrink-0 p-4 text-center border-r ${isToday ? 'border-blue-100' : 'border-gray-100'}`}>
                      <div className={`text-xs font-medium uppercase tracking-wider ${isToday ? 'text-blue-600' : 'text-gray-500'}`}>
                    ...
                    <div className="flex-1 p-4 space-y-3">
                      {groupedEvents[date].map((event) => (
                        <div
                          key={event.id}
                          className="flex items-start gap-3 p-3 rounded-xl bg-white border shadow-sm transition-all hover:shadow-md"
                          style={{ borderLeftWidth: '4px', borderLeftColor: event.color }}
                        >
                          <div className="mt-0.5 p-1.5 rounded-md bg-gray-50 border">
