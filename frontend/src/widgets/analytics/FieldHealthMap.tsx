...
interface FieldNode {
  id: number;
  name: string;
  status: 'healthy' | 'warning' | 'critical' | 'resting' | 'unknown';
  occupation: number;
  capacity: number;
  sensors?: SensorData;
...
export const FieldHealthMap = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const mappedFields: FieldNode[] = rawList.map((f: any, idx: number) => ({
        id: f.id ?? (idx + 1),
        name: f.name || f.potrero_nombre || `Potrero ${String(idx + 1).padStart(2, '0')}`,
        status: (['healthy', 'warning', 'critical', 'resting'].includes(f.status) ? f.status : 'unknown') as FieldNode['status'],
        occupation: Number(f.occupation ?? f.ocupacion ?? f.animal_count ?? f.current_animals ?? 0),
        capacity: Number(f.capacity ?? f.capacidad ?? 0),
        sensors: f.sensors,
  ...
  if (!loading && fields.length === 0) {
    return (
      <Card className="min-h-[260px] flex flex-col items-center justify-center border border-border bg-card p-6 text-center">
        <MapIcon className="h-10 w-10 text-muted-foreground mb-4" />
        <CardTitle className="text-lg">Sin datos de potreros</CardTitle>
        <CardDescription className="mt-2 max-w-md">
          El mapa se mostrará cuando existan potreros registrados en la base de datos.
        </CardDescription>
        <Button variant="outline" className="mt-4" onClick={() => navigate('/admin/fields')}>
          Registrar potrero
        </Button>
      </Card>
    );
  ...
  return (
    <Card className="min-h-[550px] max-h-[700px] overflow-hidden rounded-xl border border-border bg-card shadow-sm flex flex-col">
      <CardHeader className="p-4 sm:p-6 border-b border-border/60 bg-gradient-to-r from-primary/5 to-transparent flex flex-row items-center justify-between shrink-0">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            ...
            <CardDescription className="font-bold uppercase tracking-widest text-[11px] opacity-70">Mapa Topográfico de Potreros</CardDescription>
          </div>
        </div>
        <Button
          variant="outline"
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center gap-2">
                      ...
                      <Badge variant="outline" className={cn("text-[11px] font-extrabold uppercase tracking-wider px-2 py-0.5 border fit-clamp max-w-[120px]", getStatusBadgeColor(field.status))}>
            ...
            >
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <Badge variant="outline" className={cn("mt-2 uppercase tracking-widest text-[11px] font-black px-2.5 py-1 border", getStatusBadgeColor(selectedField.status))}>
                    Estado: {getStatusLabel(selectedField.status)}
                  </Badge>
                </div>

                <div className="space-y-3">
                   <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-muted-foreground">Carga Animal</h4>
                ...
                {selectedField.sensors && (
                  <div className="p-3 bg-muted/40 rounded-xl border border-border/60 space-y-2">
                    <h4 className="text-[11px] font-black uppercase tracking-wider text-muted-foreground">Telemetría de Terreno</h4>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-card p-2 rounded-lg border border-border/40">
                        <span className="text-muted-foreground block text-[11px] uppercase font-bold">Temperatura</span>
                        <span className="font-black text-sm">{selectedField.sensors.temp}°C</span>
                      </div>
                      <div className="bg-card p-2 rounded-lg border border-border/40">
                        <span className="text-muted-foreground block text-[11px] uppercase font-bold">Humedad</span>
                      ...
                      {selectedField.sensors.soil_ph && (
                        <div className="bg-card p-2 rounded-lg border border-border/40 col-span-2">
                          <span className="text-muted-foreground block text-[11px] uppercase font-bold">pH del Suelo</span>
      ...
      <div className="p-4 sm:p-5 bg-muted/20 border-t border-border flex flex-wrap items-center justify-between gap-3 shrink-0">
        ...
        <p className="text-[11px] font-black text-muted-foreground/60 uppercase tracking-widest">VillaLuz Monitoreo de Potreros</p>
      </div>
    </Card>
  );
};
