...
const Header: React.FC<HeaderProps> = ({ isSidebarOpen, onToggleSidebar }) => {
  ...
  return (
    <header className="sticky top-0 z-50 w-full h-16 border-b bg-background/95 backdrop-blur shadow-sm transition-all duration-300" role="banner">
      {/* ── Fila principal del header ── */}
      <div className="flex h-16 items-center justify-between px-4 gap-4">

        {/* IZQUIERDA: Botón Sidebar (Móvil) + Logo y Título "VILLA LUZ" (Visible >= lg) + Selector de Finca */}
        <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
          <button
            type="button"
            onClick={onToggleSidebar}
            className="lg:hidden flex-shrink-0 flex items-center justify-center w-9 h-9 rounded-xl border border-border bg-surface hover:bg-state-hover focus-visible:ring-2 focus-visible:ring-primary transition-all shadow-sm"
          ...
          <div className="hidden lg:block h-6 w-[1px] bg-border mx-1 flex-shrink-0" />

          {/* FincaSelector acotado para que no desborde */}
          <div className="min-w-0 max-w-[140px] sm:max-w-[200px] lg:max-w-xs flex-shrink-0">
            <FincaSelector />
          </div>
        </div>

        {/* CENTRO: Barra de Búsqueda Global (Visible >= md) */}
        <div className="hidden md:flex flex-1 max-w-md">
          <GlobalSearchBar />
        </div>

        {/* DERECHA: SyncStatus (4 PENDIENTES) + NotificationCenter + ThemeToggle + UserMenu */}
